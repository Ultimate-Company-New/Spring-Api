...
import springapi.services.ProductReviewService;
import springapi.services.UserLogService;
import springapi.services.interfaces.ProductReviewSubTranslator;

/**
 * Base test class for ProductReviewService tests. Provides common setup, test data, and helper
 * methods for all ProductReview test files.
 */
@ExtendWith(MockitoExtension.class)
abstract class ProductReviewServiceTestBase {

  @Mock protected ProductReviewRepository productReviewRepository;

  @Mock protected UserLogService userLogService;

  @Mock
  protected springapi.filterquerybuilder.ProductReviewFilterQueryBuilder
  ...
  protected void stubProductReviewRepositoryFindByReviewIdAndClientId(
      Long reviewId, Long clientId, ProductReview result) {
    lenient()
        .when(productReviewRepository.findByReviewIdAndClientId(reviewId, clientId))
        .thenReturn(result);
  }

  protected void stubProductReviewRepositoryFindByReviewIdAndClientIdAny(ProductReview result) {
    lenient()
        .when(productReviewRepository.findByReviewIdAndClientId(anyLong(), anyLong()))
        .thenReturn(result);
  }
  ...
  protected void stubProductReviewRepositoryMarkAllDescendantsAsDeleted(int updatedCount) {
    lenient()
        .when(productReviewRepository.markAllDescendantsAsDeleted(eq(TEST_REVIEW_ID), anyString()))
        .thenReturn(updatedCount);
  }

  protected void stubProductReviewRepositoryMarkAllDescendantsAsDeleted(
      Long reviewId, int updatedCount) {
    lenient()
        .when(productReviewRepository.markAllDescendantsAsDeleted(eq(reviewId), anyString()))
        .thenReturn(updatedCount);
  }
