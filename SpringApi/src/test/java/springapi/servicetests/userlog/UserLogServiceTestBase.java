...
/**
 * Base test class for UserLogService tests. Contains common mocks, test data, and setup methods.
 */
@ExtendWith(MockitoExtension.class)
abstract class UserLogServiceTestBase {

  @Mock protected UserLogRepository userLogRepository;

  @Mock protected UserLogFilterQueryBuilder userLogFilterQueryBuilder;

  @Mock protected HttpServletRequest request;

  @InjectMocks protected UserLogService userLogService;

  // For controller delegation tests
  @Mock protected UserLogService mockUserLogService;
  ...
  protected void setUp() {
    ...
    testUserLog =
        new UserLog(
            TEST_USER_ID, TEST_CARRIER_ID, TEST_ACTION, TEST_OLD_VALUE, TEST_NEW_VALUE, "admin");
  ...
  protected void stubUserLogServiceFetchUserLogsInBatches(
      UserLogService mockService,
      UserLogsRequestModel req,
      PaginationBaseResponseModel<UserLogsResponseModel> res) {
    lenient().when(mockService.fetchUserLogsInBatches(req)).thenReturn(res);
  }

  protected void stubUserLogServiceFetchUserLogsInBatchesMock(
      UserLogsRequestModel req, PaginationBaseResponseModel<UserLogsResponseModel> res) {
  ...
  protected void stubServiceThrowsUnauthorizedException() {
    lenient()
        ...
        .thenThrow(new springapi.exceptions.UnauthorizedException("Unauthorized"));
  }
