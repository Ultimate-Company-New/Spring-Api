...
class GetPaymentByIdTest extends PaymentServiceTestBase {
  ...
  /**
   * Purpose: Verify payment repository is queried with the requested payment id. Expected Result:
   * paymentRepository.findById is called exactly once. Assertions: verify interaction count.
   */
  @Test
  ...
  void getPaymentById_verifyPaymentRepositoryInteraction_success() {
    ...
    NotFoundException ex =
        assertThrows(NotFoundException.class, () -> paymentService.getPaymentById(TEST_PAYMENT_ID));
  ...
  /**
   * Purpose: Verify service returns payment details when payment exists and belongs to current
   * client. Expected Result: Returned payment matches repository entity. Assertions: Payment id and
   * client id.
   */
  @Test
  ...
  /**
   * Purpose: Verify missing payment id throws NotFoundException. Expected Result: NotFoundException
  ...
  void getPaymentById_f03_zeroId_failure() {
    ...
    // Act
    NotFoundException ex =
        assertThrows(NotFoundException.class, () -> paymentService.getPaymentById(0L));
  ...
  /**
   * Purpose: Verify max long payment id throws NotFoundException. Expected Result:
   * NotFoundException with payment not found message. Assertions: Exception type and exact message.
   */
  @Test
  ...
  void getPaymentById_f04_maxLongId_failure() {
    ...
    // Act
    NotFoundException ex =
        assertThrows(NotFoundException.class, () -> paymentService.getPaymentById(Long.MAX_VALUE));
  ...
  /**
   * Purpose: Verify access-denied scenario when payment belongs to a different client. Expected
  ...
  void getPaymentById_f05_clientAccessDenied_failure() {
    ...
    BadRequestException ex =
        assertThrows(
            BadRequestException.class, () -> paymentService.getPaymentById(TEST_PAYMENT_ID));
  ...
  /**
   * Purpose: Verify multiple calls independently invoke payment repository. Expected Result:
   * NotFoundException for each call and two repository invocations. Assertions: Exception type and
   * repository invocation count.
   */
  @Test
  ...
  void getPaymentById_f06_multipleCalls_failure() {
    ...
    // Act
    NotFoundException exOne =
        assertThrows(NotFoundException.class, () -> paymentService.getPaymentById(1L));
    NotFoundException exTwo =
   ...
   * NotFoundException with payment not found message for each id. Assertions: Exception type and
   * exact message.
   */
  @Test
  ...
  void getPaymentById_f07_additionalInvalidIds_failure() {
    ...
    for (Long invalidId : invalidIds) {
      ...
      NotFoundException ex =
          assertThrows(NotFoundException.class, () -> paymentService.getPaymentById(invalidId));
  ...
  /**
   * Purpose: Verify receipt download endpoint has expected permission annotation for payment-id
   * based access. Expected Result: Annotation exists and contains VIEW_PURCHASE_ORDERS_PERMISSION.
