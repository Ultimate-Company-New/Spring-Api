...
class FetchUserGroupsInClientInBatchesTest extends UserGroupServiceTestBase {
  ...
  /**
   * Purpose: Verify boolean column filtering works. Expected Result: Success result returned.
  ...
  void fetchUserGroupsInClientInBatches_BooleanColumnFilter_Success() {
    ...
    Page<UserGroup> page =
        new PageImpl<>(Collections.singletonList(testUserGroup), PageRequest.of(0, 10), 1);
  ...
  /**
   * Purpose: Verify no filters (basic pagination) works. Expected Result: Query builder called with
   * correct params. Assertions: verify
   */
  @Test
  ...
  void fetchUserGroupsInClientInBatches_NoFilters_Success() {
    ...
    // Assert
    verify(userGroupFilterQueryBuilder)
        .findPaginatedEntitiesWithMultipleFilters(
            anyLong(), any(), anyString(), any(), anyBoolean(), any());
  }

  /**
   * Purpose: Verify number column filtering works. Expected Result: Success result returned.
   * Assertions: assertNotNull
   */
  @Test
  ...
  /**
   * Purpose: Verify string column filtering works. Expected Result: Success result returned.
   * Assertions: assertNotNull
   */
  @Test
   ...
   * with InvalidColumnName message. Assertions: assertThrows, assertTrue
   */
  @Test
  ...
  void fetchUserGroupsInClientInBatches_InvalidColumn_ThrowsBadRequestException() {
    ...
    assertTrue(
        ex.getMessage()
            .startsWith(
                String.format(
                    ErrorMessages.CommonErrorMessages.INVALID_COLUMN_NAME, "invalidColumn")));
  }

  /**
   * Purpose: Verify invalid logic operator throws BadRequestException. Expected Result:
   * BadRequestException with InvalidLogicOperator message. Assertions: assertThrows, assertEquals
   */
  @Test
  ...
  /**
   * Purpose: Verify invalid pagination throws BadRequestException. Expected Result:
   * BadRequestException with InvalidPagination message. Assertions: assertThrows, assertEquals
   */
  @Test
  ...
  void fetchUserGroupsInClientInBatches_controller_permission_forbidden()
    ...
    Method method =
        UserGroupController.class.getMethod("getUserGroupsInBatches", UserGroupRequestModel.class);
    ...
    assertNotNull(
        annotation, "@PreAuthorize annotation should be present on getUserGroupsInBatches method");
    assertTrue(
