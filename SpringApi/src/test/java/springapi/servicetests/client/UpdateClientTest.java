...
class UpdateClientTest extends ClientServiceTestBase {
  ...
  void updateClient_UpdateLogoImgBB_Success() {
    ...
    ImgbbHelper.ImgbbUploadResponse mockResponse =
        new ImgbbHelper.ImgbbUploadResponse("http://imgbb.com/new.png", "newHash");
  ...
  void updateClient_DuplicateName_ThrowsBadRequestException() {
    ...
    BadRequestException ex =
        assertThrows(
            BadRequestException.class, () -> clientService.updateClient(testClientRequest));

    // Assert
    assertEquals(
        String.format(
            ErrorMessages.ClientErrorMessages.DUPLICATE_CLIENT_NAME_FORMAT,
            testClientRequest.getName()),
        ex.getMessage());
  }
  ...
  void updateClient_InvalidImageLocation_ThrowsBadRequestException() {
    ...
    assertEquals(
        String.format(
            ErrorMessages.ConfigurationErrorMessages.INVALID_IMAGE_LOCATION_CONFIG_FORMAT,
            "invalid"),
        ex.getMessage());
  }
  ...
  void updateClient_NegativeId_ThrowsNotFoundException() {
    ...
    NotFoundException ex =
        assertThrows(NotFoundException.class, () -> clientService.updateClient(testClientRequest));
  ...
  void updateClient_NullRequest_ThrowsBadRequestException() {
    ...
    // Act & Assert
    BadRequestException ex =
        assertThrows(BadRequestException.class, () -> clientService.updateClient(null));
  ...
  void updateClient_p03_controller_permission_forbidden() throws NoSuchMethodException {
    ...
    var method =
        ClientController.class.getMethod(
            "updateClient", Long.class, springapi.models.requestmodels.ClientRequestModel.class);
    testClientRequest.setClientId(TEST_CLIENT_ID);
    stubServiceUpdateClientDoNothing();
    ...
    // Assert
    assertNotNull(
        preAuthorizeAnnotation, "updateClient method should have @PreAuthorize annotation");
