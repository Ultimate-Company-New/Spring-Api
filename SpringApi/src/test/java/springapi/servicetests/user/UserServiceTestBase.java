...
/**
 * Base test class for UserService tests. Contains common mocks, test data, and centralized stubbing
 * methods.
 */
...
abstract class UserServiceTestBase {
  ...
  @Mock protected PermissionRepository permissionRepository;

  @Mock protected UserClientPermissionMappingRepository userClientPermissionMappingRepository;

  @Mock protected UserFilterQueryBuilder userFilterQueryBuilder;

  @Mock protected UserGroupUserMapRepository userGroupUserMapRepository;
  ...
  protected springapi.controllers.UserController userController;
  protected springapi.controllers.UserController userControllerWithMock;

  protected User testUser;
  protected UserRequestModel testUserRequest;
  protected static final Long TEST_USER_ID = 1L;
  protected static final Long TEST_CLIENT_ID = 100L;
  protected static final String[] STRING_OPERATORS = {
  ...
  void setUp() {
    ...
    userController = new springapi.controllers.UserController(userService);
    userControllerWithMock = new springapi.controllers.UserController(mockUserService);
  ...
  protected void stubUserRepositoryFindByEmailWithAllRelations(User result) {
    lenient()
        .when(userRepository.findByEmailWithAllRelations(anyString(), anyLong()))
        .thenReturn(result);
  }

  protected void stubUserRepositoryFindByIdWithAllRelations(User result) {
    lenient()
        .when(userRepository.findByIdWithAllRelations(anyLong(), anyLong()))
        .thenReturn(result);
  }
  ...
  protected void stubUserClientPermissionMappingRepositorySaveAll(
  ...
  protected void stubUserClientPermissionMappingRepositoryDeleteByUserIdAndClientId() {
    lenient()
        .doNothing()
        .when(userClientPermissionMappingRepository)
        .deleteByUserIdAndClientId(anyLong(), anyLong());
  }
  ...
  protected MockedConstruction<springapi.helpers.EmailTemplates> mockedEmailTemplates;
  protected MockedConstruction<springapi.helpers.FirebaseHelper> mockedFirebaseHelper;
  protected MockedConstruction<springapi.helpers.ImgbbHelper> mockedImgbbHelper;
    ...
    mockedEmailTemplates =
        org.mockito.Mockito.mockConstruction(
            ...
            (mock, context) -> {
              lenient()
                  .when(
                      mock.sendNewUserAccountConfirmation(
  ...
  protected void stubMockUserServiceBulkCreateUsersAsyncThrowsUnauthorized(String message) {
    ...
    lenient()
        .doThrow(new UnauthorizedException(unauthorizedMessage))
        .when(mockUserService)
        .bulkCreateUsersAsync(anyList(), anyLong(), anyString(), anyLong());
  }
  ...
  protected void stubMockUserServiceBulkCreateUsersAsync(List<UserRequestModel> users) {
    lenient()
        .doNothing()
        .when(mockUserService)
        .bulkCreateUsersAsync(eq(users), anyLong(), anyString(), anyLong());
  }
  ...
  protected void stubMockUserServiceUpdateUserThrowsUnauthorized(String message) {
    ...
    lenient()
        .doThrow(new UnauthorizedException(unauthorizedMessage))
        .when(mockUserService)
        .updateUser(any(UserRequestModel.class));
  }
  ...
  protected void stubMockUserServiceCreateUserThrowsUnauthorized(String message) {
    ...
    lenient()
        .doThrow(new UnauthorizedException(unauthorizedMessage))
        .when(mockUserService)
        .createUser(any(UserRequestModel.class));
  }
  ...
  protected void stubMockUserServiceCreateUserThrowsForbidden(String message) {
    ...
    lenient()
        .doThrow(new PermissionException(forbiddenMessage))
        .when(mockUserService)
        .createUser(any(UserRequestModel.class));
  }
  ...
  protected void stubMockUserServiceFetchUsersInCarrierInBatches(
      PaginationBaseResponseModel<UserResponseModel> response) {
    lenient()
        .when(mockUserService.fetchUsersInCarrierInBatches(any(UserRequestModel.class)))
        .thenReturn(response);
  }
