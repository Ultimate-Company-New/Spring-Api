...
class ConfirmEmailTest extends UserServiceTestBase {
  ...
  /**
   * Purpose: Verify repository is called exactly once to find user. Expected Result: findById is
   * called once. Assertions: verify(userRepository, times(1)).findById(TEST_USER_ID);
   */
  @Test
  ...
  /**
   * Purpose: Verify null emailConfirmed is treated as false and updated. Expected Result: Email is
   * confirmed successfully. Assertions: assertTrue(user.getEmailConfirmed());
   */
  @Test
  ...
  /**
   * Purpose: Verify successful email confirmation. Expected Result: emailConfirmed is set to true.
  ...
  /**
   * Purpose: Verify already confirmed email throws BadRequestException. Expected Result:
   * BadRequestException with "Account has already been confirmed" message.
   */
  @Test
  ...
  void confirmEmail_alreadyConfirmed_throwsBadRequestException() {
    ...
    BadRequestException ex =
        assertThrows(
            BadRequestException.class, () -> userService.confirmEmail(TEST_USER_ID, "valid-token"));
  ...
  /**
   * Purpose: Verify empty token string throws BadRequestException. Expected Result:
   * BadRequestException with "Invalid token" message.
   */
  @Test
  ...
  void confirmEmail_emptyToken_throwsBadRequestException() {
    ...
    // Act
    BadRequestException ex =
        assertThrows(BadRequestException.class, () -> userService.confirmEmail(TEST_USER_ID, ""));
  ...
  /**
   * Purpose: Verify max long user ID throws NotFoundException when not found. Expected Result:
   * NotFoundException with "Invalid User Id" message.
   */
  @Test
  ...
  void confirmEmail_maxLongUserId_throwsNotFoundException() {
    ...
    NotFoundException ex =
        assertThrows(
            NotFoundException.class, () -> userService.confirmEmail(Long.MAX_VALUE, "valid-token"));
  ...
  /**
   * Purpose: Verify negative user ID throws NotFoundException. Expected Result: NotFoundException
   * with "Invalid User Id" message.
   */
  @Test
  ...
  void confirmEmail_negativeUserId_throwsNotFoundException() {
    ...
    // Act
    NotFoundException ex =
        assertThrows(NotFoundException.class, () -> userService.confirmEmail(-1L, "valid-token"));
  ...
  /**
   * Purpose: Verify null token throws BadRequestException. Expected Result: BadRequestException
   * with "Invalid token" message.
   */
  @Test
  ...
  void confirmEmail_nullToken_throwsBadRequestException() {
    ...
    BadRequestException ex =
        assertThrows(
            BadRequestException.class, () -> userService.confirmEmail(TEST_USER_ID, "any-token"));
  ...
  /**
   * Purpose: Verify token mismatch throws BadRequestException. Expected Result: BadRequestException
   * with "Invalid token" message.
   */
  @Test
  ...
  void confirmEmail_tokenMismatch_throwsBadRequestException() {
    ...
    BadRequestException ex =
        assertThrows(
            BadRequestException.class, () -> userService.confirmEmail(TEST_USER_ID, "wrong-token"));
  ...
  /**
   * Purpose: Verify non-existent user throws NotFoundException. Expected Result: NotFoundException
   * with "Invalid User Id" message.
   */
  @Test
  ...
  void confirmEmail_userNotFound_throwsNotFoundException() {
    ...
    NotFoundException ex =
        assertThrows(
            NotFoundException.class, () -> userService.confirmEmail(TEST_USER_ID, "valid-token"));
  ...
  /**
   * Purpose: Verify zero user ID throws NotFoundException. Expected Result: NotFoundException with
  ...
  void confirmEmail_zeroUserId_throwsNotFoundException() {
    ...
    // Act
    NotFoundException ex =
        assertThrows(NotFoundException.class, () -> userService.confirmEmail(0L, "valid-token"));
  ...
  /**
   * Purpose: Verify controller has @PreAuthorize annotation. Expected Result: Annotation is present
   * and permitAll(). Assertions: assertNotNull, assertEquals("permitAll()", annotation.value()),
   * HTTP status OK.
   */
  @Test
