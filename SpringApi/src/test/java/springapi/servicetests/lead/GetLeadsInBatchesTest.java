 ...
 * validation logic. * Test Count: 5 tests
 */
...
class GetLeadsInBatchesTest extends LeadServiceTestBase {
  ...
  void getLeadsInBatches_SingleComprehensiveTest_Success() {
    ...
    // Act & Assert
    BadRequestException pagEx =
        assertThrows(
            BadRequestException.class, () -> leadService.getLeadsInBatches(testLeadRequest));
    ...
    Page<Lead> leadPage =
        new PageImpl<>(Collections.singletonList(testLead), PageRequest.of(0, 10), 1);
    stubLeadFilterQueryBuilderFindPaginatedEntities(leadPage);
    PaginationBaseResponseModel<LeadResponseModel> result =
  ...
  /**
   * Purpose: Reject invalid pagination requests. Expected Result: BadRequestException is thrown.
  ...
  void getLeadsInBatches_p03_controller_permission_unauthorized() {
    ...
    LeadController controller =
        new LeadController(leadServiceMock); // leadServiceMock is now generic
