...
class GetMessagesByUserIdTest extends MessageServiceTestBase {
  ...
  /**
   * Purpose: Verify that pagination works correctly with default page size (0 in request).
   * Scenario: Call getMessagesByUserId with start/end at 0. Expected: Repository called with
   * correct Pageable and result returned.
   */
  @Test
  ...
  /**
   * Purpose: Verify that isRead flag is correctly set based on the presence of a read map.
   * Scenario: Retrieve multiple messages where some have a read map entry and others don't.
  ...
  void getMessagesByUserId_MixedReadStatus_Success() {
    ...
    // m1 read, m2 unread
    stubMessageUserReadMapRepositoryFindByMessageIdAndUserId(
        1L, TEST_USER_ID, new MessageUserReadMap());
  ...
  /**
   * Purpose: Verify that isRead is set to true when a read mapping exists in the database.
   * Scenario: Retrieve a message where the user has already marked it as read. Expected:
   * result.isRead property is true.
   */
  @Test
  ...
  /**
   * Purpose: Verify successful retrieval of messages for a valid user. Scenario: Call
   * getMessagesByUserId with correct user ID and pagination. Expected: List of messages returned
   * with correct mapping and unread status.
   */
  @Test
  ...
  /**
   * Purpose: Verify that the start parameter is correctly converted to Pageable offset. Scenario:
   * Set start=50 and end=60 in pagination request. Expected: Repository is called with a Pageable
   * whose offset matches the start value.
   */
  @Test
  ...
  void getMessagesByUserId_VerifyOffset_Success() {
    ...
    verify(messageRepository)
        .findMessagesByUserIdPaginated(anyLong(), anyLong(), pageableCaptor.capture());
    assertEquals(50, pageableCaptor.getValue().getOffset());
  }

  /**
   * Purpose: Verify that a zero-range pagination request (start == end) is handled correctly.
   * Scenario: Call getMessagesByUserId with start=10 and end=10. Expected: Method executes without
   * error and makes repository call.
   */
  @Test
  ...
  /**
   * Purpose: Reject retrieval request when user ID is negative. Expected Result:
   * BadRequestException with user invalid ID error code.
   */
  @Test
  ...
  /**
   * Purpose: Verify that passing a null request object results in a NullPointerException. Expected
  ...
  void getMessagesByUserId_NullPaginationRequest_ThrowsNullPointerException() {
    ...
    NullPointerException ex =
        assertThrows(NullPointerException.class, () -> messageService.getMessagesByUserId(null));
    assertEquals(ErrorMessages.MessagesErrorMessages.NULL_PAGINATION_REQUEST, ex.getMessage());
  }

  /**
   * Purpose: Propagate repository exceptions when paginated message lookup fails. Expected Result:
  ...
  void getMessagesByUserId_RepositoryError_Propagates() {
    ...
    paginationRequest.setId(TEST_USER_ID);
    stubUserRepositoryFindByUserIdAndClientId(Optional.of(testUser));
    stubMessageRepositoryFindMessagesByUserIdPaginatedThrows(
        ErrorMessages.MessagesErrorMessages.PAGE_ERROR);

    // Act & Assert
    RuntimeException ex =
        assertThrows(
            RuntimeException.class, () -> messageService.getMessagesByUserId(paginationRequest));
    assertEquals(ErrorMessages.MessagesErrorMessages.PAGE_ERROR, ex.getMessage());
  }

  /**
   * Purpose: Verify that unauthorized access checks are handled at controller level. Scenario:
   * Service layer is called for a valid user (service doesn't enforce auth). Expected: Service
   * returns valid result or empty list without throwing security exceptions.
   */
  @Test
  ...
  /**
   * Purpose: Reject retrieval request when user record is not found in database. Expected Result:
  ...
  void getMessagesByUserId_UserRepositoryError_Propagates() {
    ...
    PaginationBaseRequestModel paginationRequest = createValidPaginationRequest();
    paginationRequest.setId(TEST_USER_ID);
    stubUserRepositoryFindByUserIdAndClientIdThrows(
        ErrorMessages.MessagesErrorMessages.USER_DB_ERROR);
  ...
  /**
   * Purpose: Reject retrieval request when user ID is zero. Expected Result: BadRequestException
  ...
  /**
   * Purpose: Verify that the getMessagesByUserId controller method is protected by
   * correct @PreAuthorize permission. Expected: Method has @PreAuthorize referencing
  ...
  void getMessagesByUserId_VerifyPreAuthorizeAnnotation_Success() throws NoSuchMethodException {
    ...
    // Assert
    assertNotNull(
        preAuthorizeAnnotation, "getMessagesByUserId method should have @PreAuthorize annotation");
  ...
  /**
   * Purpose: Verify that the controller correctly delegates getMessagesByUserId calls to the
