...
class CreateUserTest extends UserServiceTestBase {
    ...
    /**
     * Purpose: Verify ImgBB flow fails when API key is missing for profile picture upload.
     * Expected Result: BadRequestException with configuration message.
     * Assertions: Exact exception message.
     */
    @Test
    ...
    void createUser_ImgbbMissingApiKeyForProfilePicture_Failure() {
        ...
        setupCreateUserMocks();
        org.springframework.test.util.ReflectionTestUtils.setField(userService, "imageLocation", "imgbb");
        ...
        // Assert
        assertEquals(ErrorMessages.ConfigurationErrorMessages.IMGBB_API_KEY_NOT_CONFIGURED, exception.getMessage());
    }

    /**
     * Purpose: Verify ImgBB flow fails when upload response is null.
     * Expected Result: BadRequestException with ER010 message.
     * Assertions: Exact exception message.
     */
    @Test
    ...
    /**
     * Purpose: Verify Firebase flow fails when Google credentials are missing.
     * Expected Result: BadRequestException with ER011 message.
     * Assertions: Exact exception message.
     */
    @Test
    ...
    void createUser_FirebaseMissingGoogleCredThrowsEr011_Failure() {
        ...
        testUserRequest.setProfilePictureBase64("base64-image");
        setupCreateUserMocks();
        org.springframework.test.util.ReflectionTestUtils.setField(userService, "imageLocation", "firebase");
    ...
    /**
     * Purpose: Verify invalid imageLocation setting is rejected.
     * Expected Result: BadRequestException with invalid configuration message.
     * Assertions: Exception message contains invalid imageLocation marker.
     */
    @Test
    ...
    void createUser_InvalidImageLocationConfiguration_Failure() {
        ...
        testUserRequest.setProfilePictureBase64("base64-image");
        setupCreateUserMocks();
        org.springframework.test.util.ReflectionTestUtils.setField(userService, "imageLocation", "invalid-store");
     ...
     * Expected Result: BadRequestException with failed confirmation message.
     * Assertions: Exception message contains failed confirmation constant.
     */
    @Test
    ...
    void createUser_ConfirmationEmailFalseThrows_Failure() {
        ...
        mockedEmailTemplates = org.mockito.Mockito.mockConstruction(com.example.SpringApi.Helpers.EmailTemplates.class,
                (mock, context) -> when(mock.sendNewUserAccountConfirmation(anyLong(), anyString(), anyString(), anyString()))
                        .thenReturn(false));
        ...
        // Assert
        assertTrue(exception.getMessage().contains(ErrorMessages.CommonErrorMessages.FAILED_TO_SEND_CONFIRMATION_EMAIL));
    }

    /**
     * Purpose: Verify createUser wraps thrown confirmation email errors.
     * Expected Result: BadRequestException with wrapped email exception reason.
     * Assertions: Exact exception message.
     */
    @Test
