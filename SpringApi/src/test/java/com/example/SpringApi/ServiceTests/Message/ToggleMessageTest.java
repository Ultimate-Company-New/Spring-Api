...
class ToggleMessageTest extends MessageServiceTestBase {
  ...
  /**
   * Purpose: Verify that an active message can be successfully toggled to a deleted state.
   * Scenario: Call toggleMessage for a message where isDeleted is currently false. Expected:
   * Message is saved with isDeleted set to true.
   */
  @Test
  ...
  /**
   * Purpose: Verify that a deleted message can be successfully restored to an active state.
   * Scenario: Call toggleMessage for a message where isDeleted is currently true. Expected: Message
   * is saved with isDeleted set to false.
   */
  @Test
  ...
  /**
   * Purpose: Verify that successive calls to toggleMessage flip the deletion state correctly each
   * time. Scenario: Call toggleMessage twice on the same message object. Expected: state changes
  ...
  /**
   * Purpose: Verify that toggling a message triggers correct activity logging. Expected Result:
   * UserLogService is called with a descriptive log message.
   */
  @Test
  ...
  /**
   * Purpose: Reject toggle request when specified message record does not exist in database.
  ...
  /**
   * Purpose: Reject toggle request when provided message ID is negative. Expected Result:
  ...
  void toggleMessage_NegativeId_ThrowsNotFoundException() {
    ...
    // Act & Assert
    assertThrowsNotFound(
        ErrorMessages.MessagesErrorMessages.INVALID_ID, () -> messageService.toggleMessage(-1L));
  }

  /**
   * Purpose: Propagate repository exceptions when database save fails during toggle. Expected
   ...
   * Service layer is called for a valid message (service doesn't enforce auth). Expected Result:
  ...
  /**
   * Purpose: Reject toggle request when provided message ID is zero. Expected Result:
   * NotFoundException with message invalid ID error code.
   */
  @Test
  ...
  void toggleMessage_ZeroId_ThrowsNotFoundException() {
    ...
    // Act & Assert
    assertThrowsNotFound(
        ErrorMessages.MessagesErrorMessages.INVALID_ID, () -> messageService.toggleMessage(0L));
  }
  ...
  /**
   * Purpose: Verify that the toggleMessage controller method is protected by correct @PreAuthorize
   * permission. Expected: Method has @PreAuthorize referencing DELETE_MESSAGES_PERMISSION.
   */
  @Test
  ...
  void toggleMessage_VerifyPreAuthorizeAnnotation_Success() throws NoSuchMethodException {
    ...
    assertTrue(
        annotation.value().contains(Authorizations.DELETE_MESSAGES_PERMISSION),
        "@PreAuthorize should reference DELETE_MESSAGES_PERMISSION");
  }

  /**
   * Purpose: Verify that the controller correctly delegates toggleMessage calls to the service
