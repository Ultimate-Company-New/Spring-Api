...
class GetMessageDetailsByIdTest extends MessageServiceTestBase {
  ...
  /**
   * Purpose: Verify message details are correctly mapped from database model to response model.
   * Scenario: Retrieve message and verify HTML description is preserved in mapping. Expected:
   * Message response contains exact database values for title and description.
   */
  @Test
  ...
  /**
   * Purpose: Verify that a message can be successfully retrieved by its ID. Scenario: Call
   * getMessageDetailsById with a valid existing message ID. Expected: Correct message details
   * returned with matching ID and title.
   */
  @Test
  ...
  /**
   * Purpose: Verify that the sendAsEmail flag is correctly mapped in the response. Scenario:
   * Retrieve message where sendAsEmail is set to true. Expected: Result object has sendAsEmail
  ...
  /**
   * Purpose: Verify that the repository method for retrieving message with targets is called.
   * Scenario: Call getMessageDetailsById for any valid ID. Expected:
   * messageRepository.findByMessageIdAndClientIdWithTargets is invoked with provided ID.
   */
  @Test
  ...
  void getMessageDetailsById_WithTargets_Success() {
    ...
    verify(messageRepository)
        .findByMessageIdAndClientIdWithTargets(TEST_MESSAGE_ID, TEST_CLIENT_ID);
  }
  ...
  /**
   * Purpose: Reject retrieval request when message ID does not exist. Expected Result:
   * NotFoundException with ER007/InvalidId error message.
   */
  @Test
  ...
  /**
   * Purpose: Reject retrieval request when message ID is negative. Expected Result:
  ...
  /**
   * Purpose: Propagate repository exceptions when message lookup fails. Expected Result:
  ...
  void getMessageDetailsById_RepositoryError_Propagates() {
    ...
    RuntimeException ex =
        assertThrows(
            RuntimeException.class, () -> messageService.getMessageDetailsById(TEST_MESSAGE_ID));
   ...
   * Service layer is called for a valid ID (service does not enforce auth). Expected: Service
   * returns valid result without throwing exceptions.
   */
  @Test
  ...
  /**
   * Purpose: Reject retrieval request when message ID is zero. Expected Result: BadRequestException
   * with InvalidId error code.
   */
  @Test
  ...
  /**
   * Purpose: Verify that the getMessageDetailsById controller method is protected by
  ...
  /**
   * Purpose: Verify that the controller correctly delegates getMessageDetailsById calls to the
   * service layer. Expected: Service method called with correct ID and HTTP 200 returned.
   */
  @Test
