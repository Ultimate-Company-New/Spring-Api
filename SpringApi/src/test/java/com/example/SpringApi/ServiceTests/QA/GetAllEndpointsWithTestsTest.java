...
class GetAllEndpointsWithTestsTest extends QAServiceTestBase {
    ...
    void getAllEndpointsWithTests_s03_servicesOrdered_byName() {
        ...
        // Assert
        assertNotNull(services);
        Set<String> uniqueServiceNames = services.stream()
                .map(QAResponseModel::getServiceName)
    ...
    void getAllEndpointsWithTests_s04_eachService_hasControllerMapping() {
        ...
        assertTrue(services.stream()
                .allMatch(service -> service.getControllerName() != null && service.getControllerName().endsWith("Controller")));
        assertTrue(services.stream()
                .map(QAResponseModel::getControllerName)
                .distinct()
                .count() >= 1);
    }
    ...
    void getAllEndpointsWithTests_s06_serviceInfo_containsControllerName() {
        ...
        assertFalse(services.isEmpty());
        assertTrue(services.stream()
                .allMatch(service -> service.getControllerName() != null && !service.getControllerName().isBlank()));
        assertEquals(services.size(), services.stream().filter(service -> service.getControllerName() != null).count());
    }
    ...
    void getAllEndpointsWithTests_s07_serviceInfo_containsBasePath() {
        ...
        assertFalse(services.isEmpty());
        assertTrue(services.stream()
                .allMatch(service -> service.getBasePath() != null && service.getBasePath().startsWith("/api/")));
    ...
    void getAllEndpointsWithTests_s10_methodsWithoutTests_showZeroCoverage() {
        ...
        assertNotNull(services);
        assertTrue(services.stream()
                .allMatch(service -> service.getCoveragePercentage() >= 0.0 && service.getCoveragePercentage() <= 100.0));
        assertTrue(services.stream().noneMatch(service -> Double.isNaN(service.getCoveragePercentage())));
    }
    ...
    /**
     * Purpose: Verify latest test results are loaded and attached when available.
     * Expected Result: Service scan succeeds and repository is queried for service results.
     * Assertions: Result list is returned and repository method is invoked with normalized service name.
     */
    @Test
    void getAllEndpointsWithTests_s13_latestResultsPresent_queriesLatestResultRepository() {
        // Arrange
        LatestTestResult latestResult = createLatestTestResult(101L, "QAService", "getDashboardData_s01_success");
        ...
        assertNotNull(services);
        assertFalse(services.isEmpty());
        verify(latestTestResultRepository).findByClientIdAndServiceNameOrderByTestMethodNameAsc(
                anyLong(),
                eq("QAService"));
    }
