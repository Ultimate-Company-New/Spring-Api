 ...
 * validation scenarios. * Test Count: 16 tests
 */
...
class BulkCreateLeadsTest extends LeadServiceTestBase {
  ...
  /**
   * Purpose: Verify bulk creation succeeds when all provided leads are valid. Expected Result:
  ...
  /**
   * Purpose: Verify system handles large batches of leads successfully. Expected Result: System
   * processes all 50 leads without error. Assertions: Result counts match the large input list
   * size.
   */
  @Test
  ...
  /**
   * Purpose: Verify results for mixed valid and invalid leads. Expected Result: Valid leads are
   * saved, invalid leads are reported as failures. Assertions: Success count and failure count
   * reflect the split input.
   */
  @Test
  ...
  /**
   * Purpose: Verify results for partial success scenarios with three valid leads. Expected Result:
   * Three successes and one failure reported. Assertions: Counts match expectations based on manual
   * validity setup.
   */
  @Test
  ...
  /**
   * Purpose: Verify creation of a single lead via the bulk creation method. Expected Result: Single
   * lead is saved successfully. Assertions: Total requested is 1 and success count is 1.
   */
  @Test
  ...
  /**
   * Purpose: Verify logging is performed during bulk create operations. Expected Result: User log
   * entries are created for the process. Assertions: logData is invoked at least once.
   */
  @Test
  ...
  /**
   * Purpose: Verify notification logic is skipped for synchronous bulk creation. Expected Result:
   * Message service is never called for context messages. Assertions: createMessageWithContext call
   * count is zero.
   */
  @Test
  ...
  /**
   * Purpose: Verify result when all provided leads in the batch are invalid. Expected Result:
   * Result model indicates 100% failure rate. Assertions: Success count is zero and repository save
   * is never called.
   */
  @Test
  ...
  /**
   * Purpose: Reject empty lead list for bulk operations. Expected Result: BadRequestException is
   * thrown. Assertions: Error message matches ListCannotBeNullOrEmpty.
   */
  @Test
  ...
  void bulkCreateLeads_EmptyList_ThrowsBadRequestException() {
    ...
    BadRequestException ex =
        assertThrows(BadRequestException.class, () -> leadService.bulkCreateLeads(emptyLeads));
    assertEquals(
  ...
  /**
   * Purpose: Reject invalid email format within a batch. Expected Result: Specific lead is marked
   * as failed in the result set. Assertions: Failure count is incremented.
   */
  @Test
  ...
  /**
   * Purpose: Reject leads missing required fields in a bulk batch. Expected Result: Leads with
   * missing fields are reported as failures. Assertions: Repository save is skipped for invalid
   * entries.
   */
  @Test
  ...
  /**
   * Purpose: Reject null lead list for bulk operations. Expected Result: BadRequestException is
   * thrown. Assertions: Error message matches ListCannotBeNullOrEmpty.
   */
  @Test
  ...
  void bulkCreateLeads_NullList_ThrowsBadRequestException() {
    ...
    // Act & Assert
    BadRequestException ex =
        assertThrows(BadRequestException.class, () -> leadService.bulkCreateLeads(null));
    assertEquals(
        String.format(ErrorMessages.CommonErrorMessages.LIST_CANNOT_BE_NULL_OR_EMPTY, "Lead"),
        ex.getMessage());
  }
   ...
   * Given: Batch of 5 leads, 1 invalid; stub repository to handle this When: bulkCreateLeads is
   * called Then: Returned result contains failure entry for invalid lead
   */
  @Test
