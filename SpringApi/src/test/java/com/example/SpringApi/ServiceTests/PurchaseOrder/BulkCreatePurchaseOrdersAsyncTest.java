...
class BulkCreatePurchaseOrdersAsyncTest extends PurchaseOrderServiceTestBase {
  ...
  /**
   * Purpose: Verify controller has @PreAuthorize for bulkCreatePurchaseOrders. Expected Result:
   * Annotation exists and includes INSERT_PURCHASE_ORDERS_PERMISSION. Assertions: Annotation is
   * present and contains permission.
   */
  @Test
  @DisplayName("bulkCreatePurchaseOrdersAsync - Verify @PreAuthorize Annotation")
  void bulkCreatePurchaseOrdersAsync_VerifyPreAuthorizeAnnotation_Success()
    ...
    // Assert
    assertNotNull(
        annotation, "@PreAuthorize annotation should be present on bulkCreatePurchaseOrders");
    assertTrue(
        annotation.value().contains(Authorizations.INSERT_PURCHASE_ORDERS_PERMISSION),
