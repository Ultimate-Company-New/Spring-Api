...
class BulkCreatePackagesTest extends PackageServiceTestBase {
  ...
  void bulkCreatePackages_f03_EmptyList_ThrowsBadRequestException() {
    ...
    BadRequestException exception =
        assertThrows(BadRequestException.class, () -> packageService.bulkCreatePackages(packages));

    // Assert
    assertEquals(
        String.format(ErrorMessages.CommonErrorMessages.LIST_CANNOT_BE_NULL_OR_EMPTY, "Package"),
  ...
  void bulkCreatePackages_p01_controller_permission_unauthorized() {
    ...
    PackageController controller =
        new PackageController(packageServiceMock, concretePackageServiceMock);
    ...
    verify(packageServiceMock, never())
  ...
  void bulkCreatePackages_p03_WithValidRequest_DelegatesToService() {
    ...
    ResponseEntity<?> response =
        controller.bulkCreatePackages(List.of(createValidPackageRequest()));

    // Assert
    verify(packageServiceMock)
        .bulkCreatePackagesAsync(anyList(), anyLong(), anyString(), anyLong());
...
class BulkCreatePackagesDuplicateTests extends PackageServiceTestBase {
   ...
   * Expected Result: All duplicates are saved successfully (if business logic allows). Assertions:
   * Success count equals number of duplicate packages.
   */
  @Test
  ...
  /**
   * Purpose: Verify successful bulk creation when all provided packages are valid. Expected Result:
   * Result model indicates 100% success rate. Assertions: Total requested, success count, and
   * repository save count match.
   */
  @Test
  @DisplayName("Bulk Create Packages - All Valid - Success")
   ...
   * Expected Result: Valid packages are saved, invalid ones are reported. Assertions: Failure count
   * is greater than 0 for alternating valid/invalid pattern.
   */
  @Test
  @DisplayName("Bulk Create Packages - Alternating Valid Invalid - Partial Success")
  ...
  /**
   * Purpose: Verify bulk creation handles extremely large batch (1000 items). Expected Result: All
   * 1000 packages are processed successfully. Assertions: Total requested equals 1000 and success
   * count equals 1000.
   */
  @Test
  ...
  /**
   * Purpose: Verify system handles large batches (100 items) correctly. Expected Result: All 100
   * packages are processed and success is recorded. Assertions: Total requested count matches 100.
   */
  @Test
  @DisplayName("Bulk Create Packages - Large Batch - Success")
  ...
  /**
   * Purpose: Verify partial success when the list contains both valid and invalid packages.
   * Expected Result: Valid packages are saved, invalid ones are reported as failures. Assertions:
   * Success count is 2 and failure count is 1.
   */
  @Test
  @DisplayName("Bulk Create Packages - Mixed Invalid and Valid - Partial Success")
  ...
  /**
   * Purpose: Verify creation of a single valid package via bulk method. Expected Result: Single
   * success recorded. Assertions: Success count is 1 and failure count is 0.
   */
  @Test
  @DisplayName("Bulk Create Packages - Single Valid Item - Success")
  ...
  /**
   * Purpose: Verify result when all items in the batch have invalid names. Expected Result: All
   * items are reported as failures. Assertions: Failure count equals the total requested count.
   */
  @Test
  @DisplayName("Bulk Create Packages - All Invalid Names - All Fail")
  ...
  /**
   * Purpose: Handle database runtime errors during individual package processing. Expected Result:
   * Error is caught and recorded as a failure for that specific item. Assertions: Failure count is
   * 1.
   */
  @Test
  @DisplayName("Bulk Create Packages - Database Error - Records Failure")
  ...
  /**
   * Purpose: Reject bulk creation when the provided list is empty. Expected Result:
   * BadRequestException is thrown. Assertions: Error message matches
   * CommonErrorMessages.LIST_CANNOT_BE_NULL_OR_EMPTY.
   */
  @Test
  @DisplayName("Bulk Create Packages - Empty List - Throws BadRequestException")
  ...
  /**
   * Purpose: Reject bulk creation for invalid attribute: Breadth Zero. Expected Result: Failure
   * count is 1. Assertions: Result failure count is 1.
   */
  @Test
  @DisplayName("Bulk Create Packages - Invalid Breadth Zero - Fails")
  ...
  /**
   * Purpose: Reject bulk creation for invalid attribute: Height Zero. Expected Result: Failure
   * count is 1. Assertions: Result failure count is 1.
   */
  @Test
  @DisplayName("Bulk Create Packages - Invalid Height Zero - Fails")
  ...
  /**
   * Purpose: Reject bulk creation for invalid attribute: Length Zero. Expected Result: Failure
   * count is 1. Assertions: Result failure count is 1.
   */
  @Test
  @DisplayName("Bulk Create Packages - Invalid Length Zero - Fails")
  ...
  /**
   * Purpose: Reject bulk creation for invalid attribute: Negative Weight. Expected Result: Failure
   * count is 1. Assertions: Result failure count is 1.
   */
  @Test
  @DisplayName("Bulk Create Packages - Invalid Negative Weight - Fails")
  ...
  /**
   * Purpose: Reject bulk creation when the provided list is null. Expected Result:
   * BadRequestException is thrown. Assertions: Error message matches
   * CommonErrorMessages.LIST_CANNOT_BE_NULL_OR_EMPTY.
   */
  @Test
  ...
  void bulkCreatePackages_f17_NullList_ThrowsBadRequestException() {
    ...
    // Act
    BadRequestException exception =
        assertThrows(BadRequestException.class, () -> packageService.bulkCreatePackages(null));
  ...
  /**
   * Purpose: Reject bulk creation when the package name is null. Expected Result: Failure count is
   * 1. Assertions: Result failure count is 1.
   */
  @Test
  @DisplayName("Bulk Create Packages - Null Package Name - Fails")
  ...
  /**
   * Purpose: Verify controller delegates bulkCreatePackages to service. Expected Result: Returns
   * HTTP 200 OK when delegation succeeds. Assertions: Response status is 200 OK.
   */
  @Test
  @DisplayName("bulkCreatePackages - Controller delegates to service")
