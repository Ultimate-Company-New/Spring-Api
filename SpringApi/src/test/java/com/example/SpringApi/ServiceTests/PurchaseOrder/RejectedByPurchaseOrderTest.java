...
class RejectedByPurchaseOrderTest extends PurchaseOrderServiceTestBase {
  ...
  /**
   * Purpose: Verify reject succeeds when not already rejected. Expected Result: Purchase order is
  ...
  /**
   * Purpose: Reject when already rejected. Expected Result: BadRequestException is thrown.
  ...
  void rejectedByPurchaseOrder_AlreadyRejected_Failure() {
    ...
    assertThrowsBadRequest(
        ErrorMessages.PurchaseOrderErrorMessages.ALREADY_REJECTED,
        () -> purchaseOrderService.rejectedByPurchaseOrder(TEST_PO_ID));
  }

  /**
   * Purpose: Reject for missing purchase order. Expected Result: NotFoundException is thrown.
  ...
  /**
   * Purpose: Verify controller has @PreAuthorize for rejectedByPurchaseOrder. Expected Result:
  ...
  void rejectedByPurchaseOrder_VerifyPreAuthorizeAnnotation_Success() throws NoSuchMethodException {
    ...
    // Assert
    assertNotNull(
        annotation, "@PreAuthorize annotation should be present on rejectedByPurchaseOrder");
    assertTrue(
