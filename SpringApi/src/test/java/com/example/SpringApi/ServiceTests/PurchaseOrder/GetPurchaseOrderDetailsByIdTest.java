...
class GetPurchaseOrderDetailsByIdTest extends PurchaseOrderServiceTestBase {
  ...
  /**
   * Purpose: Verify controller has @PreAuthorize for getPurchaseOrderDetailsById. Expected Result:
   * Annotation exists and includes VIEW_PURCHASE_ORDERS_PERMISSION. Assertions: Annotation is
   * present and contains permission.
   */
  @Test
  @DisplayName("getPurchaseOrderDetailsById - Verify @PreAuthorize Annotation")
  void getPurchaseOrderDetailsById_VerifyPreAuthorizeAnnotation_Success()
      throws NoSuchMethodException {
    // Arrange
    Method method =
        PurchaseOrderController.class.getMethod("getPurchaseOrderDetailsById", long.class);
    ...
    // Assert
    assertNotNull(
        annotation, "@PreAuthorize annotation should be present on getPurchaseOrderDetailsById");
    assertTrue(
        annotation.value().contains(Authorizations.VIEW_PURCHASE_ORDERS_PERMISSION),
