...
class UpdateMessageTest extends MessageServiceTestBase {
  ...
  /**
   * Purpose: Verify that scheduled email cancellation logic is triggered when validation passes.
   * Scenario: Update message with valid parameters where an old batch needs to be cancelled.
   * Expected Result: BadRequestException thrown if date change violates business rules (logic
   * check).
   */
  @Test
  @DisplayName(
      "Update Message - Cancel Email Logic - Only works if batch exists and not blocked by validation")
  void updateMessage_CancelEmailLogic_IfValidationPasses() {
    ...
    try (MockedConstruction<EmailHelper> emailHelperMock = stubEmailHelperCancelEmail()) {
      assertThrowsBadRequest(
          ErrorMessages.MessagesErrorMessages.CANNOT_MODIFY_SCHEDULED_PUBLISH_DATE,
          () -> messageService.updateMessage(validRequest));
    }
  }

  /**
   * Purpose: Verify that message notes can be successfully updated. Scenario: Call updateMessage
   * with a new notes string. Expected Result: Message is saved with the updated notes.
   */
  @Test
  ...
  /**
   * Purpose: Verify that the message title can be successfully updated. Scenario: Call
   * updateMessage with a new title. Expected Result: Message is saved with the updated title.
   */
  @Test
  ...
  /**
   * Purpose: Verify that old user and group mappings are deleted before saving the updated message.
   * Expected Result: Repository delete and flush methods called for both mapping types.
   */
  @Test
  ...
  /**
   * Purpose: Verify that a new email batch is generated when sendAsEmail is toggled from false to
   * true. Expected Result: EmailHelper generates new batch ID which is saved to the message.
   */
  @Test
  ...
  /**
   * Purpose: Verify that no batch ID is generated when sendAsEmail remains false. Expected Result:
   * Saved entity has null batch ID.
   */
  @Test
  ...
  /**
   * Purpose: Verify business rules for rescheduling an already scheduled email. Expected Result:
   * BadRequestException if rule violation occurs (placeholder test for logic).
   */
  @Test
  ...
  /**
   * Purpose: Verify that no new batch is generated if the publish date remains unchanged. Expected
   * Result: Message saved with original batch ID.
   */
  @Test
  ...
  /**
   * Purpose: Verify that an email is sent immediately if rescheduled to a null publish date.
  ...
  void updateMessage_SendImmediate_SendsEmail() {
    ...
    // Act & Assert
    try (MockedConstruction<EmailTemplates> templatesMock =
        mockConstruction(EmailTemplates.class)) {
      messageService.updateMessage(validRequest);
      assertFalse(templatesMock.constructed().isEmpty(), "EmailTemplates should be constructed");
      if (!templatesMock.constructed().isEmpty()) {
        verify(templatesMock.constructed().get(0))
            .sendMessageEmail(anyList(), anyString(), anyString(), any(), any());
      }
    }
  }

  /**
   * Purpose: Verify successful message update with standard valid parameters. Expected Result: No
   * exceptions thrown and repository save called.
   */
  @Test
  ...
  /**
   * Purpose: Verify that target user mappings are correctly refreshed during update. Scenario: Call
   * updateMessage with a non-null list of user IDs. Expected Result: Repository save called for new
   * mapping records.
   */
  @Test
  ...
  /**
   * Purpose: Verify message with maximum allowed title length (500 chars) is accepted during
   * update. Expected Result: Update succeeds without validation errors.
   */
  @Test
  ...
  /**
   * Purpose: Verify that successful update triggers activity logging. Expected Result:
   * UserLogService.logData called with identifying route info.
   */
  @Test
  ...
  /**
   * Purpose: Reject update when descriptionHtml is blank. Expected Result: BadRequestException with
   * ER007 error code.
   */
  @Test
  ...
  /**
   * Purpose: Reject adding a publish date to an email that has already been flagged as sent.
   * Expected Result: BadRequestException with CannotAddPublishDateAfterSent message.
   */
  @Test
  ...
  void updateMessage_CannotAddPublishDateAfterSent_ThrowsException() {
    ...
    // Act & Assert
    assertThrowsBadRequest(
        ErrorMessages.MessagesErrorMessages.CANNOT_ADD_PUBLISH_DATE_AFTER_SENT,
        () -> messageService.updateMessage(validRequest));
  }

  /**
   * Purpose: Reject disabling sendAsEmail flag once it has been enabled for a message. Expected
   * Result: BadRequestException with CannotDisableSendAsEmailOnce message.
   */
  @Test
  ...
  void updateMessage_CannotDisableSendAsEmail_ThrowsException() {
    ...
    // Act & Assert
    assertThrowsBadRequest(
        ErrorMessages.MessagesErrorMessages.CANNOT_DISABLE_SEND_AS_EMAIL_ONCE,
        () -> messageService.updateMessage(validRequest));
  }

  /**
   * Purpose: Reject modification of scheduled publish date if it doesn't match specific business
   * logic (legacy check). Expected Result: BadRequestException with ER011 error code.
   */
  @Test
  ...
  void updateMessage_CannotModifyScheduledPublishDate_LogicCheck() {
    ...
    // Act & Assert
    assertThrowsBadRequest(
        ErrorMessages.MessagesErrorMessages.ER011,
        () -> messageService.updateMessage(validRequest));
  }

  /**
   * Purpose: Reject update for non-existent client ID. Expected Result: NotFoundException with
   * client invalid ID message.
   */
  @Test
  ...
  /**
   * Purpose: Verify that disabling sendAsEmail triggers cancellation of any existing email batch.
   * Expected Result: BadRequestException thrown as Rule 1 (Cannot disable) takes precedence.
   */
  @Test
  ...
  /**
   * Purpose: Reject update when the targets list (users and groups) are both empty/null. Expected
   * Result: BadRequestException with ER008 error code.
   */
  @Test
  ...
  /**
   * Purpose: Reject update request when specified message ID does not exist. Expected Result:
  ...
  /**
   * Purpose: Reject update request when message ID is missing/null in the model. Expected Result:
   * BadRequestException with message invalid ID error code.
   */
  @Test
  ...
  /**
   * Purpose: Reject update when provided publishDate is in the past. Expected Result:
   * BadRequestException with ER009 error code.
   */
  @Test
  ...
  /**
   * Purpose: Reject update when publishDate is too far in future ( Shipgrid limit). Expected
   * Result: BadRequestException with ER010 error code.
   */
  @Test
  ...
  /**
   * Purpose: Reject update with publishDate if sendAsEmail is toggled to false. Expected Result:
  ...
  /**
   * Purpose: Propagate repository exceptions when database update fails. Expected Result:
  ...
  void updateMessage_RepositoryFailure_Propagates() {
    ...
    stubMessageRepositorySaveThrowsRuntimeException(
        ErrorMessages.MessagesErrorMessages.UPDATE_FAILED);
  ...
  /**
   * Purpose: Reject message with title exceeding maximum length during update. Expected Result:
   * BadRequestException with TitleTooLong error.
   */
  @Test
  ...
  /**
   * Purpose: Reject update request when message ID is zero. Expected Result: BadRequestException
  ...
  /**
   * Purpose: Verify that the updateMessage controller method is protected by correct @PreAuthorize
   * permission. Expected: Method has @PreAuthorize referencing UPDATE_MESSAGES_PERMISSION.
   */
  @Test
  ...
  void updateMessage_VerifyPreAuthorizeAnnotation_Success() throws NoSuchMethodException {
    ...
    assertTrue(
        annotation.value().contains(Authorizations.UPDATE_MESSAGES_PERMISSION),
        "@PreAuthorize should reference UPDATE_MESSAGES_PERMISSION");
  }

  /**
   * Purpose: Verify that the controller correctly delegates updateMessage calls to the service
   * layer. Expected Result: Status 200 returned upon delegation.
   */
  @Test
