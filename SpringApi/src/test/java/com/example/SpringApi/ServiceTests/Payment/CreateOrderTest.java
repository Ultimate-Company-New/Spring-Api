...
class CreateOrderTest extends PaymentServiceTestBase {
    ...
    /**
     * Purpose: Verify createOrder completes end-to-end with amount fallback, customer prefill, and saved payment metadata.
     * Expected Result: Successful response with Razorpay order details and prefill values.
     * Assertions: Response values, saved payment data, and Razorpay payload notes.
     */
    @Test
    ...
    void createOrder_s02_summaryFallbackAndPrefill_success() throws Exception {
        ...
        try {
            ...
            assertEquals(TEST_PO_ID.toString(),
    ...
    /**
     * Purpose: Verify createOrder allows payment only for PENDING_APPROVAL status.
     * Expected Result: BadRequestException with status validation message.
     * Assertions: Exception type and exact message.
     */
    @Test
    ...
    /**
     * Purpose: Verify createOrder falls back to order summary amount and fails when summary is missing.
     * Expected Result: BadRequestException with order summary not found message.
     * Assertions: Exception type and exact message.
     */
    @Test
    ...
    /**
     * Purpose: Verify createOrder validates client Razorpay API key before proceeding.
     * Expected Result: BadRequestException with API key not configured message.
     * Assertions: Exception type and exact message.
     */
    @Test
    ...
    void createOrder_f10_razorpayApiKeyMissing_failure() {
        ...
        // Assert
        assertEquals(ErrorMessages.PaymentErrorMessages.RAZORPAY_API_KEY_NOT_CONFIGURED, ex.getMessage());
    }

    /**
     * Purpose: Verify createOrder validates client Razorpay API secret before proceeding.
     * Expected Result: BadRequestException with API secret not configured message.
     * Assertions: Exception type and exact message.
     */
    @Test
    ...
    void createOrder_f11_razorpayApiSecretMissing_failure() {
        ...
        // Assert
        assertEquals(ErrorMessages.PaymentErrorMessages.RAZORPAY_API_SECRET_NOT_CONFIGURED, ex.getMessage());
    }
     ...
     * Expected Result: BadRequestException with formatted create-order failure message.
    ...
    void createOrder_f13_razorpayOrderCreateException_failure() {
        ...
        try {
            ...
            assertEquals(String.format(
                    ...
                    "gateway temporarily unavailable"), ex.getMessage());
        } finally {
            closeMockedRazorpayClientConstruction();
        }
    }
