...
class CancelReturnShipmentTest extends ShippingServiceTestBase {
  ...
  /**
   * Purpose: Verify return shipment save is called on success. Expected Result: Return shipment
   * saved. Assertions: verify save called.
   */
  @Test
  ...
  /**
   * Purpose: Verify cancel return succeeds with valid data. Expected Result: No exception thrown.
  ...
  /**
   * Purpose: Verify already cancelled return shipment throws BadRequestException. Expected Result:
   * BadRequestException with AlreadyCancelled message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify cancel API error throws BadRequestException. Expected Result:
   * BadRequestException with FailedToCancelReturn message. Assertions: Exception type and message.
   */
  @Test
  ...
  void cancelReturnShipment_CancelApiError_ThrowsBadRequestException() {
    ...
    assertEquals(
        String.format(
            ErrorMessages.ReturnShipmentErrorMessages.FAILED_TO_CANCEL_RETURN,
            ErrorMessages.OPERATION_FAILED),
        ex.getMessage());
  }

  /**
   * Purpose: Verify missing ShipRocket return order id throws BadRequestException. Expected Result:
   * BadRequestException with NoShipRocketOrderId message. Assertions: Exception type and message.
   */
  @Test
  ...
  void cancelReturnShipment_MissingOrderId_ThrowsBadRequestException() {
    ...
    // Assert
    assertEquals(
        ErrorMessages.ReturnShipmentErrorMessages.NO_SHIP_ROCKET_ORDER_ID, ex.getMessage());
  }

  /**
   * Purpose: Verify missing ShipRocket credentials throws BadRequestException. Expected Result:
   * BadRequestException with ShipRocketCredentialsNotConfigured. Assertions: Exception type and
   * message.
   */
  @Test
  ...
  /**
   * Purpose: Verify non-numeric return order id throws BadRequestException. Expected Result:
   * BadRequestException with InvalidId format message. Assertions: Exception type and message.
   */
  @Test
  ...
  void cancelReturnShipment_InvalidReturnOrderIdFormat_ThrowsBadRequestException() {
    ...
    // Assert
    assertEquals(
        ErrorMessages.ReturnShipmentErrorMessages.INVALID_ID + " Format error: RET-XYZ",
        ex.getMessage());
  }

  /**
   * Purpose: Verify return shipment not found throws NotFoundException. Expected Result:
  ...
  void cancelReturnShipment_NotFound_ThrowsNotFoundException() {
    ...
    // Assert
    assertEquals(
        String.format(ErrorMessages.ReturnShipmentErrorMessages.NOT_FOUND, TEST_RETURN_SHIPMENT_ID),
        ex.getMessage());
  }
  ...
  /**
   * Purpose: Verify controller has @PreAuthorize for cancelReturn. Expected Result: Annotation
