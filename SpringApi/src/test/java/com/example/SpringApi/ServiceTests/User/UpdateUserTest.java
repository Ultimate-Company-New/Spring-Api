...
class UpdateUserTest extends UserServiceTestBase {
    ...
    /**
     * Purpose: Verify updateUser creates a new address when user has addressId but the address record is missing.
     * Expected Result: New address saved and user updated with new addressId.
     * Assertions: Address save and user save invocations.
     */
    @Test
    ...
    /**
     * Purpose: Verify ImgBB profile-picture update fails when API key is missing.
     * Expected Result: BadRequestException with ImgBB configuration message.
    ...
    /**
     * Purpose: Verify ImgBB profile-picture update fails when upload returns null response.
    ...
    /**
     * Purpose: Verify Firebase profile-picture update fails when upload API returns false.
     * Expected Result: BadRequestException with ER010 message.
     * Assertions: Exact exception message.
     */
    @Test
    ...
    /**
     * Purpose: Verify invalid imageLocation setting is rejected during profile update.
    ...
    void updateUser_invalidImageLocation_throwsBadRequestException() {
        // Arrange
        setupUpdateUserMocks();
        org.springframework.test.util.ReflectionTestUtils.setField(userService, "imageLocation", "unsupported-store");
