...
class CalculateShippingTest extends ShippingServiceTestBase {
  ...
  /**
   * Purpose: Verify controller has @PreAuthorize for calculateShipping. Expected Result: Annotation
   * exists and includes VIEW_PURCHASE_ORDERS_PERMISSION. Assertions: Annotation is present and
   * contains permission.
   */
  @Test
  ...
  void calculateShipping_VerifyPreAuthorizeAnnotation_Success() throws NoSuchMethodException {
    // Arrange
    Method method =
        ShippingController.class.getMethod(
            "calculateShipping", ShippingCalculationRequestModel.class);
