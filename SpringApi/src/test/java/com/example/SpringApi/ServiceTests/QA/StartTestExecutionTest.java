...
class StartTestExecutionTest extends QAServiceTestBase {
     ...
     * Expected Result: Command contains surefire flags and -Dtest selector.
     * Assertions: Command tokens and selector format.
     */
    @Test
    void startTestExecution_privateBuildMavenCommand_withClassAndMethodFilter_success() {
        ...
        // Act
        Object commandObj = invokePrivateMethod(
                realService,
                "buildMavenCommand",
     ...
     * Expected Result: Base maven command returned.
     * Assertions: Command tokens and absence of -Dtest.
     */
    @Test
    ...
    /**
     * Purpose: Verify parser updates execution counters from surefire summary line.
     * Expected Result: Completed/passed/failed/skipped counters are updated correctly.
     * Assertions: Counter values and total-tests adjustment.
     */
    @Test
    void startTestExecution_privateParseTestOutput_updatesCounters_success() {
        // Arrange
        QAService realService = createRealQAService();
        TestExecutionStatusModel status = new TestExecutionStatusModel("exec-parse", "QAService", null, 1);
     ...
     * Assertions: Status value equals RUNNING.
     */
    @Test
    void startTestExecution_privateParseTestOutput_runningLine_setsRunningStatus_success() {
        // Arrange
        QAService realService = createRealQAService();
        TestExecutionStatusModel status = new TestExecutionStatusModel("exec-running", "QAService", null, 2);
    ...
    /**
     * Purpose: Verify project directory resolver prioritizes current directory when pom and test sources exist.
     * Expected Result: Current temporary directory path is returned.
     * Assertions: Returned path equals temporary project path.
     */
    @Test
    void startTestExecution_privateFindProjectDirectory_currentDirPreferred_success() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
    ...
    /**
     * Purpose: Verify executeTestsAsync safely returns when execution status is missing.
     * Expected Result: No exception and no process start.
     * Assertions: Method returns null (void invocation) without failure.
     */
    @Test
    ...
    /**
     * Purpose: Verify markExecutionFailed updates terminal fields and clears running process tracking.
     * Expected Result: Status is FAILED with error metadata and running process entry removed.
     * Assertions: Status fields and map cleanup.
     */
    @Test
    void startTestExecution_privateMarkExecutionFailed_updatesStatusAndClearsProcess_success() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
        clearExecutionTrackingState();
        TestExecutionStatusModel status = new TestExecutionStatusModel("exec-fail", "QAService", null, 3);
        ...
        if (runningProcessesObj != null) {
            java.lang.reflect.Method putMethod = runningProcessesObj.getClass()
     ...
     * Expected Result: No exception and status remains unchanged.
     * Assertions: Results list remains empty.
     */
    @Test
    void startTestExecution_privateParseSurefireReports_missingDirectory_noopSuccess() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
        TestExecutionStatusModel status = new TestExecutionStatusModel("exec-no-reports", "QAService", null, 0);
     ...
     * Expected Result: Command contains -Dtest with class-only selector.
     * Assertions: Selector exists and does not include method suffix.
     */
    @Test
    ...
    /**
     * Purpose: Verify project directory resolution falls back to ./SpringApi when current dir is a parent workspace.
     * Expected Result: SpringApi child directory is selected.
     * Assertions: Returned path equals currentDir/SpringApi.
     */
    @Test
    void startTestExecution_privateFindProjectDirectory_springApiSubdirSelected_success() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
    ...
    /**
     * Purpose: Verify project directory resolution checks parent/SpringApi when running from nested module path.
     * Expected Result: Parent SpringApi directory is selected.
     * Assertions: Returned path equals parent workspace SpringApi directory.
     */
    @Test
    void startTestExecution_privateFindProjectDirectory_parentSpringApiSelected_success() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
    ...
    /**
     * Purpose: Verify project directory resolution returns current directory when no pom/test roots are discoverable.
     * Expected Result: Current user.dir path is returned.
     * Assertions: Returned path equals current working directory.
     */
    @Test
    void startTestExecution_privateFindProjectDirectory_noMatches_returnsCurrentDirectory_success() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
    ...
    /**
     * Purpose: Verify test-file parser handles nested classes, display names, and multi-line parameterized @ValueSource.
     * Expected Result: Expanded parameterized test names and declaring nested class names are captured.
     * Assertions: Parsed method names, display names, and declaring class names.
     */
    @Test
    void startTestExecution_privateReadTestMethodsFromFile_parsesNestedAndParameterizedTests_success() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
        ...
        try {
            // Act
            Object parsedObj = invokePrivateMethod(
                    realService,
    ...
    /**
     * Purpose: Verify parser handles IOException while reading a discovered test path.
     * Expected Result: Empty parsed list is returned.
     * Assertions: Returned list exists and is empty.
     */
    @Test
    void startTestExecution_privateReadTestMethodsFromFile_ioFailure_returnsEmptyList_success() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
    ...
    /**
     * Purpose: Verify @ValueSource count parser supports null, braced multi-value, empty braced, and single value forms.
     * Expected Result: Correct counts are returned for each form.
     * Assertions: Exact count values per input variant.
     */
    @Test
    void startTestExecution_privateParseValueSourceCount_handlesSupportedVariants_success() {
        ...
        // Act
        int nullCount = (Integer) invokePrivateMethod(
                realService,
                "parseValueSourceCount",
                new Class<?>[] { String.class },
                (Object) null);
        int multiCount = (Integer) invokePrivateMethod(
                realService,
                "parseValueSourceCount",
                new Class<?>[] { String.class },
                "@ValueSource(strings = {\"A\", \"B\", \"C\"})");
        int emptyCount = (Integer) invokePrivateMethod(
                realService,
                "parseValueSourceCount",
                new Class<?>[] { String.class },
                "@ValueSource(strings = {})");
        int singleCount = (Integer) invokePrivateMethod(
                realService,
                "parseValueSourceCount",
                new Class<?>[] { String.class },
    ...
    /**
     * Purpose: Verify parameter-list type extraction handles null/empty input, annotations, final modifier, and generics.
     * Expected Result: Surefire-friendly simple type names are returned.
     * Assertions: Exact extracted type list text.
     */
    @Test
    void startTestExecution_privateExtractParameterTypesForSurefireName_handlesAnnotationsAndGenerics_success() {
        // Arrange
        QAService realService = createRealQAService();

        // Act
        String nullTypes = (String) invokePrivateMethod(
                realService,
                "extractParameterTypesForSurefireName",
                new Class<?>[] { String.class },
                (Object) null);
        String emptyTypes = (String) invokePrivateMethod(
                realService,
                "extractParameterTypesForSurefireName",
                new Class<?>[] { String.class },
                "   ");
        String extractedTypes = (String) invokePrivateMethod(
                realService,
                "extractParameterTypesForSurefireName",
                new Class<?>[] { String.class },
    ...
    /**
     * Purpose: Verify nested declaring-class builder returns outer$inner selector and falls back when outer class is absent.
     * Expected Result: Correct nested selector for matching outer class and fallback to outer name otherwise.
     * Assertions: Exact selector values.
     */
    @Test
    void startTestExecution_privateBuildDeclaringTestClassName_nestedAndFallback_success() {
        ...
        classStack.add(createQaPrivateInnerInstance(
                "ClassScope",
        ...
        // Act
        String nestedSelector = (String) invokePrivateMethod(
                realService,
                "buildDeclaringTestClassName",
                new Class<?>[] { String.class, ArrayDeque.class },
                "ParserFixtureTest",
                classStack);
        String fallbackSelector = (String) invokePrivateMethod(
                realService,
                "buildDeclaringTestClassName",
                new Class<?>[] { String.class, ArrayDeque.class },
                "OtherOuterTest",
    ...
    /**
     * Purpose: Verify associated-test matcher supports underscore and camelCase naming and enriches tests with latest run status.
     * Expected Result: Both naming styles are matched and latest result metadata is applied.
     * Assertions: Matched test count and populated status for mapped latest result.
     */
    @Test
    void startTestExecution_privateFindAssociatedTests_matchesUnderscoreAndCamelCase_success() {
        ...
        allTestMethods.add(createQaPrivateInnerInstance(
                "TestMethodInfo",
        ...
        // Act
        Object associatedObj = invokePrivateMethod(
                realService,
                "findAssociatedTests",
        ...
        String firstStatus = (String) invokePrivateMethod(
                first,
                "getLastRunStatus",
                new Class<?>[] {});
    ...
    /**
     * Purpose: Verify declaring class resolution supports exact parameterized method names and base names.
     * Expected Result: Nested declaring class selector is resolved for both exact and stripped names.
     * Assertions: Exact nested class selector and null handling.
     */
    @Test
    void startTestExecution_privateResolveDeclaringClassAndStripParameterizedSuffix_success() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
        Path tempProject = createParserFixtureProject();
        String originalUserDir = System.getProperty("user.dir");
        ...
        try {
            // Act
            String exactSelector = (String) invokePrivateMethod(
                    realService,
                    "resolveDeclaringTestClassForTestMethod",
                    new Class<?>[] { String.class, String.class },
                    "ParserFixtureTest",
                    "parseValues(String)[1]");
            String strippedSelector = (String) invokePrivateMethod(
                    realService,
                    "resolveDeclaringTestClassForTestMethod",
                    new Class<?>[] { String.class, String.class },
                    "ParserFixtureTest",
                    "parseValues");
            String invalidSelector = (String) invokePrivateMethod(
                    realService,
                    "resolveDeclaringTestClassForTestMethod",
                    new Class<?>[] { String.class, String.class },
                    "",
                    "anything");
            String strippedName = (String) invokePrivateMethod(
                    realService,
                    "stripParameterizedSuffix",
                    new Class<?>[] { String.class },
                    "parseValues(String)[2]");
            String strippedNull = (String) invokePrivateMethod(
                    realService,
                    "stripParameterizedSuffix",
                    new Class<?>[] { String.class },
                    (Object) null);
    ...
    /**
     * Purpose: Verify expected test count calculation supports service-level/method-level and dashboard failure fallback.
     * Expected Result: Service/method counts are non-negative and dashboard failures return 0.
     * Assertions: Numeric ranges and fallback value.
     */
    @Test
    void startTestExecution_privateCalculateExpectedTestCount_serviceMethodAndFailureFallback_success() {
        // Arrange
        QAService realService = createRealQAService();

        // Act
        int serviceCount = (Integer) invokePrivateMethod(
                realService,
                ...
                null,
                null);
        int methodCount = (Integer) invokePrivateMethod(
                realService,
                "calculateExpectedTestCount",
                new Class<?>[] { String.class, String.class, List.class },
                "QAService",
        ...
        int failedFallback = (Integer) invokePrivateMethod(
                qaService,
                "calculateExpectedTestCount",
                new Class<?>[] { String.class, String.class, List.class },
                "QAService",
    ...
    /**
     * Purpose: Verify surefire XML parsing captures passed/failed/skipped results and de-duplicates repeated testcases.
     * Expected Result: Parsed result list contains one entry per unique testcase with correct statuses.
     * Assertions: Result count, statuses, and parsed failure details.
     */
    @Test
    void startTestExecution_privateParseSurefireReports_parsesAndDeduplicatesResults_success() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
        TestExecutionStatusModel status = new TestExecutionStatusModel("exec-xml-parse", "QAService", null, 0);
    ...
    /**
     * Purpose: Verify surefire report listing failure is wrapped as application exception.
     * Expected Result: Runtime wrapper contains ApplicationException root cause.
     * Assertions: Root cause type and error constant message.
     */
    @Test
    void startTestExecution_privateParseSurefireReports_notDirectory_throwsApplicationException() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
        TestExecutionStatusModel status = new TestExecutionStatusModel("exec-surefire-fail", "QAService", null, 0);
        ...
        // Act
        RuntimeException exception = assertThrows(RuntimeException.class, () -> invokePrivateMethod(
                realService,
                "parseSurefireReports",
                new Class<?>[] { TestExecutionStatusModel.class, Path.class, String.class },
                status,
        ...
        assertEquals(ErrorMessages.TestExecutorErrorMessages.FAILED_TO_LIST_SUREFIRE_REPORTS, appException.getMessage());
    }

    /**
     * Purpose: Verify XML report read failures are wrapped as application exceptions with formatted file name.
     * Expected Result: Runtime wrapper contains ApplicationException root cause.
    ...
    void startTestExecution_privateParseXmlReport_missingFile_throwsApplicationException() {
        // Arrange
        QAService realService = createRealQAService();
        TestExecutionStatusModel status = new TestExecutionStatusModel("exec-xml-missing", "QAService", null, 0);
        Path missingFile = Path.of("target/surefire-reports/TEST-DefinitelyMissing-" + System.nanoTime() + ".xml");
        ...
        assertNotNull(appException);
        assertEquals(
                String.format(ErrorMessages.TestExecutorErrorMessages.FAILED_TO_PARSE_SUREFIRE_REPORT_FORMAT,
                        missingFile.getFileName()),
                appException.getMessage());
    }

    /**
     * Purpose: Verify executeTestsAsync can complete a failing Maven run and persist terminal status metadata.
     * Expected Result: Execution finishes with COMPLETED_WITH_FAILURES and completion metadata is set.
     * Assertions: Terminal status, duration, and completion timestamp.
     */
    @Test
    void startTestExecution_privateExecuteTestsAsync_nonExistentTestClass_marksCompletedWithFailures() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
        clearExecutionTrackingState();
        String executionId = "exec-nonexistent-test-class";
        TestExecutionStatusModel status = new TestExecutionStatusModel(executionId, "QAService", null, 1);
    ...
    /**
     * Purpose: Verify executeTestsAsync handles process start I/O failure and marks execution as failed.
     * Expected Result: ApplicationException is thrown and status is updated to FAILED.
     * Assertions: Root cause type/message and FAILED status metadata.
     */
    @Test
    void startTestExecution_privateExecuteTestsAsync_ioFailure_marksFailedAndThrowsApplicationException() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
        clearExecutionTrackingState();
        ...
        Path missingDir = Path.of(System.getProperty("java.io.tmpdir"),
     ...
     */
    @Test
    void startTestExecution_privateExecuteTestsAsync_parseReportFailure_marksFailedWithExecutionFailed() throws Exception {
        // Arrange
        QAService realService = createRealQAService();
        clearExecutionTrackingState();
        ...
        try {
            ...
            ApplicationException appException = findCause(exception, ApplicationException.class);
            assertNotNull(appException);
            assertEquals(ErrorMessages.TestExecutorErrorMessages.EXECUTION_FAILED, appException.getMessage());
            assertEquals("FAILED", status.getStatus());
    ...
    private void putExecutionStatus(String executionId, TestExecutionStatusModel status) throws Exception {
        Object activeExecutionsObj = getPrivateStaticFieldValue(QAService.class, "activeExecutions");
        java.lang.reflect.Method putMethod = activeExecutionsObj.getClass().getMethod("put", Object.class, Object.class);
