...
class CreateReturnTest extends ShippingServiceTestBase {
   ...
   */
  /**
   * Purpose: Verify AWB assignment failure is ignored. Expected Result: ReturnShipmentResponseModel
  ...
  /**
   * Purpose: Verify delivered date null still allows return. Expected Result:
   * ReturnShipmentResponseModel returned. Assertions: Response is not null.
   */
  @Test
  ...
  /**
   * Purpose: Verify full return succeeds. Expected Result: ReturnShipmentResponseModel returned.
  ...
  /**
   * Purpose: Verify multiple valid items return succeeds. Expected Result:
   * ReturnShipmentResponseModel returned. Assertions: Response is not null.
   */
  @Test
  ...
  void createReturn_MultipleItems_Success() {
    ...
    CreateReturnRequestModel.ReturnProductItem item2 =
  ...
  /**
   * Purpose: Verify partial return succeeds. Expected Result: ReturnShipmentResponseModel returned.
   * Assertions: Response is not null.
   */
  @Test
  ...
  /**
   * Purpose: Verify warehouse nickname fallback is used when name on address is null. Expected
   * Result: ReturnShipmentResponseModel returned. Assertions: Response is not null.
   */
  @Test
  @DisplayName("createReturn - Warehouse Name Null Uses Nickname - Success")
  void createReturn_WarehouseNameNullUsesNickname_Success() {
    // Arrange
    testShipment.setShipRocketStatus("DELIVERED");
    testShipment.setDeliveredDate(LocalDateTime.now().minusDays(1));
    testShipment.setShipmentProducts(List.of(testShipmentProduct));
    testPickupAddress.setNameOnAddress(null);
    testPickupLocation.setAddressNickName("WH-NICK");
    stubShipmentRepositoryFindByShipmentIdAndClientId(testShipment);
    stubClientServiceGetClientById(testClientResponse);
    stubProductRepositoryFindById(testProduct);
    stubReturnShipmentRepositorySave(testReturnShipment);
  ...
  /**
   * Purpose: Verify null address/contact fields use fallback defaults in ShipRocket return request.
   * Expected Result: Return creation succeeds and captured request contains fallback values.
   * Assertions: Captured request fields match expected defaults.
   */
  @Test
  @DisplayName("createReturn - Request Fallback Fields - Success")
  void createReturn_RequestFallbackFields_Success() {
    // Arrange
    testShipment.setShipRocketStatus("DELIVERED");
    testShipment.setDeliveredDate(LocalDateTime.now().minusDays(1));
    testShipment.setShipmentProducts(List.of(testShipmentProduct));
    testDeliveryAddress.setNameOnAddress(null);
    testDeliveryAddress.setStreetAddress2(null);
    testDeliveryAddress.setCountry(null);
    testDeliveryAddress.setEmailOnAddress(null);
    testDeliveryAddress.setPhoneOnAddress(null);
    testPickupAddress.setNameOnAddress(null);
    testPickupAddress.setStreetAddress2(null);
    testPickupAddress.setCountry(null);
    testPickupAddress.setEmailOnAddress(null);
    testPickupAddress.setPhoneOnAddress(null);
    testPickupLocation.setAddressNickName("WH-FALLBACK");
    createReturnRequest.setLength(null);
    createReturnRequest.setBreadth(null);
    createReturnRequest.setHeight(null);
    createReturnRequest.setWeight(null);
    ...
    // Assert
    assertNotNull(result);
    ArgumentCaptor<ShipRocketReturnOrderRequestModel> captor =
        ArgumentCaptor.forClass(ShipRocketReturnOrderRequestModel.class);
    org.mockito.Mockito.verify(shipRocketHelper).createReturnOrderAsJson(captor.capture());
    ShipRocketReturnOrderRequestModel captured = captor.getValue();
    assertEquals("Customer", captured.getPickupCustomerName());
    assertEquals("India", captured.getPickupCountry());
    assertEquals("", captured.getPickupEmail());
    assertEquals("", captured.getPickupPhone());
    assertEquals("WH-FALLBACK", captured.getShippingCustomerName());
    assertEquals("India", captured.getShippingCountry());
    assertEquals("", captured.getShippingEmail());
    assertEquals("", captured.getShippingPhone());
    assertEquals(java.math.BigDecimal.valueOf(11), captured.getLength());
    assertEquals(java.math.BigDecimal.valueOf(11), captured.getBreadth());
    assertEquals(java.math.BigDecimal.valueOf(11), captured.getHeight());
    assertEquals(java.math.BigDecimal.valueOf(0.5), captured.getWeight());
  }
   ...
   */
  /**
   * Purpose: Verify missing ShipRocket credentials throws BadRequestException. Expected Result:
  ...
  /**
   * Purpose: Verify non-delivered shipment throws BadRequestException. Expected Result:
   * BadRequestException with OnlyDeliveredCanReturn message. Assertions: Exception type and
   * message.
   */
  @Test
  ...
  void createReturn_NotDelivered_ThrowsBadRequestException() {
    ...
    // Assert
    assertEquals(
        String.format(ErrorMessages.ReturnShipmentErrorMessages.ONLY_DELIVERED_CAN_RETURN, "NEW"),
        ex.getMessage());
  }

  /**
   * Purpose: Verify return window exceeded throws BadRequestException. Expected Result:
   * BadRequestException with ProductPastReturnWindow message. Assertions: Exception type and
   * message.
   */
  @Test
  ...
  void createReturn_PastReturnWindow_ThrowsBadRequestException() {
    ...
    assertEquals(
        String.format(
            ...
            testProduct.getReturnWindowDays()),
        ex.getMessage());
  }

  /**
   * Purpose: Verify null productId throws BadRequestException. Expected Result: BadRequestException
   * with ProductIdRequired message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify product not found throws NotFoundException. Expected Result: NotFoundException
   * with ProductErrorMessages.ER013 message. Assertions: Exception type and message.
   */
  @Test
  ...
  void createReturn_ProductNotFound_ThrowsNotFoundException() {
    ...
    // Assert
    assertEquals(
        String.format(ErrorMessages.ProductErrorMessages.ER013, TEST_PRODUCT_ID), ex.getMessage());
  }

  /**
   * Purpose: Verify product not in shipment throws BadRequestException. Expected Result:
   * BadRequestException with ProductNotInShipment message. Assertions: Exception type and message.
   */
  @Test
  ...
  void createReturn_ProductNotInShipment_ThrowsBadRequestException() {
    ...
    // Assert
    assertEquals(
        String.format(ErrorMessages.ReturnShipmentErrorMessages.PRODUCT_NOT_IN_SHIPMENT, 999L),
        ex.getMessage());
  }

  /**
   * Purpose: Verify product not returnable throws BadRequestException. Expected Result:
   * BadRequestException with ProductNotReturnable message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify empty products list throws BadRequestException. Expected Result:
   * BadRequestException with AtLeastOneProductRequired message. Assertions: Exception type and
   * message.
   */
  @Test
  ...
  void createReturn_ProductsEmpty_ThrowsBadRequestException() {
    ...
    // Assert
    assertEquals(
        ErrorMessages.ReturnShipmentErrorMessages.AT_LEAST_ONE_PRODUCT_REQUIRED, ex.getMessage());
  }

  /**
   * Purpose: Verify null products list throws BadRequestException. Expected Result:
   * BadRequestException with AtLeastOneProductRequired message. Assertions: Exception type and
   * message.
   */
  @Test
  ...
  /**
   * Purpose: Verify return quantity exceeds throws BadRequestException. Expected Result:
   * BadRequestException with ReturnQuantityExceeds message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify negative quantity throws BadRequestException. Expected Result:
   * BadRequestException with ValidQuantityRequired message. Assertions: Exception type and message.
   */
  @Test
  ...
  void createReturn_QuantityNegative_ThrowsBadRequestException() {
    ...
    // Assert
    assertEquals(
        ErrorMessages.ReturnShipmentErrorMessages.VALID_QUANTITY_REQUIRED, ex.getMessage());
  }

  /**
   * Purpose: Verify null quantity throws BadRequestException. Expected Result: BadRequestException
   * with ValidQuantityRequired message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify quantity zero throws BadRequestException. Expected Result: BadRequestException
   * with ValidQuantityRequired message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify empty reason throws BadRequestException. Expected Result: BadRequestException
   * with ReturnReasonRequired message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify null reason throws BadRequestException. Expected Result: BadRequestException
   * with ReturnReasonRequired message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify null return window days throws BadRequestException. Expected Result:
   * BadRequestException with ProductNotReturnable message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify second item with null productId throws BadRequestException. Expected Result:
   * BadRequestException with ProductIdRequired message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify second item with null quantity throws BadRequestException. Expected Result:
   * BadRequestException with ValidQuantityRequired message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify second item with null reason throws BadRequestException. Expected Result:
   * BadRequestException with ReturnReasonRequired message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify null shipmentId throws BadRequestException. Expected Result:
   * BadRequestException with ShipmentIdRequired message. Assertions: Exception type and message.
   */
  @Test
  ...
  /**
   * Purpose: Verify shipment not found throws NotFoundException. Expected Result: NotFoundException
  ...
  /**
   * Purpose: Verify shipment products null triggers ProductNotInShipment. Expected Result:
   * BadRequestException with ProductNotInShipment message. Assertions: Exception type and message.
   */
  @Test
  ...
  void createReturn_ShipmentProductsNull_ThrowsBadRequestException() {
    ...
    assertEquals(
        String.format(
            ErrorMessages.ReturnShipmentErrorMessages.PRODUCT_NOT_IN_SHIPMENT, TEST_PRODUCT_ID),
        ex.getMessage());
  }

  /**
   * Purpose: Verify ShipRocket create return failure throws BadRequestException. Expected Result:
   * BadRequestException with FailedToCreateReturn message. Assertions: Exception type and message.
   */
  @Test
  ...
  void createReturn_ShipRocketFailure_ThrowsBadRequestException() {
    ...
    stubClientServiceGetClientById(testClientResponse);
    stubProductRepositoryFindById(testProduct);
    stubShipRocketHelperCreateReturnOrderAsJsonThrows(
    ...
    assertEquals(
        String.format(
            ErrorMessages.ReturnShipmentErrorMessages.FAILED_TO_CREATE_RETURN,
  ...
  /**
   * Purpose: Verify controller has @PreAuthorize for createReturn. Expected Result: Annotation
   * exists and includes MODIFY_SHIPMENTS_PERMISSION. Assertions: Annotation is present and contains
   * permission.
   */
  @Test
  ...
  void createReturn_VerifyPreAuthorizeAnnotation_Success() throws NoSuchMethodException {
    // Arrange
    Method method =
        ShippingController.class.getMethod("createReturn", CreateReturnRequestModel.class);
