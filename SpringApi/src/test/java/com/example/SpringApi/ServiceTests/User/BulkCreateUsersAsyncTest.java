...
class BulkCreateUsersAsyncTest extends UserServiceTestBase {
    ...
    /**
     * Purpose: Verify outer critical-error handler branch executes when post-processing logging fails.
     * Expected Result: Method catches the exception internally and completes without propagating.
     * Assertions: assertDoesNotThrow and logDataWithContext invocation.
     */
    @Test
    ...
    void bulkCreateUsersAsync_criticalErrorBranchHandled_success() {
        ...
        stubPasswordHelper();
        stubStandardCreateUserMocks();
        when(userLogService.logDataWithContext(anyLong(), anyString(), anyLong(), anyString(), anyString()))
