...
class CreateMessageTest extends MessageServiceTestBase {
  ...
  /**
   * Purpose: Verify successful message creation when notes are blank. Scenario: CreateMessage with
   * a title, description, targets and blank notes. Expected: Message created successfully.
   */
  @Test
  ...
  /**
   * Purpose: Verify successful message creation with group targets. Scenario: CreateMessage with
   * group target IDs provided. Expected: Message saved and group mappings created.
   */
  @Test
  ...
  /**
   * Purpose: Verify successful message creation with mixed (user and group) targets. Scenario:
   * CreateMessage with both user and group target IDs. Expected: Message saved and both types of
   * mappings created.
   */
  @Test
  ...
  /**
   * Purpose: Verify successful message creation when notes are null. Scenario: CreateMessage with a
   * title, description, targets and null notes. Expected: Message created successfully.
   */
  @Test
  ...
  /**
   * Purpose: Verify that no recipient lookup occurs when sendAsEmail is false. Scenario:
   * CreateMessage with sendAsEmail set to false. Expected: Recipient lookup repository method never
   * called.
   */
  @Test
  ...
  void createMessage_SendAsEmailFalse_NoRecipientLookup() {
    ...
    verify(userRepository, never())
        .findAllUserEmailsByClientAndUserIdsAndGroupIds(anyLong(), anyList(), anyList());
  }

  /**
   * Purpose: Verify that no email is sent when sendAsEmail is true but no recipient emails are
   * found. Scenario: CreateMessage with sendAsEmail=true but repository returns empty email list.
  ...
  /**
   * Purpose: Verify that no batch ID is generated when an immediate email (no publishDate) is
   * created. Scenario: CreateMessage with sendAsEmail=true but null publishDate. Expected: Message
   * saved without generating a batch ID.
   */
  @Test
  ...
  /**
   * Purpose: Verify that a batch ID is generated when a scheduled email is created. Scenario:
   * CreateMessage with sendAsEmail=true and a future publishDate. Expected: EmailHelper used to
   * generate batch ID which is saved to the message.
   */
  @Test
  ...
  /**
   * Purpose: Verify that an email is sent to recipients when sendAsEmail is true and recipients are
   * found. Scenario: CreateMessage with sendAsEmail=true and repository returns a list of recipient
   * emails. Expected: EmailTemplates.sendMessageEmail is called with correct parameters.
   */
  @Test
  ...
  void createMessage_SendAsEmailWithRecipients_SendsEmail() {
    ...
    // Act & Assert
    try (MockedConstruction<EmailHelper> emailHelperMock =
            stubEmailHelperGenerateBatchId("batch-email");
      ...
      verify(constructed)
          .sendMessageEmail(
              eq(List.of(TEST_EMAIL)), eq(TEST_TITLE), eq(TEST_DESC_HTML), any(), any());
    }
  }

  /**
   * Purpose: Verify successful message creation when email sending is not requested. Scenario:
   * CreateMessage with sendAsEmail=false. Expected: Message saved and user activity logged, but no
   * email operations performed.
   */
  @Test
  ...
  /**
   * Purpose: Verify message with maximum allowed title length (500 chars) is accepted. Scenario:
   * CreateMessage with 500-character title. Expected: Message created successfully without
   * validation errors.
   */
  @Test
  ...
  /**
   * Purpose: Verify that title and description fields are trimmed during message creation.
   * Scenario: CreateMessage with leading and trailing whitespace in fields. Expected: Saved entity
   * contains trimmed strings.
   */
  @Test
  ...
  /**
   * Purpose: Verify successful message creation with individual user targets. Scenario:
   * CreateMessage with a list of user IDs. Expected: Message saved and user mappings created for
   * each user ID.
   */
  @Test
  ...
  /**
   * Purpose: Verify that successful message creation triggers activity logging. Scenario:
   * CreateMessage with valid request. Expected: UserLogService.logDataWithContext called with
   * correct details.
   */
  @Test
  ...
  /**
   * Purpose: Reject message creation when descriptionHtml is blank. Expected Result:
   * BadRequestException with ER007 error code.
   */
  @Test
  ...
  void createMessage_BlankDescriptionHtml_ThrowsBadRequestException() {
    ...
    assertThrowsBadRequest(
        ErrorMessages.MessagesErrorMessages.ER007,
        () -> messageService.createMessage(validRequest));
  }

  /**
   * Purpose: Reject message creation when title is blank. Expected Result: BadRequestException with
   * ER003 error code.
   */
  @Test
  ...
  void createMessage_BlankTitle_ThrowsBadRequestException() {
    ...
    // Act & Assert
    assertThrowsBadRequest(
        ErrorMessages.MessagesErrorMessages.ER003,
        () -> messageService.createMessage(validRequest));
  }

  /**
   * Purpose: Reject message creation for non-existent client ID. Expected Result: NotFoundException
   * with client invalid ID error message.
   */
  @Test
  ...
  /**
   * Purpose: Reject scheduled email when publishDate is too far in future ( Shipgrid limit).
   * Scenario: CreateMessage with publishDate 74 hours away. Expected Result: BadRequestException
   * with ER010 error code.
   */
  @Test
  ...
  void createMessage_EmailBeyond72Hours_ThrowsBadRequestException() {
    ...
    // Act & Assert
    assertThrowsBadRequest(
        ErrorMessages.MessagesErrorMessages.ER010,
        () -> messageService.createMessage(validRequest));
  }

  /**
   * Purpose: Reject scheduled email when publishDate is in the past. Scenario: CreateMessage with
   * publishDate 1 hour past. Expected Result: BadRequestException with ER009 error code.
   */
  @Test
  ...
  void createMessage_EmailInPast_ThrowsBadRequestException() {
    ...
    // Act & Assert
    assertThrowsBadRequest(
        ErrorMessages.MessagesErrorMessages.ER009,
        () -> messageService.createMessage(validRequest));
  }

  /**
   * Purpose: Reject message creation when targets list is empty. Expected Result:
   * BadRequestException with ER008 error code.
   */
  @Test
  ...
  void createMessage_EmptyTargets_ThrowsBadRequestException() {
    ...
    BadRequestException exception =
        assertThrows(BadRequestException.class, () -> messageService.createMessage(validRequest));
    assertEquals(ErrorMessages.MessagesErrorMessages.ER008, exception.getMessage());
  }

  /**
   * Purpose: Reject message creation when targets list is null. Expected Result:
   * BadRequestException with ER008 error code.
   */
  @Test
  ...
  /**
   * Purpose: Reject message creation when descriptionHtml is null. Expected Result:
   * BadRequestException with ER007 error code.
   */
  @Test
  ...
  /**
   * Purpose: Reject message creation when request model is null. Expected Result:
   * BadRequestException with InvalidId error code (legacy exception code).
   */
  @Test
  ...
  void createMessage_NullRequest_ThrowsBadRequestException() {
    ...
    // Act & Assert
    BadRequestException exception =
        assertThrows(BadRequestException.class, () -> messageService.createMessage(null));
    assertEquals(ErrorMessages.MessagesErrorMessages.INVALID_ID, exception.getMessage());
  }

  /**
   * Purpose: Reject message creation when title is null. Expected Result: BadRequestException with
   * ER003 error code.
   */
  @Test
  ...
  /**
   * Purpose: Reject message creation with publishDate if sendAsEmail is false. Expected Result:
   * BadRequestException with PublishDateRequiresSendAsEmail error code.
   */
  @Test
  ...
  void createMessage_PublishDateRequiresSendAsEmail_ThrowsBadRequestException() {
    ...
    // Act & Assert
    assertThrowsBadRequest(
        ErrorMessages.MessagesErrorMessages.PUBLISH_DATE_REQUIRES_SEND_AS_EMAIL,
        () -> messageService.createMessage(validRequest));
  }

  /**
   * Purpose: Propagate repository exceptions when database save fails. Expected Result:
  ...
  void createMessage_RepositorySaveFailure_Propagates() {
    // Arrange
    stubClientRepositoryFindById(Optional.of(testClient));
    stubMessageRepositorySaveThrowsRuntimeException(
  ...
  /**
   * Purpose: Reject message with title exceeding maximum length is rejected. Scenario:
   * CreateMessage with 501-character title. Expected Result: BadRequestException with TitleTooLong
   * error.
   */
  @Test
  ...
  void createMessage_TitleTooLong_ThrowsBadRequestException() {
    ...
    // Act & Assert
    assertThrowsBadRequest(
        ErrorMessages.MessagesErrorMessages.TITLE_TOO_LONG,
        () -> messageService.createMessage(validRequest));
  }

  /**
   * Purpose: Reject message creation when client ID is missing/zero. Expected Result:
   * NotFoundException with client invalid ID message.
   */
  @Test
  ...
  /**
   * Purpose: Verify that the createMessage controller method is protected by correct @PreAuthorize
   * permission. Expected: Method has @PreAuthorize referencing INSERT_MESSAGES_PERMISSION.
   */
  @Test
  ...
  void createMessage_VerifyPreAuthorizeAnnotation_Success() throws NoSuchMethodException {
    // Arrange
    var method =
        MessageController.class.getMethod(
            "createMessage", com.example.SpringApi.Models.RequestModels.MessageRequestModel.class);
    ...
    // Assert
    assertNotNull(
        preAuthorizeAnnotation, "createMessage method should have @PreAuthorize annotation");
  ...
  /**
   * Purpose: Verify that the controller correctly delegates createMessage calls to the service
   * layer. Expected: Service method called with correct request and HTTP 200 returned.
   */
  @Test
