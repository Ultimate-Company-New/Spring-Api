...
class RecordCashPaymentFollowUpTest extends PaymentServiceTestBase {
    ...
    /**
     * Purpose: Verify follow-up cash payment enforces allowed statuses.
    ...
    /**
     * Purpose: Verify payment date is mandatory for follow-up manual payment.
     * Expected Result: BadRequestException with payment date required message.
     * Assertions: Exception type and exact message.
     */
    @Test
    ...
    /**
     * Purpose: Verify positive amount is mandatory for follow-up manual payment.
     * Expected Result: BadRequestException with valid payment amount required message.
     * Assertions: Exception type and exact message.
     */
    @Test
    ...
    /**
     * Purpose: Verify follow-up payment amount cannot exceed pending amount.
     * Expected Result: BadRequestException with formatted pending-amount exceeded message.
     * Assertions: Exception type and exact message.
     */
    @Test
    ...
    void recordCashPaymentFollowUp_f11_amountExceedsPendingAmount_failure() {
        // Arrange
        testPurchaseOrder.setPurchaseOrderStatus(PurchaseOrder.Status.APPROVED_WITH_PARTIAL_PAYMENT.getValue());
        ...
        assertEquals(String.format(
                ...
                new BigDecimal("500.00")), ex.getMessage());
    }

    /**
     * Purpose: Verify follow-up UPI manual payment updates payment method and order status as fully paid.
     * Expected Result: Success response with APPROVED status and UPI payment method.
    ...
    void recordCashPaymentFollowUp_s02_upiPaymentFullApproval_success() {
        ...
        // Act
        PaymentVerificationResponseModel response = paymentService.recordCashPaymentFollowUp(testCashPaymentRequest);
    ...
    /**
     * Purpose: Verify follow-up manual payment supports CASH method when UPI transaction id is absent/blank.
     * Expected Result: Success response with saved payment method CASH and no order-summary update when summary is absent.
     * Assertions: Saved payment method, test-payment flag, and successful response.
     */
    @Test
