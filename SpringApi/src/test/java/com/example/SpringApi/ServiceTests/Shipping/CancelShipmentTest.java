...
class CancelShipmentTest extends ShippingServiceTestBase {
  ...
  /**
   * Purpose: Verify shipment save is called on success. Expected Result: Shipment repository save
   * called. Assertions: verify save called.
   */
  @Test
  ...
  /**
   * Purpose: Verify already cancelled shipment throws BadRequestException. Expected Result:
  ...
  /**
   * Purpose: Verify cancel API exception throws BadRequestException. Expected Result:
   * BadRequestException with InvalidIdWithMessageFormat message. Assertions: Exception type and
   * message.
   */
  @Test
  ...
  void cancelShipment_CancelApiError_ThrowsBadRequestException() {
    ...
    assertEquals(
        String.format(
            ErrorMessages.ShipmentErrorMessages.INVALID_ID_WITH_MESSAGE_FORMAT,
  ...
  /**
   * Purpose: Verify missing ShipRocket order id throws BadRequestException. Expected Result:
  ...
  /**
   * Purpose: Verify non-numeric order id throws BadRequestException. Expected Result:
   * BadRequestException with InvalidIdFormatErrorFormat message. Assertions: Exception type and
   * message.
   */
  @Test
  ...
  void cancelShipment_NonNumericOrderId_ThrowsBadRequestException() {
    ...
    // Assert
    assertEquals(
        String.format(ErrorMessages.ShipmentErrorMessages.INVALID_ID_FORMAT_ERROR_FORMAT, "SR-ABC"),
        ex.getMessage());
  }

  /**
   * Purpose: Verify missing shipment throws NotFoundException. Expected Result: NotFoundException
  ...
  /**
   * Purpose: Verify controller has @PreAuthorize for cancelShipment. Expected Result: Annotation
