...
class GetRazorpayKeyIdTest extends PaymentServiceTestBase {
    ...
    /**
     * Purpose: Verify controller delegates getRazorpayKeyId to service.
    ...
    /**
     * Purpose: Verify client repository is queried during key-id lookup flow.
     ...
     * Expected Result: Public Razorpay key is returned.
     * Assertions: Returned key matches configured client key.
     */
    @Test
    ...
    /**
     * Purpose: Verify key-id lookup fails when client context is unavailable.
    ...
    /**
     * Purpose: Verify service validates client Razorpay API key presence.
    ...
    void getRazorpayKeyId_razorpayApiKeyMissing_failure() {
        ...
        // Act
        BadRequestException ex = assertThrows(BadRequestException.class, () -> paymentService.getRazorpayKeyId());
    ...
    /**
     * Purpose: Verify endpoint is intentionally open (no @PreAuthorize annotation).
     * Expected Result: @PreAuthorize is absent.
     * Assertions: annotation is null.
     */
    @Test
    ...
    /**
     * Purpose: Verify controller handles downstream service exception for key-id endpoint.
     * Expected Result: HTTP 500 INTERNAL_SERVER_ERROR.
     * Assertions: Response status is INTERNAL_SERVER_ERROR.
     */
    @Test
    @DisplayName("getRazorpayKeyId - Service Exception - Failure")
    void getRazorpayKeyId_serviceException_failure() {
        // Arrange
        stubPaymentServiceGetRazorpayKeyIdThrows(new RuntimeException(ErrorMessages.ERROR_INTERNAL_SERVER_ERROR));
