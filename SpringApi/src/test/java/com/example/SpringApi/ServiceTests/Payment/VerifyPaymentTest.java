...
class VerifyPaymentTest extends PaymentServiceTestBase {
  ...
  /**
   * Purpose: Verify client repository interaction is performed during verifyPayment. Expected
  ...
  void verifyPayment_verifyClientRepositoryInteraction_success() {
    ...
    NotFoundException ex =
        assertThrows(
            NotFoundException.class, () -> paymentService.verifyPayment(testVerifyRequest));
  ...
  /**
   * Purpose: Verify negative purchase order id follows the same client-not-found path. Expected
  ...
  /**
   * Purpose: Verify zero purchase order id follows the same client-not-found path. Expected Result:
  ...
  /**
   * Purpose: Verify max long purchase order id follows the same client-not-found path. Expected
   * Result: NotFoundException with Client InvalidId message. Assertions: Exception type and exact
   * message.
   */
  @Test
  ...
  /**
   * Purpose: Verify additional invalid IDs all follow the same client-not-found path. Expected
   * Result: NotFoundException with Client InvalidId for each id. Assertions: Exception type and
  ...
  /**
   * Purpose: Verify verifyPayment fails when purchase order belongs to another client. Expected
  ...
  void verifyPayment_f06_clientAccessDenied_failure() {
    ...
    BadRequestException ex =
        assertThrows(
            BadRequestException.class, () -> paymentService.verifyPayment(testVerifyRequest));
  ...
  /**
   * Purpose: Verify verifyPayment validates purchase order existence after resolving client
   * credentials. Expected Result: NotFoundException with purchase order invalid id message.
  ...
  /**
   * Purpose: Verify verifyPayment fails when tracked payment order is not found. Expected Result:
   * BadRequestException with payment order not found message. Assertions: Exception type and exact
   * message.
   */
  @Test
  ...
  void verifyPayment_f07_paymentOrderNotFound_failure() {
    ...
    stubClientRepositoryFindByIdDefaultClient(Optional.of(testClient));
    stubPurchaseOrderRepositoryFindById(TEST_PO_ID, Optional.of(testPurchaseOrder));
    stubPaymentRepositoryFindByRazorpayOrderId(
        testVerifyRequest.getRazorpayOrderId(), Optional.empty());
   ...
   * Expected Result: Response success=false and payment status FAILED. Assertions: Response fields
   * and updated payment state.
   */
  @Test
  ...
  void verifyPayment_f08_invalidSignatureMarksFailed_failure() {
    ...
    stubPurchaseOrderRepositoryFindById(TEST_PO_ID, Optional.of(testPurchaseOrder));
    stubPaymentRepositoryFindByRazorpayOrderId(
        testVerifyRequest.getRazorpayOrderId(), Optional.of(testPayment));
  ...
  /**
   * Purpose: Verify valid signature captures payment and marks purchase order as APPROVED when
   * fully paid. Expected Result: Success response with APPROVED status and zero pending amount.
   ...
   * Expected Result: BadRequestException with formatted overpayment message. Assertions: Exception
  ...
  /**
   * Purpose: Verify signature helper safely returns false when secret is invalid/null. Expected
   * Result: Signature verification returns false without propagating exception. Assertions: Return
   * value is false.
   */
  @Test
  ...
  void verifyPayment_f10_signatureHelperHandlesNullSecret_failure() {
    ...
    Boolean result =
        ReflectionTestUtils.invokeMethod(
            paymentService, "verifyRazorpaySignature", "order_abc", "pay_abc", "signature", null);
  ...
  /**
   * Purpose: Verify controller method has expected @PreAuthorize permission constant. Expected
   * Result: Annotation exists and contains UPDATE_PURCHASE_ORDERS_PERMISSION. Assertions:
   * annotation not null and value contains expected permission.
   */
  @Test
  ...
  void verifyPayment_p02_controllerPreAuthorizeAnnotation_success() throws NoSuchMethodException {
    // Arrange
    Method method =
        PaymentController.class.getMethod("verifyPayment", RazorpayVerifyRequestModel.class);
  ...
  /**
   * Purpose: Verify controller delegates verifyPayment request to service. Expected Result: HTTP
   * 200 OK with delegated service call. Assertions: Service interaction and response code.
   */
  @Test
