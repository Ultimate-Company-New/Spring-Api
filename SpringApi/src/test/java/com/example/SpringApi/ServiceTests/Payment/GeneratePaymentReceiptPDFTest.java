...
class GeneratePaymentReceiptPDFTest extends PaymentServiceTestBase {
    ...
    /**
     * Purpose: Verify receipt generation validates purchase order existence.
    ...
    /**
     * Purpose: Verify receipt generation validates client existence.
     ...
     * Expected Result: Generated byte[] returned and helper calls executed once.
     * Assertions: Returned PDF bytes and helper invocations.
     */
    @Test
    ...
    void generatePaymentReceiptPDF_s03_generatesPdfBytes_success() throws Exception {
        ...
        try (MockedStatic<HTMLHelper> htmlHelperMock = org.mockito.Mockito.mockStatic(HTMLHelper.class);
             MockedStatic<PDFHelper> pdfHelperMock = org.mockito.Mockito.mockStatic(PDFHelper.class)) {
            htmlHelperMock.when(() -> HTMLHelper.replaceBrTags(org.mockito.ArgumentMatchers.anyString()))
                    .thenAnswer(invocation -> invocation.getArgument(0));
            pdfHelperMock.when(() -> PDFHelper.convertHtmlToPdf(org.mockito.ArgumentMatchers.anyString()))
            ...
            htmlHelperMock.verify(() -> HTMLHelper.replaceBrTags(org.mockito.ArgumentMatchers.anyString()), times(1));
            pdfHelperMock.verify(() -> PDFHelper.convertHtmlToPdf(org.mockito.ArgumentMatchers.anyString()), times(1));
            verify(userLogService, times(1)).logData(1L,
    ...
    /**
     * Purpose: Verify receipt HTML population handles captured timestamp and optional payment metadata fields.
     * Expected Result: PDF bytes are generated successfully with non-null payment metadata.
     * Assertions: Returned bytes and helper invocation counts.
     */
    @Test
    ...
    /**
     * Purpose: Verify receipt HTML population falls back to paymentDate and amountPaidPaise when amountPaid is absent.
     * Expected Result: PDF bytes are generated successfully using alternate date/amount branches.
     * Assertions: Returned bytes and helper invocation counts.
     */
    @Test
