...
class InitiateRefundTest extends PaymentServiceTestBase {
    ...
    /**
     * Purpose: Verify refund success path uses remaining refundable amount when amount is not provided.
     * Expected Result: Payment is updated as fully refunded and order summary pending amount is increased.
     * Assertions: Updated payment fields, summary pending amount, and refund payload defaults.
     */
    @Test
    ...
    void initiateRefund_s02_fullRefundUsesRemainingAmount_success() throws Exception {
        ...
        try {
            ...
            assertEquals("Customer requested refund",
    ...
    /**
     * Purpose: Verify refund success path respects explicit amount and reason values.
     * Expected Result: Payment becomes partially refunded and custom reason is sent in payload.
     * Assertions: Payment status, refunded amount, and captured custom reason.
     */
    @Test
    ...
    void initiateRefund_s03_partialRefundUsesExplicitReason_success() throws Exception {
        ...
        try {
            // Act
            Payment updatedPayment = paymentService.initiateRefund(TEST_PAYMENT_ID, 10000L, "Damaged shipment");

            // Assert
            assertEquals("rfnd_partial_001", updatedPayment.getLastRefundId());
            assertEquals(10000L, updatedPayment.getAmountRefundedPaise());
            assertEquals(Payment.PaymentStatus.PARTIALLY_REFUNDED.getValue(), updatedPayment.getPaymentStatus());
            assertEquals(new BigDecimal("200.00"), testOrderSummary.getPendingAmount());
            assertEquals("Damaged shipment", capturedRazorpayRefundRequest.getJSONObject("notes").getString("reason"));
        } finally {
            closeMockedRazorpayClientConstruction();
        }
    }
    ...
    /**
     * Purpose: Verify refund flow validates payment existence after client validation.
    ...
    /**
     * Purpose: Verify refund flow denies access when payment belongs to another client.
    ...
    /**
     * Purpose: Verify refund is rejected when payment is not in refundable state.
     * Expected Result: BadRequestException with cannot refund message.
     * Assertions: Exception type and exact message.
     */
    @Test
    ...
    /**
     * Purpose: Verify requested refund amount cannot exceed remaining refundable amount.
     * Expected Result: BadRequestException with refundable amount exceeded message.
    ...
    void initiateRefund_f09_amountExceedsRefundableAmount_failure() {
        ...
        assertEquals(String.format(
                ...
                70000L), ex.getMessage());
    }

    /**
     * Purpose: Verify refund gateway errors are wrapped into a standardized BadRequestException message.
     * Expected Result: BadRequestException with formatted process-refund failure message.
     * Assertions: Exception type and exact formatted message.
     */
    @Test
    ...
    void initiateRefund_f10_razorpayRefundException_failure() {
        ...
        try {
            ...
            assertEquals(String.format(
                    ...
                    "refund api unavailable"), ex.getMessage());
        } finally {
            closeMockedRazorpayClientConstruction();
        }
    }
