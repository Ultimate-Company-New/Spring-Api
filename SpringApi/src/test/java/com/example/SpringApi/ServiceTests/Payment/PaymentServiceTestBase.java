...
class PaymentServiceTestBase {
    ...
    protected void stubPaymentRepositoryFindByRazorpayOrderId(String razorpayOrderId, Optional<Payment> result) {
    ...
    protected void stubPaymentRepositoryFindAllByPurchaseOrderId(Long purchaseOrderId, List<Payment> result) {
        lenient().when(paymentRepository.findAllByPurchaseOrderId(purchaseOrderId)).thenReturn(result);
    }

    protected void stubPaymentRepositoryGetTotalNetPaidPaiseForEntity(String entityType, Long entityId, Long totalPaidPaise) {
        lenient().when(paymentRepository.getTotalNetPaidPaiseForEntity(entityType, entityId)).thenReturn(totalPaidPaise);
    }

    protected void stubPaymentRepositoryGetTotalNetPaidPaiseForPurchaseOrder(Long purchaseOrderId, Long totalPaidPaise) {
        lenient().when(paymentRepository.getTotalNetPaidPaiseForEntity(
                Payment.EntityType.PURCHASE_ORDER.getValue(), purchaseOrderId)).thenReturn(totalPaidPaise);
    }

    protected void stubPaymentRepositoryGetTotalNetPaidPaiseForPurchaseOrderSequence(Long purchaseOrderId, Long... totals) {
        if (totals == null || totals.length == 0) {
            return;
        }
        Long[] remaining = totals.length > 1 ? java.util.Arrays.copyOfRange(totals, 1, totals.length) : new Long[0];
        lenient().when(paymentRepository.getTotalNetPaidPaiseForEntity(
                Payment.EntityType.PURCHASE_ORDER.getValue(), purchaseOrderId)).thenReturn(totals[0], remaining);
    }
    ...
    protected void stubOrderSummaryRepositoryFindByPurchaseOrderId(Long purchaseOrderId, Optional<OrderSummary> result) {
        lenient().when(orderSummaryRepository.findByPurchaseOrderId(purchaseOrderId)).thenReturn(result);
    }

    protected void stubOrderSummaryRepositoryFindByEntityTypeAndEntityId(String entityType, Long entityId, Optional<OrderSummary> result) {
        lenient().when(orderSummaryRepository.findByEntityTypeAndEntityId(entityType, entityId)).thenReturn(result);
    }

    protected void stubOrderSummaryRepositorySaveReturnsArgument() {
        lenient().when(orderSummaryRepository.save(any(OrderSummary.class))).thenAnswer(invocation -> invocation.getArgument(0));
    }
    ...
    protected void stubRazorpayClientOrderCreateReturns(String razorpayOrderId) {
        closeMockedRazorpayClientConstruction();
        mockedRazorpayClient = org.mockito.Mockito.mockConstruction(RazorpayClient.class, (mock, context) -> {
            mockedRazorpayOrderClient = org.mockito.Mockito.mock(OrderClient.class);
            mock.orders = mockedRazorpayOrderClient;
            lenient().when(mockedRazorpayOrderClient.create(any(JSONObject.class))).thenAnswer(invocation -> {
    ...
    protected void stubRazorpayClientPaymentRefundReturns(String refundId) {
        ...
        mockedRazorpayClient = org.mockito.Mockito.mockConstruction(RazorpayClient.class, (mock, context) -> {
            ...
            lenient().when(mockedRazorpayPaymentClient.refund(anyString(), any(JSONObject.class))).thenAnswer(invocation -> {
    ...
    protected String createValidRazorpaySignature(String razorpayOrderId, String razorpayPaymentId, String secret) {
        try {
