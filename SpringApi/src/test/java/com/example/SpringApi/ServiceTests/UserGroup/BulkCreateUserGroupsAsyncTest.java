...
class BulkCreateUserGroupsAsyncTest extends UserGroupServiceTestBase {
  ...
  /**
   * Purpose: Verify all valid groups are created async successfully. Expected Result: Groups saved,
   * message notification created. Assertions: verify
   */
  @Test
  ...
  void bulkCreateUserGroupsAsync_success_allValid() {
    ...
    verify(messageService, times(1))
        .createMessageWithContext(any(), eq(TEST_USER_ID), eq("testUser"), eq(TEST_CLIENT_ID));
  }

  /**
   * Purpose: Verify partial failure handles valid groups and duplicates. Expected Result: Valid
   * group saved, message notification created. Assertions: verify
   */
  @Test
  ...
  void bulkCreateUserGroupsAsync_success_partialFailures() {
    ...
    stubUserGroupRepositoryFindByGroupNameForPartialFailure(
        validRequest.getGroupName(), duplicateRequest.getGroupName());
    stubUserGroupRepositorySave(testUserGroup);
    stubUserGroupUserMapRepositorySaveAll(new ArrayList<>());
    stubUserLogServiceLogDataWithContext(true);
  ...
  /**
   * Purpose: Verify duplicate name does not save but notifies user. Expected Result: message
   * notification created, repo.save never called. Assertions: verify
   */
  @Test
  ...
  /**
   * Purpose: Verify empty list handles gracefully with notification. Expected Result: message
   * notification created. Assertions: verify
   */
  @Test
  ...
  /**
   * Purpose: Verify null list handles gracefully with notification. Expected Result: message
   * notification created. Assertions: verify
   */
  @Test
  ...
  /**
   * Purpose: Verify unexpected exception during async processing records failure. Expected Result:
   * message notification created. Assertions: verify
   */
  @Test
