 ...
 * 
 * Provides shared mock objects and common test data initialization for all
 * Login-related tests.
 * This class should be extended by all LoginService test classes to ensure
 * consistent
...
abstract class LoginServiceTestBase {
    ...
    @Mock
    protected ILoginSubTranslator loginServiceMock;
     ...
     */
    protected void stubEnvironmentSendGridProperties(String senderEmail, String senderName, String apiKey) {
     ...
     */
    protected void stubEnvironmentBrevoProperties(String senderEmail, String senderName, String apiKey) {
     ...
     */
    protected void stubUserClientMappingRepositoryFindByApiKey(String apiKey, Optional<UserClientMapping> mapping) {
     ...
     */
    protected void stubUserClientMappingRepositoryFindByUserId(Long userId, List<UserClientMapping> mappings) {
        lenient().when(userClientMappingRepository.findByUserId(userId)).thenReturn(mappings);
    }

    /**
     * Stub
     * userClientPermissionMappingRepository.findClientPermissionMappingByUserId.
     */
    protected void stubUserClientPermissionMappingByUserId(Long userId,
            List<com.example.SpringApi.Models.DatabaseModels.UserClientPermissionMapping> mappings) {
        lenient().when(userClientPermissionMappingRepository.findClientPermissionMappingByUserId(userId))
                .thenReturn(mappings);
    }
    ...
    protected void stubJwtTokenProviderGenerateToken(String token) {
        lenient().when(jwtTokenProvider.generateToken(any(User.class), anyList(), anyLong())).thenReturn(token);
    }

    /**
     * Stub PasswordHelper random password and hash/salt.
     */
    protected org.mockito.MockedStatic<PasswordHelper> stubPasswordHelperRandomPassword(String randomPassword,
            String[] saltAndHash) {
     ...
     */
    protected org.mockito.MockedStatic<PasswordHelper> stubPasswordHelperCheckPassword(boolean result) {
        org.mockito.MockedStatic<PasswordHelper> mocked = mockStatic(PasswordHelper.class);
        mocked.when(() -> PasswordHelper.checkPassword(anyString(), anyString(), anyString()))
                .thenReturn(result);
        return mocked;
    }
     ...
     */
    protected org.mockito.MockedConstruction<EmailTemplates> stubEmailTemplatesSendResetPasswordEmail(boolean result) {
        return mockConstruction(EmailTemplates.class,
                (mock, context) -> lenient().when(mock.sendResetPasswordEmail(anyString(), anyString()))
                        .thenReturn(result));
    }
    ...
    protected void stubLoginServiceThrowsUnauthorizedOnResetPassword() {
        lenient().when(loginServiceMock.resetPassword(any())).thenThrow(
                new com.example.SpringApi.Exceptions.UnauthorizedException(ErrorMessages.ERROR_UNAUTHORIZED));
    }
    ...
    protected void stubLoginServiceThrowsUnauthorizedOnConfirmEmail() {
        lenient().doThrow(new com.example.SpringApi.Exceptions.UnauthorizedException(ErrorMessages.ERROR_UNAUTHORIZED))
                .when(loginServiceMock).confirmEmail(any());
    }

    protected void stubLoginServiceThrowsUnauthorizedOnGetToken() {
        lenient().when(loginServiceMock.getToken(any())).thenThrow(
                new com.example.SpringApi.Exceptions.UnauthorizedException(ErrorMessages.ERROR_UNAUTHORIZED));
    }

    protected void stubLoginServiceThrowsUnauthorizedOnSignIn() {
        lenient().when(loginServiceMock.signIn(any())).thenThrow(
                new com.example.SpringApi.Exceptions.UnauthorizedException(ErrorMessages.ERROR_UNAUTHORIZED));
    }
}
