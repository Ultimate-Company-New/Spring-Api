...
class VerifyPaymentFollowUpTest extends PaymentServiceTestBase {
   ...
   * Expected Result: clientRepository.findById is called once. Assertions: verify interaction
   * count.
   */
  @Test
  ...
  void verifyPaymentFollowUp_verifyClientRepositoryInteraction_success() {
    ...
    NotFoundException ex =
        assertThrows(
            NotFoundException.class, () -> paymentService.verifyPaymentFollowUp(testVerifyRequest));
  ...
  /**
   * Purpose: Verify follow-up verification fails when client context is unavailable. Expected
  ...
  /**
   * Purpose: Verify additional invalid IDs all follow same client-not-found path. Expected Result:
  ...
  /**
   * Purpose: Verify follow-up verification rejects purchase orders from other clients. Expected
  ...
  /**
   * Purpose: Verify follow-up verification fails when payment order lookup fails. Expected Result:
  ...
  /**
   * Purpose: Verify invalid follow-up signature marks payment as failed and returns failure
   * response. Expected Result: Response success=false and payment status FAILED. Assertions:
   * Response fields and payment status.
   */
  @Test
   ...
   * Expected Result: Success response with APPROVED_WITH_PARTIAL_PAYMENT. Assertions: Response
   * status and pending amount.
   */
  @Test
  ...
  void verifyPaymentFollowUp_p02_controllerPreAuthorizeAnnotation_success()
      throws NoSuchMethodException {
    // Arrange
    Method method =
        PaymentController.class.getMethod(
            "verifyPaymentFollowUp", RazorpayVerifyRequestModel.class);
  ...
  /**
   * Purpose: Verify controller delegates follow-up verification request to service. Expected
