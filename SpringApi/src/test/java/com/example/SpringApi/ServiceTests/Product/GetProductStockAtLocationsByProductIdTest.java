...
class GetProductStockAtLocationsByProductIdTest extends ProductServiceTestBase {
  ...
  /**
   * Purpose: Verify stock results returned when mappings exist. Expected Result: Non-empty response
   * list. Assertions: Response list size matches mappings.
   */
  @Test
  ...
  void getProductStockAtLocationsByProductId_ValidProduct_Success() {
    ...
    // Act
    List<ProductStockByLocationResponseModel> results =
        productService.getProductStockAtLocationsByProductId(TEST_PRODUCT_ID, 1, "12345", false);
  ...
  /**
   * Purpose: Verify InvalidDataAccessResourceUsageException is handled. Expected Result: Empty list
   * returned. Assertions: Result list is empty.
   */
  @Test
  ...
  /**
   * Purpose: Verify SQLGrammarException is handled. Expected Result: Empty list returned.
  ...
  /**
   * Purpose: Verify generic exception is handled. Expected Result: Empty list returned. Assertions:
  ...
  /**
   * Purpose: Verify controller permission test for unauthorized access. Expected Result: Error
   * response returned when service throws unauthorized. Assertions: Response status is
   * INTERNAL_SERVER_ERROR (unauthorized not explicitly handled).
   */
  @Test
  @DisplayName(
      "getProductStockAtLocationsByProductId - Controller permission unauthorized - Success")
  void getProductStockAtLocationsByProductId_controller_permission_unauthorized() {
    ...
    ResponseEntity<?> response =
        controller.getProductStockAtLocationsByProductId(TEST_PRODUCT_ID, 1, "12345", false);
  ...
  /**
   * Purpose: Verify @PreAuthorize annotation exists. Expected Result: Annotation includes required
   * permissions. Assertions: Annotation contains VIEW_PRODUCTS_PERMISSION.
   */
  @Test
  @DisplayName("getProductStockAtLocationsByProductId - Verify @PreAuthorize annotation - Success")
  void getProductStockAtLocationsByProductId_VerifyPreAuthorizeAnnotation_Success()
