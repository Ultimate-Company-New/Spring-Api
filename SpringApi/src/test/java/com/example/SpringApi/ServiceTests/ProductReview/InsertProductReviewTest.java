...
class InsertProductReviewTest extends ProductReviewServiceTestBase {
  ...
  void insertProductReview_f01_InvalidReviewText_ThrowsBadRequestException(
      String scenario, Consumer<ProductReviewRequestModel> requestMutator) {
    // Arrange
    ProductReviewRequestModel request = buildValidProductReviewRequest();
    ...
    BadRequestException ex =
        assertThrows(
            BadRequestException.class, () -> productReviewService.insertProductReview(request));

    // Assert
    assertEquals(
        ErrorMessages.ProductReviewErrorMessages.ER002, ex.getMessage(), "Scenario: " + scenario);
  }

  private static Stream<Arguments> invalidReviewTextScenarios() {
    return Stream.of(
        Arguments.of(
            "Empty Review Text", (Consumer<ProductReviewRequestModel>) req -> req.setReview("")),
        Arguments.of(
            "Null Review Text", (Consumer<ProductReviewRequestModel>) req -> req.setReview(null)),
        Arguments.of(
            "Whitespace Review Text",
            (Consumer<ProductReviewRequestModel>) req -> req.setReview("   ")));
  }
  ...
  void insertProductReview_verifyPreAuthorizeAnnotation_success() throws NoSuchMethodException {
    // Arrange
    Method method =
        ProductReviewController.class.getMethod(
            "insertProductReview", ProductReviewRequestModel.class);
