...
abstract class FilterQueryBuilderTestBase {
    ...
    protected <T> QueryFixture<T> stubPagedQueries(Class<T> entityClass, List<T> resultList, long totalCount) {
        ...
        when(entityManager.createQuery(anyString(), eq(Long.class))).thenAnswer(invocation -> {
            capturedCountQuery.set(invocation.getArgument(0, String.class));
            return countQuery;
        });

        when(entityManager.createQuery(anyString(), eq(entityClass))).thenAnswer(invocation -> {
    ...
    protected PurchaseOrderDetailsQueryFixture stubPurchaseOrderDetailsQueries(
        ...
        when(entityManager.createQuery(anyString(), eq(PurchaseOrder.class))).thenAnswer(invocation -> {
            capturedMainQuery.set(invocation.getArgument(0, String.class));
            return mainQuery;
        });

        when(entityManager.createQuery(anyString(), eq(ShipmentPackage.class))).thenAnswer(invocation -> {
            capturedShipmentPackageQuery.set(invocation.getArgument(0, String.class));
            return shipmentPackageQuery;
        });

        when(entityManager.createQuery(anyString(), eq(Resources.class))).thenAnswer(invocation -> {
            capturedResourcesQuery.set(invocation.getArgument(0, String.class));
            return resourcesQuery;
        });

        when(entityManager.createQuery(anyString(), eq(Payment.class))).thenAnswer(invocation -> {
