...
class GetLeadDetailsTest extends LeadServiceTestBase {
     ...
     * Expected Result: Authorization is verified during the fetch process.
     * Assertions: hasAuthority is called with correct permission.
     */
    @Test
    @DisplayName("Get Lead Details - Permission check - Success Verifies Authorization")
    void getLeadDetails_PermissionCheck_SuccessVerifiesAuthorization() {
        // Arrange
        when(leadRepository.findLeadWithDetailsById(DEFAULT_LEAD_ID, TEST_CLIENT_ID)).thenReturn(testLead);
