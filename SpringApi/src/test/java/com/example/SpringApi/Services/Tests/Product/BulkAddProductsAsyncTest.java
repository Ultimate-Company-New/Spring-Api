...
class BulkAddProductsAsyncTest extends ProductServiceTestBase {
     ...
     * Expected Result: Products inserted and log entry created.
     ...
     * Expected Result: All products attempted and saved.
     * Assertions: Repository save invoked.
     */
    @Test
     ...
     * Expected Result: Processing continues with success log.
     * Assertions: UserLogService called with success message.
     */
    @Test
    ...
    /**
     * Purpose: Verify database error is handled and reported.
     * Expected Result: Exception is handled without throwing.
     * Assertions: No exception thrown.
     */
    @Test
    ...
    void bulkAddProductsAsync_DatabaseError_ExceptionHandled() {
        // Arrange
        List<ProductRequestModel> products = Collections.singletonList(testProductRequest);
        stubProductRepositorySaveThrows(new RuntimeException(ErrorMessages.CommonErrorMessages.DATABASE_ERROR));
        stubClientServiceGetClientById(TEST_CLIENT_ID, testClientResponse);
        stubUserLogServiceLogDataWithContext();
        ...
        try (MockedConstruction<ImgbbHelper> imgbbMock = mockConstruction(ImgbbHelper.class,
                (mock, context) -> stubImgbbHelperUploadSuccess(mock))) {
            // Act & Assert
            assertDoesNotThrow(() -> productService.bulkAddProductsAsync(products, DEFAULT_USER_ID, CREATED_USER,
                    TEST_CLIENT_ID));
        }
    }
     ...
     * Expected Result: No saves or logs executed.
     ...
     * Expected Result: No saves executed.
     * Assertions: Repository save not called.
     */
    @Test
    ...
    /**
     * Purpose: Verify partial invalid product input is handled.
     * Expected Result: Processing continues without throwing.
     * Assertions: No exception thrown.
     */
    @Test
    ...
    void bulkAddProductsAsync_controller_permission_unauthorized() {
        ...
        List<ProductRequestModel> products = Arrays.asList(testProductRequest, testProductRequest,
                testProductRequest, testProductRequest, testProductRequest, testProductRequest);
