...
class UserServiceTest {
    ...
    void getUserById_Success() {
        ...
        when(userRepository.findById(TEST_USER_ID)).thenReturn(Optional.of(testUser));
        when(userRepository.findUserPermissions(TEST_USER_ID)).thenReturn(permissionData);
        ...
        verify(userRepository, times(1)).findById(TEST_USER_ID);
        verify(userRepository, times(1)).findUserPermissions(TEST_USER_ID);
    }
    ...
    void getUserByEmail_Success() {
        ...
        when(userRepository.findUserPermissions(TEST_USER_ID)).thenReturn(new ArrayList<>());
    ...
    void fetchUsersInCarrierInBatches_Success() {
        ...
        when(userRepository.findPaginatedUsers(
            eq(100L), isNull(), anyString(), eq("contains"), anyString(), eq(false), any(PageRequest.class)
        )).thenReturn(userPage);
    ...
    void fetchUsersInCarrierInBatches_WithFilter_Success() {
        ...
        when(userRepository.findPaginatedUsers(
            eq(100L), isNull(), eq("firstName"), eq("contains"), eq("Test"), eq(false), any(PageRequest.class)
        )).thenReturn(userPage);
