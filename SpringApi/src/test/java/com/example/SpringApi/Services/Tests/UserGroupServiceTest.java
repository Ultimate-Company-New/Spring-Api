...
class UserGroupServiceTest extends BaseTest {
    ...
    void setUp() {
        ...
        testUserGroupRequest.setUserIds(Arrays.asList(DEFAULT_USER_ID, 2L, 3L));
        
        // Initialize test user group using constructor
        testUserGroup = new UserGroup(testUserGroupRequest, DEFAULT_CREATED_USER, DEFAULT_CLIENT_ID);
        ...
        testMapping = new UserGroupUserMap(DEFAULT_USER_ID, TEST_GROUP_ID, DEFAULT_CREATED_USER);
        ...
        lenient().doReturn(DEFAULT_CLIENT_ID).when(userGroupService).getClientId();
        lenient().doReturn(DEFAULT_USER_ID).when(userGroupService).getUserId();
        lenient().doReturn(DEFAULT_LOGIN_NAME).when(userGroupService).getUser();
    }

    // ==================== Toggle User Group Tests ====================
    
    @Nested
    @DisplayName("toggleUserGroup Tests")
    ...
    @Nested
    @DisplayName("getUserGroupDetailsById Tests")
    ...
    @Nested
    @DisplayName("createUserGroup Tests")
    ...
    @Nested
    @DisplayName("updateUserGroup Tests")
    ...
    @Nested
    @DisplayName("fetchUserGroupsInClientInBatches Tests")
    class FetchUserGroupsInClientInBatchesTests {
        ...
        void fetchUserGroupsInClientInBatches_Success() {
            ...
            // Act
            PaginationBaseResponseModel<UserGroupResponseModel> result = userGroupService.fetchUserGroupsInClientInBatches(request);
        ...
        void fetchUserGroupsInClientInBatches_InvalidColumn_ThrowsBadRequestException() {
            ...
            request.setFilters(Arrays.asList(invalidFilter));
         ...
         * UserGroupService validates columns and operators.
         */
        @Test
        @DisplayName("Fetch User Groups In Batches - Filter Logic Triple Loop Validation")
        void fetchUserGroupsInClientInBatches_TripleLoopValidation() {
            // 1. Columns
            String[] validColumns = {
            ...
            // Mock response
            Page<UserGroup> emptyPage = new PageImpl<>(Collections.emptyList());
            lenient().when(userGroupFilterQueryBuilder.findPaginatedEntitiesWithMultipleFilters(
            ...
            for (String column : joinArrays(validColumns, invalidColumns)) {
                for (String operator : joinArrays(validOperators, invalidOperators)) {
                    for (String value : values) {
                        UserGroupRequestModel req = new UserGroupRequestModel();
                        req.setStart(0);
                        req.setEnd(10);
                        PaginationBaseRequestModel.FilterCondition filter = new PaginationBaseRequestModel.FilterCondition();
                        ...
                        if (isValidColumn && isValidOperator) {
                            assertDoesNotThrow(() -> userGroupService.fetchUserGroupsInClientInBatches(req),
                                    "Failed for valid column/operator: " + column + "/" + operator);
                        } else {
                            BadRequestException ex = assertThrows(BadRequestException.class,
                                    () -> userGroupService.fetchUserGroupsInClientInBatches(req),
