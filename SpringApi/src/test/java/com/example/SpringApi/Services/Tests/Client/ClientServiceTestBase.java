...
public abstract class ClientServiceTestBase {
     ...
     */
    protected void stubClientFindByUserIdSequence(Long userId,
            java.util.List<Client> first,
            java.util.List<Client> second) {
        lenient().when(clientRepository.findByUserId(anyLong()))
                .thenReturn(first)
                .thenReturn(second);
    }
    ...
    protected MockedConstruction<FirebaseHelper> stubFirebaseUploadSuccess() {
        return mockConstruction(FirebaseHelper.class,
                (mock, context) -> lenient().when(mock.uploadFileToFirebase(anyString(), anyString()))
                        .thenReturn(true));
    }
     ...
     */
    protected MockedConstruction<ImgbbHelper> stubImgbbUploadResponse(ImgbbHelper.ImgbbUploadResponse response) {
        return mockConstruction(ImgbbHelper.class,
                (mock, context) -> lenient().when(mock.uploadFileToImgbb(anyString(), anyString()))
                        .thenReturn(response));
    }
}
