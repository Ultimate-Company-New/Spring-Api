...
public class GetTokenTest extends LoginServiceTestBase {
    ...
    /**
     * Purpose: Verify token generation includes multiple permissions correctly.
     * Expected Result: Token contains all three mock permissions.
     * Assertions: Permission list size in the generator matches the expected count.
     */
    @Test
    ...
    void getToken_MultiplePermissions_Success() {
        ...
        verify(jwtTokenProvider, times(1)).generateToken(any(User.class), argThat(list -> list.size() == 3), anyLong());
    }

    /**
     * Purpose: Verify token generation when no permissions exist for the client.
     * Expected Result: Token is generated with empty permissions.
     * Assertions: Returned token matches expectations and provider is called with empty list.
     */
    @Test
    ...
    /**
     * Purpose: Filter out permissions that belong to other clients during token generation.
     * Expected Result: Token generated with only matching client permissions.
     * Assertions: Permission list passed to generator is empty when mappings don't match.
     */
    @Test
    ...
    /**
     * Purpose: Verify token generation succeeds with standard valid inputs.
     * Expected Result: JWT token is generated.
     * Assertions: Returned token matches the provider output.
     */
    @Test
    ...
    /**
     * Purpose: Reject token request for different unknown user names.
     * Expected Result: UnauthorizedException is thrown.
     * Assertions: Exception message matches InvalidCredentials.
     */
    @Test
    ...
    /**
     * Purpose: Reject token request with an invalid API key.
     * Expected Result: UnauthorizedException is thrown.
     * Assertions: Exception message matches InvalidCredentials.
     */
    @Test
    ...
    /**
     * Purpose: Reject token request with a missing API key string.
    ...
    /**
     * Purpose: Reject token request with a missing login name string.
     * Expected Result: BadRequestException is thrown.
     * Assertions: Exception message matches ER015.
     */
    @Test
    ...
    /**
     * Purpose: Reject token request with a null API key.
     * Expected Result: BadRequestException is thrown.
     * Assertions: Exception message matches ER015.
     */
    @Test
    ...
    /**
     * Purpose: Reject token request with a null login name.
     * Expected Result: BadRequestException is thrown.
     * Assertions: Exception message matches ER015.
     */
    @Test
    ...
    /**
     * Purpose: Reject token request for a non-existent user.
     * Expected Result: UnauthorizedException is thrown.
     * Assertions: Exception message matches InvalidCredentials.
     */
    @Test
