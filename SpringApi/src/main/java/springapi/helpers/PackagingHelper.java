...
public class PackagingHelper {
  ...
  public static class PackagingEstimateResult {
    ...
    public PackagingEstimateResult(
      ...
      this.totalPackagesUsed =
          packagesUsed.stream().mapToInt(PackageUsageResult::getQuantityUsed).sum();
      this.totalPackagingCost =
          packagesUsed.stream()
      ...
      this.errorMessage =
          canPackAllItems
              ? null
              : "Not enough packages to pack all items. Can only pack "
                  + packedItems
                  + " of "
                  + requestedItems
                  + " items.";
    ...
    public List<PackageUsageResult> getPackagesUsed() {
      return packagesUsed;
    }

    public BigDecimal getTotalPackagingCost() {
      return totalPackagingCost;
    }

    public int getTotalPackagesUsed() {
      return totalPackagesUsed;
    }

    public int getMaxItemsPackable() {
      return maxItemsPackable;
    }

    public boolean isCanPackAllItems() {
      return canPackAllItems;
    }
   ...
   */
  public PackagingEstimateResult calculatePackaging(
      ProductDimension product, List<PackageDimension> availablePackages) {
    ...
    packages.sort(
        (a, b) -> {
          double costPerVolumeA =
              a.getVolume() > 0
                  ? a.getPricePerUnit().doubleValue() / a.getVolume()
                  : Double.MAX_VALUE;
          double costPerVolumeB =
              b.getVolume() > 0
                  ? b.getPricePerUnit().doubleValue() / b.getVolume()
                  : Double.MAX_VALUE;
    ...
    for (int i = 0; i < requestedItems; i++) {
      ...
      if (!packed) {
        PackageDimension newPackage =
            getNextSuitablePackage(packages, productVolume, productWeight);
        if (newPackage != null) {
  ...
  private PackageDimension getNextSuitablePackage(
      List<PackageDimension> packages, double productVolume, double productWeight) {
    ...
    for (PackageDimension pkg : packages) {
      if (pkg.getAvailableQuantity() > 0
          && (largest == null || pkg.getVolume() > largest.getVolume())) {
        largest = pkg;
      }
    }
  ...
  public static class MultiProductUsedPackage {
    ...
    public PackageDimension getPackageDim() {
      return packageDim;
    }
  }
  ...
  public static class MultiProductPackagingResult {
    ...
    public MultiProductPackagingResult(
      ...
      this.packagesUsed = packagesUsed;
      this.totalPackagesUsed =
          packagesUsed.stream().mapToInt(MultiProductPackageUsageResult::getQuantityUsed).sum();
      this.totalPackagingCost =
          packagesUsed.stream()
      ...
      int totalRequested =
          requestedItemsByProduct.values().stream().mapToInt(Integer::intValue).sum();
    ...
    public List<MultiProductPackageUsageResult> getPackagesUsed() {
      return packagesUsed;
    }
    ...
    public Map<Long, Integer> getPackedItemsByProduct() {
      return packedItemsByProduct;
    }
  ...
  public MultiProductPackagingResult calculatePackagingForMultipleProducts(
      Map<Long, ProductDimension> products, List<PackageDimension> availablePackages) {
    ...
    for (ProductItem item : allItems) {
      ...
      if (!packed) {
        PackageDimension newPackage =
            getNextSuitablePackage(packages, item.getVolume(), item.getWeight());
        if (newPackage != null) {
          newPackage.decrementQuantity();
    ...
    Map<Long, Integer> packageUsageCount = new HashMap<>();
    Map<Long, Map<Long, Integer>> productsByPackage =
        new HashMap<>(); // packageId -> (productId -> count)
    ...
    for (MultiProductUsedPackage usedPkg : usedPackages) {
      ...
      Map<Long, Integer> pkgProducts =
          productsByPackage.computeIfAbsent(pkgId, k -> new HashMap<>());
    ...
    for (PackageDimension pkg : availablePackages) {
      ...
      if (count > 0) {
        results.add(
            new MultiProductPackageUsageResult(
                pkg.getPackageId(),
                pkg.getPackageName(),
                pkg.getPackageType(),
                count,
