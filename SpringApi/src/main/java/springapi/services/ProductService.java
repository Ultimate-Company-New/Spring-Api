...
public class ProductService extends BaseService implements ProductSubTranslator {
   ...
   * location mappings for the product based on the provided pickup location quantities.
   *
  ...
  public void addProduct(ProductRequestModel productRequestModel) {
    Long clientId =
        productRequestModel.getClientId() != null
            ? productRequestModel.getClientId()
            : getClientId();
    persistProduct(productRequestModel, getUser(), clientId);
  }

  /**
   * Edits an existing product. This method updates an existing product with the provided details.
   * All product fields can be updated except the product ID. Deletes old images from ImgBB before
   * uploading new ones. Updates pickup location mappings by deleting existing ones and creating new
   * ones.
   *
  ...
  public void editProduct(ProductRequestModel productRequestModel) {
    ...
    Product existingProduct =
        productRepository.findByIdWithRelatedEntities(
            productRequestModel.getProductId(), getClientId());
    if (existingProduct == null) {
      throw new NotFoundException(
          String.format(
              ErrorMessages.ProductErrorMessages.ER013, productRequestModel.getProductId()));
    }
    ...
    createPickupLocationMappings(
        ...
        productRequestModel.getPickupLocationQuantities(),
        getUser());
    ...
    userLogService.logData(
        getUserId(),
        SuccessMessages.ProductsSuccessMessages.UPDATE_PRODUCT
            + " "
            + updatedProduct.getProductId(),
  ...
  public void toggleReturnProduct(long id) {
    ...
    userLogService.logData(
        getUserId(),
        SuccessMessages.ProductsSuccessMessages.TOGGLE_RETURN_PRODUCT
            + " "
  ...
  public PaginationBaseResponseModel<ProductResponseModel> getProductInBatches(
      ...
      for (PaginationBaseRequestModel.FilterCondition filter :
        ...
        if (filter.getColumn() != null && !validColumns.contains(filter.getColumn())) {
          throw new BadRequestException(
              String.format(
                  ErrorMessages.ProductErrorMessages.INVALID_COLUMN_NAME_FORMAT,
                  filter.getColumn()));
        }
        ...
        if (filter.getOperator() != null && !filter.isValidOperator()) {
          throw new BadRequestException(
              String.format(
                  ErrorMessages.ProductErrorMessages.INVALID_OPERATOR_FORMAT,
                  filter.getOperator()));
        }
        ...
        if (columnType != null
            ...
            && !filter.isValidOperatorForType(columnType)) {
          throw new BadRequestException(
              String.format(
    ...
    PaginationBaseResponseModel<ProductResponseModel> response =
  ...
  public void bulkAddProductsAsync(
    ...
    try {
      ...
      userLogService.logDataWithContext(
          ...
          requestingUserLoginName,
          requestingClientId,
          SuccessMessages.ProductsSuccessMessages.INSERT_PRODUCT
              + " (Bulk: "
              + successCount
              + " succeeded, "
              + failureCount
      ...
      BulkInsertHelper.createDetailedBulkInsertResultMessage(
          ...
          new BulkInsertHelper.BulkMessageTemplate(
              PRODUCT_ENTITY_LABEL, "Products", "Title", "Product ID"),
  ...
  public List<ProductCategoryWithPathResponseModel> findCategoriesByParentId(Long parentId) {
    ...
    if (parentId == null) {
      ...
      categories =
          productCategoryRepository.findAll().stream()
              .filter(cat -> cat.getParentId() != null && cat.getParentId().equals(parentId))
              .collect(Collectors.toCollection(ArrayList::new));
    }
    ...
    for (ProductCategory category : categories) {
      ...
      result.add(
          new ProductCategoryWithPathResponseModel(
   ...
   * hierarchical path (e.g., "Electronics › Computers › Laptops"). Uses the existing
   * buildCategoryFullPath helper method for consistency.
   *
   ...
   */
  private void processAndUploadProductImages(
      ProductRequestModel productRequestModel, Product savedProduct, boolean isUpdate) {
    ...
    for (int i = 0; i < requiredImageData.length; i++) {
      ...
      if (!imageUnchanged) {
        ...
        String customFileName =
            ImgbbHelper.generateCustomFileNameForProductImage(
                environmentName,
        ...
        ImgbbHelper.ImgbbUploadResponse uploadResponse =
            imgbbHelper.uploadFileToImgbb(base64Image, customFileName);
    ...
    for (int i = 0; i < optionalImageData.length; i++) {
      if (optionalImageData[i] != null && !optionalImageData[i].trim().isEmpty()) {
        ...
        if (!imageUnchanged) {
          ...
          if (uploadResponse == null || uploadResponse.getUrl() == null) {
            String errorMsg =
                ProductImageConstants.DEFECT.equals(optionalImageTypes[i])
                    ? String.format(ErrorMessages.ProductErrorMessages.ER010, optionalImageTypes[i])
                    : String.format(
                        ErrorMessages.ProductErrorMessages.ER011, i); // For additional images
   ...
   */
  private void setProductImageUrlAndHash(
      Product product, String imageType, String url, String deleteHash) {
    switch (imageType) {
      case ProductImageConstants.MAIN:
  ...
  private void createPickupLocationMappings(
    ...
    List<ProductPickupLocationMapping> mappings =
        ProductPickupLocationMapping.createFromMap(
            productId, pickupLocationQuantities, createdUser);
   ...
   */
  @Transactional
  protected Product persistProduct(
      ProductRequestModel productRequestModel, String createdUser, Long clientId) {
  ...
  protected Product persistProduct(
    ...
    if (categoryOpt.isEmpty()) {
      throw new NotFoundException(
          String.format(
              ErrorMessages.ProductErrorMessages.ER008, productRequestModel.getCategoryId()));
    }
   ...
   *
   * <p>This method recursively builds the path from root to leaf using the separator " › ".
  ...
  public java.util.List<
    ...
    try {
      ...
      if (mappings.isEmpty()) {
        ...
        logger.debug(
            "Found {} ProductPickupLocationMapping(s) for productId: {}",
            mappings.size(),
            productId);
      }
      ...
      if (e.getMessage() != null && e.getMessage().contains("doesn't exist")) {
        logger.warn(
            "ProductPickupLocationMapping table does not exist for productId: {}. "
                + "Please run the database migration script. Returning empty stock list.",
            productId);
      } else {
        logger.warn(
            "Database resource error for productId: {}. Returning empty stock list.", productId, e);
      }
      ...
      if (e.getMessage() != null && e.getMessage().contains("doesn't exist")) {
        ...
        logger.warn(
            "SQL grammar error for productId: {}. Returning empty stock list.", productId, e);
      }
      mappings = new java.util.ArrayList<>();
    } catch (Exception e) {
