...
public class ClientService extends BaseService implements ClientSubTranslator {
   ...
   * and timestamps. The operation is logged for audit purposes.
   *
   ...
   * and updatedAt fields are updated. The operation is logged for audit purposes.
   *
  ...
  public void updateClient(ClientRequestModel clientRequest) {
    ...
    Client existingClient =
        clientRepository
            .findById(clientRequest.getClientId())
            ...
            duplicateClient -> {
              if (clientId == null || !duplicateClient.getClientId().equals(clientId)) {
                throw new BadRequestException(
                    String.format(
                        ErrorMessages.ClientErrorMessages.DUPLICATE_CLIENT_NAME_FORMAT, name));
              }
            });
  }
  ...
  private void handleImgbbLogo(Client client, String logoBase64, String env, boolean hasNewLogo) {
    ...
    if (!hasNewLogo) {
      ...
      String customFileName =
          ImgbbHelper.generateCustomFileNameForClientLogo(env, client.getName());
      ImgbbHelper.ImgbbUploadResponse uploadResponse =
          imgbbHelper.uploadFileToImgbb(logoBase64, customFileName);
  ...
  private void handleFirebaseLogo(
      Client client, String logoBase64, String env, boolean hasNewLogo) {
