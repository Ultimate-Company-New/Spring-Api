 ...
 *
 * These values must match the frontend PRODUCT_CONDITION_OPTIONS in appConstants.ts.
...
public final class ProductConditionConstants {
    ...
    private static final List<String> VALID_CONDITIONS = Collections.unmodifiableList(Arrays.asList(
            NEW_WITH_TAGS,
            NEW_WITHOUT_TAGS,
            NEW_WITH_DEFECTS,
            PRE_OWNED,
    ...
    public static boolean isValidCondition(String condition) {
        ...
        return VALID_CONDITIONS.stream()
                .anyMatch(valid -> valid.equals(normalized));
    }
