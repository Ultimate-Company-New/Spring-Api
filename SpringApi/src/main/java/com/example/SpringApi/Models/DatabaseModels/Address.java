...
public class Address {
    ...
    private static final Set<String> VALID_ADDRESS_TYPES = new HashSet<>(Set.of(
        "HOME", "WORK", "BILLING", "SHIPPING", "OFFICE", "WAREHOUSE"
    ));
    ...
    public void validateRequest(AddressRequestModel request) {
        ...
        if (request.getAddressType() == null || !VALID_ADDRESS_TYPES.contains(request.getAddressType().toUpperCase())) {
    ...
    private void setFieldsFromRequest(AddressRequestModel request) {
        ...
        this.streetAddress2 = request.getStreetAddress2() != null ? request.getStreetAddress2().trim() : null;
        this.streetAddress3 = request.getStreetAddress3() != null ? request.getStreetAddress3().trim() : null;
        this.city = request.getCity().trim();
        this.state = request.getState().trim();
        this.postalCode = request.getPostalCode().trim();
        this.nameOnAddress = request.getNameOnAddress() != null ? request.getNameOnAddress().trim() : null;
        this.emailOnAddress = request.getEmailOnAddress() != null ? request.getEmailOnAddress().trim() : null;
        this.phoneOnAddress = request.getPhoneOnAddress() != null ? request.getPhoneOnAddress().trim() : null;
        this.country = request.getCountry() != null ? request.getCountry().trim() : "India";
        this.isPrimary = request.getIsPrimary() != null && request.getIsPrimary();  // Default false if null
        this.isDeleted = request.getIsDeleted() != null && request.getIsDeleted();  // Default false if null
    }
}