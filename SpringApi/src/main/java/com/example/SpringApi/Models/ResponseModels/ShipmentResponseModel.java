 ...
 * 
 * This model is used for returning shipment information in API responses.
 * Includes all related entities: Products, Packages, Pickup Location, 
 * Order Summary, and Purchase Order details.
 * 
...
public class ShipmentResponseModel {
    ...
    public ShipmentResponseModel(Shipment shipment) {
        ...
        if (Hibernate.isInitialized(shipment.getPickupLocation()) && shipment.getPickupLocation() != null) {
            this.pickupLocation = new PickupLocationResponseModel(shipment.getPickupLocation());
        }
        
        // Extract created by user info
        if (Hibernate.isInitialized(shipment.getCreatedByUser()) && shipment.getCreatedByUser() != null) {
            this.createdByUser = new UserResponseModel(shipment.getCreatedByUser());
        }
        
        // Extract modified by user info
        if (Hibernate.isInitialized(shipment.getModifiedByUser()) && shipment.getModifiedByUser() != null) {
            this.modifiedByUser = new UserResponseModel(shipment.getModifiedByUser());
        }
        
        // Extract shipment products
        if (Hibernate.isInitialized(shipment.getShipmentProducts()) && shipment.getShipmentProducts() != null) {
        ...
        if (Hibernate.isInitialized(shipment.getShipmentPackages()) && shipment.getShipmentPackages() != null) {
        ...
        if (Hibernate.isInitialized(shipment.getOrderSummary()) && shipment.getOrderSummary() != null) {
            com.example.SpringApi.Models.DatabaseModels.OrderSummary orderSummary = shipment.getOrderSummary();
            if (orderSummary.getEntityType() != null && 
                orderSummary.getEntityType().equals(com.example.SpringApi.Models.DatabaseModels.OrderSummary.EntityType.PURCHASE_ORDER.getValue()) &&
    ...
    /**
     * Sets purchase order ID from the order summary's related purchase order.
     * This should be called after the entity is fetched with the PO data.
     * 
     * @param po The purchase order to extract ID from
     */
    public void setPurchaseOrderFromEntity(com.example.SpringApi.Models.DatabaseModels.PurchaseOrder po) {
        if (po != null) {
    ...
    public static class PackageProductResponseData {
        ...
        public PackageProductResponseData(ShipmentPackageProduct packageProduct) {
            if (packageProduct != null) {
                ...
                // Extract Product
                if (Hibernate.isInitialized(packageProduct.getProduct()) && packageProduct.getProduct() != null) {
