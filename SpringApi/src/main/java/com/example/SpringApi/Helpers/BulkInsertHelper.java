...
public class BulkInsertHelper {
  ...
  public static <T> void createDetailedBulkInsertResultMessage(
    ...
    try {
      ...
      if (response.getTotalRequested() > 0) {
        double successRate =
            (double) response.getSuccessCount() / response.getTotalRequested() * 100;
        htmlContent.append("<div style='padding: 20px; background: white;'>");
        htmlContent
            .append("<div style='margin-bottom: 10px; color: #666;'>Success Rate: <strong>")
      ...
      if (!response.getResults().isEmpty()) {
        ...
        // Success Section
        List<BulkInsertResponseModel.InsertResult<T>> successResults =
        ...
        if (!successResults.isEmpty()) {
          ...
          for (BulkInsertResponseModel.InsertResult<T> result : successResults) {
            ...
            htmlContent
                .append("<td style='padding: 12px; color: #667eea; font-weight: bold;'>")
                .append(result.getEntityId())
                .append(TD_END);
        ...
        // Failure Section
        List<BulkInsertResponseModel.InsertResult<T>> failureResults =
            response.getResults().stream().filter(r -> !r.isSuccess()).toList();
  ...
  public static void createBulkUserGroupInsertResultMessage(
      ...
      Long clientId) {
    BulkMessageTemplate template =
        new BulkMessageTemplate("User Group", "Groups", "Group Name", "Group ID");
    NotificationContext context =
        new NotificationContext(messageService, userId, userLoginName, clientId);

    createDetailedBulkInsertResultMessage(response, template, context);
  }

  /**
   * Creates a beautiful HTML message summarizing bulk user insert results with detailed tables.
   * Note: This uses BulkUserInsertResponseModel which has a different structure than
   * BulkInsertResponseModel.
   *
  ...
  public static void createBulkUserInsertResultMessage(
    ...
    try {
      ...
      if (!response.getResults().isEmpty()) {
        ...
        // Success Section
        List<BulkUserInsertResponseModel.UserInsertResult> successResults =
            response.getResults().stream()
        ...
        // Failure Section
        List<BulkUserInsertResponseModel.UserInsertResult> failureResults =
            response.getResults().stream().filter(r -> !r.isSuccess()).toList();
