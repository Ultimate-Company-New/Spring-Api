...
public class PurchaseOrderFilterQueryBuilder extends BaseFilterQueryBuilder {
  ...
  private static final String SELECTED_IDS_CLAUSE =
      "AND po.purchaseOrderId IN :" + SELECTED_IDS + " ";
  ...
  protected String mapColumnToField(String column) {
    switch (column) {
      ...
      case CLIENT_ID:
        return "po.clientId";
      case "vendorNumber":
        return "po.vendorNumber";
      case "purchaseOrderStatus":
        return "po.purchaseOrderStatus";
      case "priority":
        return "po.priority";
      case "expectedDeliveryDate":
        return "po.expectedDeliveryDate";
      case "purchaseOrderReceipt":
        return "po.purchaseOrderReceipt";
      case "termsConditionsHtml":
        return "po.termsConditionsHtml";
      case "approvedByUserId":
        return "po.approvedByUserId";
      case "approvedDate":
        return "po.approvedDate";
      case "rejectedByUserId":
        return "po.rejectedByUserId";
      case "rejectedDate":
        return "po.rejectedDate";
      case "isDeleted":
        return "po.isDeleted";
      case "notes":
        return "po.notes";
      case "createdUser":
        return "po.createdUser";
      case "modifiedUser":
        return "po.modifiedUser";
      case "createdAt":
        return "po.createdAt";
      case "updatedAt":
        return "po.updatedAt";
      ...
      case "address":
        return "(SELECT CONCAT(COALESCE(a.streetAddress, ''), ' ', COALESCE(a.city, "
  ...
  protected List<String> getDateColumns() {
    return Arrays.asList(
        "expectedDeliveryDate", "approvedDate", "rejectedDate", "createdAt", "updatedAt");
  }
  ...
  public Page<PurchaseOrder> findPaginatedEntitiesWithMultipleFilters(
    ...
    final QueryResult filterResult = buildFilterConditions(filters, logicOperator);
    ...
    // Note: JPQL doesn't support direct joins to unrelated entities, so we'll handle address
    ...
    if (selectedProductIds != null && !selectedProductIds.isEmpty()) {
      baseQuery +=
          "AND EXISTS (SELECT 1 FROM OrderSummary os2, Shipment s, ShipmentProduct sp "
  ...
  /**
   * Finds paginated purchase orders with ALL related data in a single query. Returns Page of.
   * PurchaseOrderWithDetails - extract and build response model from each.
   *
   * @param clientId The client ID to filter purchase orders by
   * @param selectedIds List of specific purchase order IDs to include (null for all)
   * @param selectedProductIds List of product IDs to filter by (null for all)
  ...
  public Page<PurchaseOrderWithDetails> findPaginatedWithDetails(
    ...
    // (Hibernate allows one collection fetch path - shipmentPackages fetched in 2nd query to avoid
    ...
    if (selectedProductIds != null && !selectedProductIds.isEmpty()) {
      baseQuery +=
          "AND EXISTS (SELECT 1 FROM OrderSummary os2, Shipment s2, ShipmentProduct sp2 "
    ...
    String countQuery =
        "SELECT COUNT(DISTINCT po) FROM PurchaseOrder po WHERE po.clientId = "
            + CLIENT_ID_PARAM
            + " ";
    if (selectedIds != null && !selectedIds.isEmpty()) {
      countQuery += SELECTED_IDS_CLAUSE;
    ...
    if (!shipmentIds.isEmpty()) {
      List<ShipmentPackage> allPackages =
          entityManager
              .createQuery(
      ...
      packagesByShipmentId =
          allPackages.stream().collect(Collectors.groupingBy(ShipmentPackage::getShipmentId));
    }
