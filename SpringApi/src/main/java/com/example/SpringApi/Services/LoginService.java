...
public class LoginService implements ILoginSubTranslator {
     ...
     * This method verifies that the token matches the user's stored token and marks
     * the email as confirmed.
     * After successful confirmation, the token is cleared from the database to
     * prevent reuse.
     * If the token is invalid, expired, or the user is not found, appropriate
     * exceptions are thrown.
     *
     * @param loginRequestModel The login request model containing the user ID and
     *                          token for email confirmation.
     * @throws BadRequestException   If the user ID is null or missing.
     * @throws UnauthorizedException If the provided token does not match the user's
     ...
     * Performs several checks: validates input, checks if user exists, email is
     * confirmed, account is not locked,
     * password is set, and password matches. On successful authentication, returns
     * a list of clients the user has access to.
     * On failed attempts, decrements locked attempts and locks the account if
     * attempts reach zero.
     *
     * @param loginRequestModel The login request model containing the login name
     *                          and password.
     * @return A list of ClientResponseModel containing logo, name, clientId, and
     *         apiKey for each client the user has access to.
     * @throws BadRequestException   If login name or password is missing or
     *                               invalid.
     * @throws NotFoundException     If the user with the specified login name is
     *                               not found.
        ...
        if (PasswordHelper.checkPassword(loginRequestModel.getPassword(), user.getPassword(), user.getSalt())) {
            ...
            for (UserClientMapping mapping : userClientMappings) {
                ...
                if (clientOpt.isPresent()) {
                    ...
                    com.example.SpringApi.Models.ResponseModels.ClientResponseModel clientResponse = new com.example.SpringApi.Models.ResponseModels.ClientResponseModel();
     ...
     * Generates a new random password, hashes it, updates the user record, unlocks
     * the account, resets login attempts to 5,
     * sends a reset password email, and logs the action.
     * The client is determined dynamically (currently hardcoded to the first
     * client).
     *
     * @param loginRequestModel The login request model containing the login name
     *                          for password reset.
     * @return True if the password reset is successful.
     * @throws BadRequestException If the login name is missing or the user does not
     *                             have a password set.
     ...
     * @param loginRequestModel The login request model containing the login name
     *                          and API key.
     * @return A JWT token string containing user permissions for the client
    ...
    public String getToken(LoginRequestModel loginRequestModel) {
        ...
        List<UserClientPermissionMapping> userClientPermissionMappings = userClientPermissionMappingRepository
                .findClientPermissionMappingByUserId(user.getUserId());
