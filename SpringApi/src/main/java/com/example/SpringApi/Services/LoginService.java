...
public class LoginService extends BaseService implements ILoginSubTranslator {
   ...
   *     token is missing, or token has been used.
  ...
  public List<com.example.SpringApi.Models.ResponseModels.ClientResponseModel> signIn(
    ...
    if (PasswordHelper.checkPassword(
        loginRequestModel.getPassword(), user.getPassword(), user.getSalt())) {
      ...
      List<UserClientMapping> userClientMappings =
          userClientMappingRepository.findByUserId(user.getUserId());
  ...
  public Boolean resetPassword(LoginRequestModel loginRequestModel) {
    ...
    if (user != null) {
      ...
      EmailTemplates emailTemplates =
          new EmailTemplates(senderName, senderEmail, apiKey, environment, client);
      boolean emailSent =
          emailTemplates.sendResetPasswordEmail(user.getLoginName(), randomPassword);

      // Verify that the email was sent successfully and contains the password
      if (!emailSent) {
        throw new ApplicationException(
            ErrorMessages.LoginErrorMessages.RESET_PASSWORD_EMAIL_FAILED);
      }
   ...
   *
   * <p>This method: 1. Validates the login name and API key are provided 2. Finds the user by login
   * name 3. Finds the UserClientMapping by API key 4. Verifies the API key belongs to the user
   * (userId match) 5. Extracts the clientId from the mapping 6. Fetches user's permissions for that
   * specific client 7. Generates a JWT token with client-specific permissions
   *
   * @param loginRequestModel The login request model containing the login name and API key.
   * @return A JWT token string containing user permissions for the client associated with the API
   *     key.
   * @throws BadRequestException If login name or API key is missing.
   * @throws UnauthorizedException If the user is not found, API key is invalid, or API key doesn't
   *     belong to the user.
   */
  @Override
  ...
  public String getToken(LoginRequestModel loginRequestModel) {
            ...
            .orElseThrow(
                () ->
                    new UnauthorizedException(
                        ErrorMessages.LoginErrorMessages.INVALID_CREDENTIALS));
    ...
    List<Long> permissionIds =
        userClientPermissionMappings.stream()
