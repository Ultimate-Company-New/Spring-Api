...
public class ShippingService extends BaseService implements IShippingSubTranslator {
  ...
  private static final Set<String> VALID_SHIPMENT_COLUMNS =
      Set.of(
          "shipmentId",
          "orderSummaryId",
          "pickupLocationId",
  ...
  public PaginationBaseResponseModel<ShipmentResponseModel> getShipmentsInBatches(
      ...
      for (PaginationBaseRequestModel.FilterCondition filter :
        ...
        if (filter.getColumn() != null && !VALID_SHIPMENT_COLUMNS.contains(filter.getColumn())) {
          throw new BadRequestException(
              String.format(
                  ErrorMessages.ShipmentErrorMessages.INVALID_COLUMN_NAME_FORMAT,
                  filter.getColumn()));
        }
        if (!filter.isValidOperator()) {
          throw new BadRequestException(
              String.format(
                  ErrorMessages.ShipmentErrorMessages.INVALID_OPERATOR_FORMAT,
                  filter.getOperator()));
        }
    ...
    List<ShipmentResponseModel> data =
  ...
  public ShipmentResponseModel getShipmentById(Long shipmentId) {
    ...
    if (shipment.getShipRocketOrderId() == null
        || shipment.getShipRocketOrderId().trim().isEmpty()) {
      throw new NotFoundException(
          String.format(ErrorMessages.ShipmentErrorMessages.NOT_FOUND, shipmentId));
    }
  ...
  public PaymentVerificationResponseModel processShipmentsAfterPaymentApproval(
    ...
    if (!purchaseOrder.getClientId().equals(clientId)) {
      throw new BadRequestException(
          ErrorMessages.CommonErrorMessages.ACCESS_DENIED_TO_PURCHASE_ORDER);
    }

    String status = purchaseOrder.getPurchaseOrderStatus();
    if (!PurchaseOrder.Status.PENDING_APPROVAL.getValue().equals(status)) {
      throw new BadRequestException(
          ErrorMessages.PaymentErrorMessages.ONLY_PENDING_APPROVAL_CAN_BE_PAID);
    }

    OrderSummary orderSummary =
        orderSummaryRepository
            .findByEntityTypeAndEntityId(
                OrderSummary.EntityType.PURCHASE_ORDER.getValue(), purchaseOrderId)
            .orElseThrow(
    ...
    PaymentVerificationResponseModel paymentResponse =
        paymentService.recordCashPayment(cashPaymentRequest);
    ...
    createShipRocketOrdersAndUpdateShipments(
        shipments, orderSummary, purchaseOrder, clientId, userName);

    userLogService.logData(
        userId,
        "Shipments processed successfully for PO #"
            + purchaseOrderId
            + " ("
  ...
  public PaymentVerificationResponseModel processShipmentsAfterPaymentApproval(
    ...
    PaymentVerificationResponseModel paymentResponse =
        paymentService.verifyPayment(razorpayVerifyRequest);
        ...
        if (mapping.getAvailableStock() < shipmentProduct.getAllocatedQuantity()) {
          throw new BadRequestException(
              String.format(
                  ...
                  shipmentProduct.getAllocatedQuantity()));
        }
      }
        ...
        if (mapping.getAvailableQuantity() < shipmentPackage.getQuantityUsed()) {
          throw new BadRequestException(
              String.format(
                  ...
                  shipmentPackage.getPackageId(),
                  pickupLocationId,
                  mapping.getAvailableQuantity(),
                  shipmentPackage.getQuantityUsed()));
        }
      }
    }
  }
  ...
  private void createShipRocketOrdersAndUpdateShipments(
    ...
    if (client.getShipRocketEmail() == null
        || client.getShipRocketEmail().trim().isEmpty()
        || client.getShipRocketPassword() == null
        || client.getShipRocketPassword().trim().isEmpty()) {
      throw new BadRequestException(
          ErrorMessages.ShippingErrorMessages.SHIP_ROCKET_CREDENTIALS_NOT_CONFIGURED);
    }

    ShipRocketHelper shipRocketHelper =
        createShipRocketHelper(client.getShipRocketEmail(), client.getShipRocketPassword());
    ...
    for (Shipment shipment : shipments) {
      PickupLocation pickupLocation =
          pickupLocationRepository
              .findById(shipment.getPickupLocationId())
              .orElseThrow(
                  () ->
                      new NotFoundException(
                          String.format(
    ...
    for (Shipment shipment : shipments) {
      ...
      ShipRocketOrderRequestModel orderRequest =
          buildShipRocketOrderRequest(
      ...
      ShipRocketOrderResponseModel shipRocketResponse =
          shipRocketHelper.createCustomOrder(orderRequest);

      validateShipRocketOrderResponse(shipRocketResponse, shipment.getShipmentId());

      updateShipmentWithShipRocketResponse(
          shipment, shipRocketResponse, shipRocketHelper, userName);
    }
  }
  ...
  private ShipRocketOrderRequestModel buildShipRocketOrderRequest(
    ...
    if (pickupLocation.getAddressNickName() == null
        || pickupLocation.getAddressNickName().trim().isEmpty()) {
      throw new BadRequestException(
          String.format(
    ...
    if (purchaseOrder.getVendorNumber() != null
        && !purchaseOrder.getVendorNumber().trim().isEmpty()) {
    ...
    for (ShipmentProduct shipmentProduct : shipmentProducts) {
      ...
      if (product != null) {
        ...
        String sku =
            product.getUpc() != null && !product.getUpc().trim().isEmpty()
                ? product.getUpc()
        ...
        if (product.getDiscount() != null && product.getDiscount().compareTo(BigDecimal.ZERO) > 0) {
          BigDecimal discountAmount =
              product.getIsDiscountPercent() != null && product.getIsDiscountPercent()
                  ? shipmentProduct
                      .getAllocatedPrice()
        ...
        Integer tax =
            orderSummary.getGstPercentage() != null
                ? orderSummary.getGstPercentage().intValue()
                : null;
        orderItems.add(
            new ShipRocketOrderRequestModel.OrderItem(
                name, sku, units, sellingPrice, discount, tax));
      }
    }
    ...
    if (orderSummary.getTotalShipping() != null
        && orderSummary.getTotalShipping().compareTo(BigDecimal.ZERO) > 0) {
      request.setShippingCharges((int) Math.round(orderSummary.getTotalShipping().doubleValue()));
    }

    if (orderSummary.getTotalDiscount() != null
        && orderSummary.getTotalDiscount().compareTo(BigDecimal.ZERO) > 0) {
          ...
          packageRepository
              ...
              .orElseThrow(
                  () ->
                      new NotFoundException(
                          String.format(
                              ErrorMessages.PackageErrorMessages.INVALID_ID_WITH_ID_FORMAT,
                              shipmentPackage.getPackageId())));
  ...
  private void populateBillingAndShippingFromAddress(
      ShipRocketOrderRequestModel request, Address deliveryAddress, String[] nameParts) {
    int pincode;
    try {
      ...
      throw new BadRequestException(
          String.format(
              ErrorMessages.ShippingErrorMessages.BILLING_POSTAL_CODE_MUST_BE_NUMERIC,
    ...
    if (phone.length() != 10) {
      throw new BadRequestException(
          String.format(
              ErrorMessages.ShippingErrorMessages.BILLING_PHONE_MUST_BE10_DIGITS,
              deliveryAddress.getPhoneOnAddress() != null
                  ? deliveryAddress.getPhoneOnAddress()
                  : "empty"));
    }
    long phoneLong = Long.parseLong(phone);

    String street1 =
        deliveryAddress.getStreetAddress() != null ? deliveryAddress.getStreetAddress() : "";
    String street2 =
    ...
    String email =
        deliveryAddress.getEmailOnAddress() != null ? deliveryAddress.getEmailOnAddress() : "";

    request.setBillingCustomerName(nameParts[0]);
    request.setBillingLastName(
        nameParts.length > 1 && !nameParts[1].isEmpty() ? nameParts[1] : null);
    ...
    request.setShippingEmail(
        deliveryAddress.getEmailOnAddress() != null
                && !deliveryAddress.getEmailOnAddress().trim().isEmpty()
  ...
  private void validateShipRocketOrderResponse(
      ShipRocketOrderResponseModel shipRocketResponse, Long shipmentId) {

    if (shipRocketResponse == null) {
      throw new BadRequestException(
          String.format(
              ErrorMessages.ShippingErrorMessages.SHIP_ROCKET_API_NULL_RESPONSE, shipmentId));
    }

    if (shipRocketResponse.getMessage() != null
        && !shipRocketResponse.getMessage().trim().isEmpty()) {
      throw new BadRequestException(
          String.format(
  ...
  private void updateShipmentWithShipRocketResponse(
    ...
    try {
      ...
      throw new BadRequestException(
          String.format(
              ErrorMessages.ShippingErrorMessages.PICKUP_GENERATION_FAILED,
              shipRocketResponse.getShipmentId(),
              e.getMessage()));
    }
    ...
    try {
      ...
      throw new BadRequestException(
          String.format(
              ErrorMessages.ShippingErrorMessages.LABEL_GENERATION_FAILED,
              shipRocketResponse.getShipmentId(),
              e.getMessage()));
    }
    ...
    try {
      ...
      throw new BadRequestException(
          String.format(
              ErrorMessages.ShippingErrorMessages.INVOICE_GENERATION_FAILED,
              shipRocketResponse.getShipmentId(),
              e.getMessage()));
    }
    ...
    try {
      ...
      throw new BadRequestException(
          String.format(
              ErrorMessages.ShippingErrorMessages.TRACKING_FETCH_FAILED,
              shipment.getShipRocketAwbCode(),
              e.getMessage()));
    }
    ...
    try {
      ...
      try {
        ...
        BadRequestException exception =
            new BadRequestException(
                String.format(
                    ErrorMessages.ShippingErrorMessages
                        .FAILED_TO_SERIALIZE_SHIP_ROCKET_RESPONSE_FORMAT,
  ...
  @Override
  public ShippingCalculationResponseModel calculateShipping(
      ShippingCalculationRequestModel request) {
    if (request == null) {
      throw new NullPointerException(
          ErrorMessages.ShippingErrorMessages.NULL_SHIPPING_CALCULATION_REQUEST);
    }
    ...
    for (ShippingCalculationRequestModel.PickupLocationShipment location :
      ...
      try {
        ShippingOptionsResponseModel shippingOptions =
            shipRocketHelper.getAvailableShippingOptions(
                location.getPickupPostcode(), deliveryPostcode, isCod, weight.toString());

        if (shippingOptions != null
            && shippingOptions.getData() != null
            && shippingOptions.getData().getAvailableCourierCompanies() != null) {

          shippingOptions
              .getData()
              .getAvailableCourierCompanies()
              .sort((a, b) -> Double.compare(a.getRate(), b.getRate()));
          ...
          if (!locationOptions.getAvailableCouriers().isEmpty()) {
            ...
            totalShippingCost =
                totalShippingCost.add(locationOptions.getAvailableCouriers().get(0).getRate());
          }
        }
      } catch (Exception e) {
  ...
  /** Creates a ShiprocketHelper instance with the current client's ShipRocket credentials. */
  ...
  public OrderOptimizationResponseModel optimizeOrder(OrderOptimizationRequestModel request) {
    ...
    if (request.getProductQuantities() == null || request.getProductQuantities().isEmpty()) {
      return OrderOptimizationResponseModel.error(
    ...
    try {
      Map<Long, ProductLocationInfo> productInfoMap =
          fetchProductData(request.getProductQuantities());
      ...
      if (request.getCustomAllocations() != null && !request.getCustomAllocations().isEmpty()) {
        CustomAllocationResult customResult =
            createCustomAllocationCandidate(
                request.getCustomAllocations(), productInfoMap, locationInfoMap);
        ...
        String feasibilityError =
            checkFeasibility(productInfoMap, request.getProductQuantities(), locationInfoMap);
        if (feasibilityError != null) {
          return OrderOptimizationResponseModel.error(feasibilityError);
        }

        candidates =
            generateCandidates(productInfoMap, locationInfoMap, request.getProductQuantities());

        if (candidates.isEmpty()) {
          return OrderOptimizationResponseModel.error(
              ErrorMessages.OrderOptimizationErrorMessages.NO_VALID_ALLOCATION_STRATEGIES_FOUND);
        }
      }
      ...
      boolean isCustomAllocation =
          request.getCustomAllocations() != null && !request.getCustomAllocations().isEmpty();
      ...
      List<AllocationCandidate> validCandidates =
          candidates.stream()
              .filter(c -> c.allCouriersAvailable)
              .sorted(Comparator.comparing(c -> c.totalCost))
              .toList();

      List<AllocationCandidate> invalidCandidates =
          candidates.stream()
              .filter(c -> !c.allCouriersAvailable)
              .sorted(Comparator.comparing(c -> c.totalCost))
              .toList();
      ...
      response.setTotalQuantity(
          request.getProductQuantities().values().stream().mapToInt(Integer::intValue).sum());
  ...
  public void cancelShipment(Long shipmentId) {
    ...
    if (clientResponse.getShipRocketEmail() == null
        || clientResponse.getShipRocketPassword() == null) {
      throw new BadRequestException(
          ErrorMessages.ShippingErrorMessages.SHIP_ROCKET_CREDENTIALS_NOT_CONFIGURED);
    }

    ShipRocketHelper shiprocketHelper =
        createShipRocketHelper(
            clientResponse.getShipRocketEmail(), clientResponse.getShipRocketPassword());
    ...
    try {
      ...
      throw new BadRequestException(
          String.format(
              ErrorMessages.ShipmentErrorMessages.INVALID_ID_FORMAT_ERROR_FORMAT,
              shipRocketOrderId));
    } catch (Exception e) {
      throw new BadRequestException(
          String.format(
              ErrorMessages.ShipmentErrorMessages.INVALID_ID_WITH_MESSAGE_FORMAT, e.getMessage()));
    }
  ...
  public ReturnShipmentResponseModel createReturn(CreateReturnRequestModel request) {
    ...
    if (request.getProducts() == null || request.getProducts().isEmpty()) {
      throw new BadRequestException(
          ErrorMessages.ReturnShipmentErrorMessages.AT_LEAST_ONE_PRODUCT_REQUIRED);
    }

    Shipment shipment =
        shipmentRepository.findByShipmentIdAndClientId(request.getShipmentId(), clientId);
    if (shipment == null) {
      throw new NotFoundException(
    ...
    if (!"DELIVERED".equals(shipment.getShipRocketStatus())) {
      throw new BadRequestException(
          String.format(
              ErrorMessages.ReturnShipmentErrorMessages.ONLY_DELIVERED_CAN_RETURN,
              shipment.getShipRocketStatus()));
    }
    ...
    if (shipment.getShipmentProducts() == null || shipment.getShipmentProducts().isEmpty()) {
      ...
      throw new BadRequestException(
          String.format(
              ErrorMessages.ReturnShipmentErrorMessages.PRODUCT_NOT_IN_SHIPMENT,
              requestedProductId));
    }
    ...
    for (CreateReturnRequestModel.ReturnProductItem item : request.getProducts()) {
      if (item.getProductId() == null) {
        throw new BadRequestException(
            ErrorMessages.ReturnShipmentErrorMessages.PRODUCT_ID_REQUIRED);
      }
      if (item.getQuantity() == null || item.getQuantity() <= 0) {
        throw new BadRequestException(
            ErrorMessages.ReturnShipmentErrorMessages.VALID_QUANTITY_REQUIRED);
      }
      if (item.getReason() == null || item.getReason().isEmpty()) {
        throw new BadRequestException(
            ErrorMessages.ReturnShipmentErrorMessages.RETURN_REASON_REQUIRED);
      }
      ...
      if (item.getQuantity() > shipmentQty) {
        throw new BadRequestException(
            String.format(
                ErrorMessages.ReturnShipmentErrorMessages.RETURN_QUANTITY_EXCEEDS,
                item.getQuantity(),
                shipmentQty,
                item.getProductId()));
      }
          ...
          productRepository
              ...
              .orElseThrow(
                  () ->
                      new NotFoundException(
                          String.format(
                              ErrorMessages.ProductErrorMessages.ER013, item.getProductId())));

      if (product.getReturnWindowDays() != null
          && product.getReturnWindowDays() > 0
          && shipment.getDeliveredDate() != null) {
        java.time.LocalDateTime returnDeadline =
            shipment.getDeliveredDate().plusDays(product.getReturnWindowDays());
        if (java.time.LocalDateTime.now().isAfter(returnDeadline)) {
          throw new BadRequestException(
              String.format(
                  ErrorMessages.ReturnShipmentErrorMessages.PRODUCT_PAST_RETURN_WINDOW,
                  product.getTitle(),
        ...
        throw new BadRequestException(
            String.format(
                ErrorMessages.ReturnShipmentErrorMessages.PRODUCT_NOT_RETURNABLE,
                product.getTitle()));
      }
    ...
    shipRocketRequest.setOrderId(
        "RET-" + shipment.getShipmentId() + "-" + System.currentTimeMillis());
    shipRocketRequest.setOrderDate(java.time.LocalDate.now().toString());

    shipRocketRequest.setPickupCustomerName(
        customerAddress.getNameOnAddress() != null
            ? customerAddress.getNameOnAddress()
            : "Customer");
    shipRocketRequest.setPickupLastName("");
    shipRocketRequest.setCompanyName(
        clientResponse.getName() != null ? clientResponse.getName() : "");
    ...
    shipRocketRequest.setPickupCountry(
        customerAddress.getCountry() != null ? customerAddress.getCountry() : "India");
    shipRocketRequest.setPickupPincode(customerAddress.getPostalCode());
    shipRocketRequest.setPickupEmail(
        customerAddress.getEmailOnAddress() != null ? customerAddress.getEmailOnAddress() : "");
    shipRocketRequest.setPickupPhone(
        customerAddress.getPhoneOnAddress() != null ? customerAddress.getPhoneOnAddress() : "");
    shipRocketRequest.setPickupIsdCode("91");

    shipRocketRequest.setShippingCustomerName(
        warehouseAddress.getNameOnAddress() != null
            ? warehouseAddress.getNameOnAddress()
    ...
    shipRocketRequest.setShippingCountry(
        warehouseAddress.getCountry() != null ? warehouseAddress.getCountry() : "India");
    ...
    for (CreateReturnRequestModel.ReturnProductItem item : request.getProducts()) {
      ...
      ShipRocketReturnOrderRequestModel.ReturnOrderItem orderItem =
    ...
    shipRocketRequest.setLength(
        request.getLength() != null ? request.getLength() : BigDecimal.valueOf(11));
    shipRocketRequest.setBreadth(
        request.getBreadth() != null ? request.getBreadth() : BigDecimal.valueOf(11));
    shipRocketRequest.setHeight(
        request.getHeight() != null ? request.getHeight() : BigDecimal.valueOf(11));
    shipRocketRequest.setWeight(
    ...
    try {
      ...
      returnOrderResponse =
          new Gson().fromJson(returnOrderJson, ShipRocketReturnOrderResponseModel.class);
    } catch (Exception e) {
      throw new BadRequestException(
          String.format(
              ErrorMessages.ReturnShipmentErrorMessages.FAILED_TO_CREATE_RETURN, e.getMessage()));
    }

    ReturnShipment returnShipment =
        ReturnShipment.fromCreateReturn(
            shipment,
            returnType,
            returnOrderResponse,
            request,
    ...
    for (CreateReturnRequestModel.ReturnProductItem item : request.getProducts()) {
      Product product = productsToReturn.get(productIndex++);
      ReturnShipmentProduct returnProduct =
          ReturnShipmentProduct.fromReturnItem(
    ...
    try {
      ...
      ShipRocketAwbResponseModel awbResponse =
          new Gson().fromJson(awbJson, ShipRocketAwbResponseModel.class);
    ...
    returnShipment =
        returnShipmentRepository.findByReturnShipmentIdAndClientId(
  ...
  public void cancelReturnShipment(Long returnShipmentId) {
    ...
    if (ReturnShipment.ReturnStatus.RETURN_CANCELLED
        .getValue()
    ...
    if (shipRocketReturnOrderId == null || shipRocketReturnOrderId.isEmpty()) {
      throw new BadRequestException(
          ErrorMessages.ReturnShipmentErrorMessages.NO_SHIP_ROCKET_ORDER_ID);
    }
    ...
    try {
      ...
      throw new BadRequestException(
          ErrorMessages.ReturnShipmentErrorMessages.INVALID_ID
              + " Format error: "
              + shipRocketReturnOrderId);
    } catch (Exception e) {
      throw new BadRequestException(
          String.format(
              ErrorMessages.ReturnShipmentErrorMessages.FAILED_TO_CANCEL_RETURN, e.getMessage()));
    }

    returnShipment.setShipRocketReturnStatus(
        ReturnShipment.ReturnStatus.RETURN_CANCELLED.getValue());
  ...
  private Map<Long, ProductLocationInfo> fetchProductData(Map<Long, Integer> productQuantities) {
    ...
    for (Product product : products) {
      ...
      info.weightKgs =
          product.getWeightKgs() != null ? product.getWeightKgs() : BigDecimal.valueOf(0.5);
      ...
      for (ProductPickupLocationMapping mapping : mappings) {
        ...
        if (!locationInfoMap.containsKey(locationId)) {
          ...
          locInfo.locationName =
              mapping.getPickupLocation() != null
        ...
        stock.availableStock =
            mapping.getAvailableStock() != null ? mapping.getAvailableStock() : 0;
    ...
    if (!locationInfoMap.isEmpty()) {
      ...
      for (PackagePickupLocationMapping pm : packageMappings) {
        ...
        if (locInfo != null && pm.getPackageEntity() != null) {
          ...
          locInfo.packageDimensions.add(
              new PackagingHelper.PackageDimension(
                  pkg.getPackageId(),
                  pkg.getPackageName(),
                  pkg.getPackageType(),
      ...
      for (Map.Entry<Long, LocationStock> entry : productInfo.stockByLocation.entrySet()) {
        ...
        if (locInfo != null && !locInfo.packageDimensions.isEmpty()) {
          ...
          double productVolume =
              productInfo.length != null
                      && productInfo.breadth != null
                      && productInfo.height != null
                  ? productInfo.length.doubleValue()
                      * productInfo.breadth.doubleValue()
                      * productInfo.height.doubleValue()
                  : 0;
          double productWeight =
              productInfo.weightKgs != null ? productInfo.weightKgs.doubleValue() : 0;
          ...
          if (canFitInAnyPackageType
              || (productInfo.length == null
                  && productInfo.breadth == null
                  && productInfo.height == null)) {
            PackagingHelper.ProductDimension productDim =
                new PackagingHelper.ProductDimension(
                    productInfo.length,
                    productInfo.breadth,
                    productInfo.height,
                    productInfo.weightKgs,
                    stock.availableStock);
            ...
            if (stock.maxItemsPackable == 0 && canFitInAnyPackageType && stock.availableStock > 0) {
              boolean hasPackageQuantity =
                  locInfo.packageDimensions.stream().anyMatch(p -> p.getAvailableQuantity() > 0);

              if (!hasPackageQuantity) {
                stock.maxItemsPackable = stock.availableStock;
                stock.packagingErrorMessage =
                    "Product fits in package types but no packages available (all have 0 quantity)";
              } else {
            ...
            stock.packagingErrorMessage =
                "Product dimensions/weight exceed all available package limits";
          }
        } else {
          stock.maxItemsPackable = 0;
        }
    ...
    for (Map.Entry<Long, Integer> entry : productQuantities.entrySet()) {
      ...
      if (info == null) {
        return String.format(
            ErrorMessages.OrderOptimizationErrorMessages.PRODUCT_NOT_FOUND_FORMAT, productId);
      }
      ...
      boolean hasPackagesConfigured =
          info.stockByLocation.values().stream()
              .anyMatch(s -> s.packageDimensions != null && !s.packageDimensions.isEmpty());

      if (!hasPackagesConfigured && locationInfoMap != null) {
        hasPackagesConfigured =
            info.stockByLocation.keySet().stream()
                .anyMatch(
                    locationId -> {
                      LocationInfo locInfo = locationInfoMap.get(locationId);
                      return locInfo != null
                          && locInfo.packageDimensions != null
                          && !locInfo.packageDimensions.isEmpty();
                    });
      }

      boolean hasAvailablePackages =
          info.stockByLocation.values().stream()
              .anyMatch(
                  s ->
                      s.packageDimensions != null
                          && s.packageDimensions.stream()
      ...
      if (hasPackagesConfigured) {
        double productVolume =
            info.length != null && info.breadth != null && info.height != null
        ...
        canFitInAnyPackageType =
            info.stockByLocation.values().stream()
                .filter(s -> s.packageDimensions != null)
                .flatMap(s -> s.packageDimensions.stream())
                .anyMatch(
                    pkg -> pkg.getVolume() >= productVolume && pkg.getMaxWeight() >= productWeight);
      ...
      if (totalPackable < requestedQty) {
        if (totalStock == 0) {
          return String.format(
              ErrorMessages.OrderOptimizationErrorMessages.INSUFFICIENT_STOCK_ZERO_FORMAT,
              info.productTitle,
              requestedQty);
          ...
          return String.format(
              ErrorMessages.OrderOptimizationErrorMessages.CANNOT_PACKAGE_WITH_DETAIL_FORMAT,
              info.productTitle,
              totalStock,
              errorDetail,
              requestedQty);
          ...
          return String.format(
              ErrorMessages.OrderOptimizationErrorMessages.INSUFFICIENT_STOCK_PACKAGING_FORMAT,
              info.productTitle,
              requestedQty,
              totalStock,
              totalPackable);
  ...
  private List<AllocationCandidate> generateCandidates(
    ...
    for (Long locationId : locationInfoMap.keySet()) {
      if (canLocationFulfillAll(locationId, productInfoMap, productQuantities, locationInfoMap)) {
        AllocationCandidate candidate =
            createSingleLocationCandidate(locationId, productQuantities);
        candidates.add(candidate);
      }
    }

    AllocationCandidate greedyConsolidation =
        createGreedyConsolidationCandidate(productInfoMap, locationInfoMap, productQuantities);
    if (greedyConsolidation.canFulfillOrder) {
      candidates.add(greedyConsolidation);
    }

    AllocationCandidate greedyStock =
        createGreedyStockCandidate(productInfoMap, locationInfoMap, productQuantities);
  ...
  private boolean canLocationFulfillAll(
      Long locationId,
      Map<Long, ProductLocationInfo> productInfoMap,
      Map<Long, Integer> productQuantities,
      Map<Long, LocationInfo> locationInfoMap) {
  ...
  private AllocationCandidate createSingleLocationCandidate(
      Long locationId, Map<Long, Integer> productQuantities) {
  ...
  private CustomAllocationResult createCustomAllocationCandidate(
              ...
              if (packable < qty) {
                errors.add(
                    PRODUCT_ERROR_PREFIX
                        ...
                        + "'. Max packable: "
                        + packable
                        + (stock.packagingErrorMessage != null
                            ? " (" + stock.packagingErrorMessage + ")"
                            : ""));
              } else {
                candidate
    ...
    if (!errors.isEmpty()) {
      return CustomAllocationResult.error(
          String.format(
              ErrorMessages.OrderOptimizationErrorMessages
                  .CUSTOM_ALLOCATION_VALIDATION_FAILED_FORMAT,
              String.join("\n• ", errors)));
    }
  ...
  private AllocationCandidate createGreedyConsolidationCandidate(
    ...
    List<Long> sortedLocations =
        locationInfoMap.keySet().stream()
            .filter(
                locId -> {
            ...
            .sorted(
                (a, b) -> {
                  long countA =
                      productInfoMap.values().stream()
                          .filter(
                              p -> {
                                LocationStock s = p.stockByLocation.get(a);
                                return s != null
                                    && Math.min(s.availableStock, s.maxItemsPackable) > 0;
                              })
                          .count();
                  long countB =
                      productInfoMap.values().stream()
      ...
      List<Long> sortedLocs =
          info.stockByLocation.entrySet().stream()
              ...
              .sorted(
                  (a, b) -> {
                    int availA =
                        Math.min(a.getValue().availableStock, a.getValue().maxItemsPackable);
                    int availB =
                        Math.min(b.getValue().availableStock, b.getValue().maxItemsPackable);
  ...
  private List<AllocationCandidate> removeDuplicateCandidates(
      List<AllocationCandidate> candidates) {
  ...
  private void evaluateCandidates(
    ...
    for (String pickupPostcode : uniquePickupPostcodes) {
      ...
      maxWeightFutures.add(
          CompletableFuture.runAsync(
              () -> {
                try {
                  double maxWeight =
                      findMaxWeightForRoute(shipRocketHelper, postcode, deliveryPostcode, isCod);
                  if (maxWeight > 0) {
    ...
    if (!isCustomAllocation) {
      for (AllocationCandidate candidate : candidates) {
        reallocateFromUnserviceableLocations(
            candidate,
            productInfoMap,
            locationInfoMap,
            serviceableLocationIds,
            unserviceableLocationIds);
      }
    }
        ...
        if (locInfo != null) {
          ...
          if (routeMaxWeight.compareTo(BigDecimal.ZERO) == 0) {
            ...
            routeErrors
                ...
                .append(locationName)
                .append(" [")
                .append(pickupPostcode)
                .append("]")
                .append(" and delivery postcode [")
                .append(deliveryPostcode)
                .append("]")
    ...
    for (AllocationCandidate candidate : candidates) {
      ...
      for (OrderOptimizationResponseModel.Shipment shipment : candidate.shipments) {
        if (shipment.getPackagesUsed() == null || shipment.getPackagesUsed().isEmpty()) {
          ...
          unavailabilityReasons
              .append("No packages available at ")
              .append(locationName)
        ...
        if (pickupPostcode != null) {
          ...
          if (shippingOpts != null
              && shippingOpts.getData() != null
            ...
            for (var courier : shippingOpts.getData().getAvailableCourierCompanies()) {
              shipment
                  .getAvailableCouriers()
                  .add(
                      ShippingCalculationResponseModel.CourierOption.fromShiprocketCourier(
                          courier));
            }
      ...
      candidate.unavailabilityReason =
          !unavailabilityReasons.isEmpty() ? unavailabilityReasons.toString() : null;
    }
  }
  ...
  private List<OrderOptimizationResponseModel.Shipment> splitIntoShipments(
    ...
    if (totalWeight.compareTo(maxWeightPerShipment) <= 0) {
      OrderOptimizationResponseModel.Shipment shipment =
          createShipment(locInfo, productAllocations, packageUsages, totalWeight, totalQty);
    ...
    while (hasRemainingProducts(trackers)) {
      List<OrderOptimizationResponseModel.ProductAllocation> currentShipmentProducts =
          new ArrayList<>();
      ...
      for (ProductAllocationTracker tracker : trackers) {
        if (tracker.remainingQty > 0) {
          ...
          if (tracker.weightPerUnit.compareTo(BigDecimal.ZERO) > 0) {
            unitsCanFit =
                remainingCapacity.divide(tracker.weightPerUnit, 0, RoundingMode.DOWN).intValue();
          } else {
          ...
          if (unitsCanFit > 0) {
            ...
            splitAlloc.setTotalWeight(
                tracker.weightPerUnit.multiply(BigDecimal.valueOf(toAllocate)));
    ...
    for (int i = 0; i < numShipments; i++) {
      ...
      int shipmentQty =
          shipmentProducts.get(i).stream()
              .mapToInt(OrderOptimizationResponseModel.ProductAllocation::getAllocatedQuantity)
              .sum();

      OrderOptimizationResponseModel.Shipment shipment =
          createShipment(
              locInfo,
              shipmentProducts.get(i),
              shipmentPackages.get(i),
              shipmentWeights.get(i),
              shipmentQty);
  ...
  private List<List<OrderOptimizationResponseModel.PackageUsage>> distributePackages(
    ...
    List<Integer> shipmentQuantities =
        shipmentProducts.stream()
            .map(
                prods ->
                    prods.stream()
                        .mapToInt(
                            OrderOptimizationResponseModel.ProductAllocation::getAllocatedQuantity)
                        .sum())
            .collect(Collectors.toCollection(ArrayList::new));
      ...
      for (int i = 0; i < numShipments && remainingPackages > 0; i++) {
        ...
        if (pkgsForShipment > 0) {
          ...
          BigDecimal pricePerUnit =
              pkg.getPackageInfo() != null && pkg.getPackageInfo().getPricePerUnit() != null
                  ? pkg.getPackageInfo().getPricePerUnit()
                  : BigDecimal.ZERO;
          splitPkg.setTotalCost(pricePerUnit.multiply(BigDecimal.valueOf(pkgsForShipment)));

          for (OrderOptimizationResponseModel.ProductAllocation prodAlloc :
              shipmentProducts.get(i)) {
  ...
  private OrderOptimizationResponseModel.Shipment createShipment(
    ...
    if (locInfo.pickupLocationEntity != null) {
      PickupLocationResponseModel pickupLocationResponse =
          new PickupLocationResponseModel(locInfo.pickupLocationEntity);
  ...
  private List<OrderOptimizationResponseModel.PackageUsage> calculatePackagingWithDetails(
    ...
    PackagingHelper.MultiProductPackagingResult estimate =
        packagingHelper.calculatePackagingForMultipleProducts(
            productDimensions, locInfo.packageDimensions);
  ...
  private void populateResponseFromCandidate(
      OrderOptimizationResponseModel response,
      AllocationCandidate candidate,
  ...
  private String generateDescription(AllocationCandidate candidate) {
    ...
    Map<Long, List<OrderOptimizationResponseModel.Shipment>> shipmentsByLocation =
        new LinkedHashMap<>();
    ...
    if (shipmentsByLocation.size() == 1) {
      ...
      String locationName =
          firstShipment.getPickupLocation() != null
      ...
      return "Split: "
          + shipmentsByLocation.entrySet().stream()
              .map(
                  entry -> {
                    List<OrderOptimizationResponseModel.Shipment> locShipments = entry.getValue();
                    String name =
                        locShipments.get(0).getPickupLocation() != null
                            ? locShipments.get(0).getPickupLocation().getAddressNickName()
                            : "Location";
                    int totalItems =
                        locShipments.stream()
                            .mapToInt(OrderOptimizationResponseModel.Shipment::getTotalQuantity)
                            .sum();
  ...
  private double findMaxWeightForRoute(
    ...
    for (double weight : weightsToTry) {
      try {
        ShippingOptionsResponseModel response =
            shipRocketHelper.getAvailableShippingOptions(
