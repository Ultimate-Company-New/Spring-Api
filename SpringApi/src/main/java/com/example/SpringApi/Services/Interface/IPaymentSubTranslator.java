 ...
 *
 * This interface defines the contract for Razorpay payment operations including
 * order creation, payment verification, cash payment recording, refunds, and receipt generation.
 ...
 * @since 2024-01-15
 */
public interface IPaymentSubTranslator {

    /**
     * Creates a Razorpay order for a purchase order payment.
     *
     * This method creates a Razorpay order and returns the necessary details for the frontend
     * to open the Razorpay checkout modal. Only PENDING_APPROVAL orders can be paid.
     *
     ...
     * @throws com.example.SpringApi.Exceptions.BadRequestException if validation fails or Razorpay API fails
     * @throws com.example.SpringApi.Exceptions.NotFoundException if purchase order or client not found
     */
     ...
     * to create additional payment orders for remaining balance.
     *
     * @param request The order request containing purchaseOrderId and optional amount
     * @return RazorpayOrderResponseModel containing order details for frontend
     * @throws com.example.SpringApi.Exceptions.BadRequestException if validation fails or Razorpay API fails
     ...
     *
     * This method verifies the payment signature, marks the payment as captured,
     * and updates the purchase order status to APPROVED or APPROVED_WITH_PARTIAL_PAYMENT.
     *
     ...
     * @throws com.example.SpringApi.Exceptions.NotFoundException if purchase order or payment not found
     */
     ...
     *
     * Creates a Payment record with CASH or UPI payment method and updates the purchase order
     ...
     *
     * This method allows payments for APPROVED or APPROVED_WITH_PARTIAL_PAYMENT orders.
     ...
     *
     * This is the public key and is safe to expose. Used when the frontend needs to
     * initialize Razorpay without creating an order first.
     *
     * @return The Razorpay Key ID (public API key)
     * @throws com.example.SpringApi.Exceptions.NotFoundException if client not found or Razorpay not configured
     */
     ...
     *
     * This method fetches the payment, validates client access, and generates a formatted
     * PDF receipt using the payment, purchase order, and client information.
     *
     * @param paymentId The ID of the payment to generate receipt for
     * @return The PDF as a byte array
     * @throws com.example.SpringApi.Exceptions.BadRequestException if validation fails
     * @throws com.example.SpringApi.Exceptions.NotFoundException if payment or purchase order not found
     * @throws TemplateException if PDF template processing fails
     * @throws IOException if PDF generation fails
     * @throws DocumentException if PDF document creation fails
     */
    byte[] generatePaymentReceiptPDF(Long paymentId) throws TemplateException, IOException, DocumentException;
     ...
     * @throws com.example.SpringApi.Exceptions.BadRequestException if validation fails or refund cannot be processed
     * @throws com.example.SpringApi.Exceptions.NotFoundException if payment not found
     */
