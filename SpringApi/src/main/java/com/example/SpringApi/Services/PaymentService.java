 ...
 * transactions are tracked in the Payment table.
 *
...
public class PaymentService extends BaseService implements PaymentSubTranslator {
    ...
    if (amount == null || amount.compareTo(BigDecimal.ZERO) <= 0) {
      OrderSummary orderSummary =
          orderSummaryRepository
              ...
              .orElseThrow(
                  () ->
                      new BadRequestException(ErrorMessages.OrderSummaryNotFoundMessage.NOT_FOUND));
  ...
  public RazorpayOrderResponseModel createOrderFollowUp(RazorpayOrderRequestModel request) {
    ...
    if (!PurchaseOrder.Status.APPROVED.getValue().equals(status)
      ...
      throw new BadRequestException(
          ErrorMessages.PaymentErrorMessages.FOLLOW_UP_PAYMENT_STATUS_REQUIRED);
    }
    ...
    if (amount == null || amount.compareTo(BigDecimal.ZERO) <= 0) {
      ...
      Long totalPaidPaise =
          paymentRepository.getTotalNetPaidPaiseForEntity(
              Payment.EntityType.PURCHASE_ORDER.getValue(), purchaseOrder.getPurchaseOrderId());
    ...
    try {
      ...
      // Set description
      payment.setDescription(
          "Follow-up payment for Purchase Order #"
              + purchaseOrder.getPurchaseOrderId()
              + " ("
              + purchaseOrder.getVendorNumber()
              + ")");
  ...
  public PaymentVerificationResponseModel verifyPayment(RazorpayVerifyRequestModel request) {
            ...
            .orElseThrow(
                () ->
                    new BadRequestException(
                        ErrorMessages.PaymentErrorMessages.PAYMENT_ORDER_NOT_FOUND));
    ...
    if (!isValidSignature) {
      ...
      return PaymentVerificationResponseModel.failure(
    ...
    // Get updated status
    purchaseOrder =
        purchaseOrderRepository.findById(purchaseOrder.getPurchaseOrderId()).orElse(purchaseOrder);
  ...
  public PaymentVerificationResponseModel recordCashPayment(CashPaymentRequestModel request) {
    ...
    if (request.getAmount() == null || request.getAmount().compareTo(BigDecimal.ZERO) <= 0) {
      throw new BadRequestException(
          ErrorMessages.PaymentErrorMessages.VALID_PAYMENT_AMOUNT_REQUIRED);
    }
    ...
    String paymentMethod =
        (request.getUpiTransactionId() != null && !request.getUpiTransactionId().trim().isEmpty())
    ...
    manualPaymentData.setDescription(
        "Cash payment for Purchase Order #"
            + purchaseOrder.getPurchaseOrderId()
            + " ("
            + purchaseOrder.getVendorNumber()
            + ")");
   ...
   * This method allows payments for APPROVED or APPROVED_WITH_PARTIAL_PAYMENT status. Does NOT
  ...
  @Override
  @Transactional
  public PaymentVerificationResponseModel verifyPaymentFollowUp(
      RazorpayVerifyRequestModel request) {
    // Get client with Razorpay credentials (for signature verification)
    Client client = getClientWithRazorpayCredentials();
    ...
    userLogService.logData(
        getUserId(),
        "Follow-up payment verified for PO #"
            + purchaseOrder.getPurchaseOrderId()
            + " ("
            + purchaseOrder.getVendorNumber()
            + "). Payment ID: "
  ...
  @Override
  @Transactional
  public PaymentVerificationResponseModel recordCashPaymentFollowUp(
    ...
    manualPaymentData.setUpiTransactionId(request.getUpiTransactionId());
    manualPaymentData.setDescription(
        "Follow-up cash payment for Purchase Order #"
            + purchaseOrder.getPurchaseOrderId()
            + " ("
            + purchaseOrder.getVendorNumber()
            + ")");
    ...
    userLogService.logData(
        getUserId(),
        "Follow-up cash payment recorded for PO #"
            + purchaseOrder.getPurchaseOrderId()
            + " ("
            + purchaseOrder.getVendorNumber()
            + "). Amount: ₹"
  ...
  public boolean isPurchaseOrderPaid(Long purchaseOrderId) {
    return paymentRepository.hasSuccessfulPayment(
  ...
  public Payment initiateRefund(Long paymentId, Long amountInPaise, String reason) {
    ...
    try {
      ...
      if (Payment.EntityType.PURCHASE_ORDER.getValue().equals(payment.getEntityType())) {
        OrderSummary orderSummary =
            orderSummaryRepository.findByPurchaseOrderId(payment.getEntityId()).orElse(null);
  ...
  private Client getClientWithRazorpayCredentials() {
    ...
    if (client.getRazorpayApiKey() == null || client.getRazorpayApiKey().trim().isEmpty()) {
      throw new BadRequestException(
          ErrorMessages.PaymentErrorMessages.RAZORPAY_API_KEY_NOT_CONFIGURED);
    }
    if (client.getRazorpayApiSecret() == null || client.getRazorpayApiSecret().trim().isEmpty()) {
      throw new BadRequestException(
          ErrorMessages.PaymentErrorMessages.RAZORPAY_API_SECRET_NOT_CONFIGURED);
    }
  ...
  private boolean isTestMode() {
    return Arrays.stream(environment.getActiveProfiles())
        .anyMatch(
            profile ->
                "localhost".equalsIgnoreCase(profile)
  ...
  private boolean verifyRazorpaySignature(
      String orderId, String paymentId, String signature, String apiSecret) {
    try {
   ...
   */
  private String formPaymentReceiptHtml(Client client, Payment payment, PurchaseOrder purchaseOrder)
    ...
    if (paymentDateTime != null) {
      templateData.put(
          "paymentDate", paymentDateTime.format(DateTimeFormatter.ofPattern("MMM dd, yyyy HH:mm")));
    } else {
    ...
    templateData.put(
        "paymentGateway",
        payment.getPaymentGateway() != null ? payment.getPaymentGateway() : "N/A");
    ...
    templateData.put(
        JSON_FIELD_CURRENCY, payment.getCurrency() != null ? payment.getCurrency() : "INR");

    if (payment.getRazorpayFee() != null
        && payment.getRazorpayFee().compareTo(BigDecimal.ZERO) > 0) {
      templateData.put("razorpayFee", payment.getRazorpayFee().toString());
    }
    if (payment.getRazorpayTax() != null
        && payment.getRazorpayTax().compareTo(BigDecimal.ZERO) > 0) {
