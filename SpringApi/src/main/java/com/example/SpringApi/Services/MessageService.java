...
public class MessageService extends BaseService implements IMessageSubTranslator {
     ...
     * Uses JPQL with LEFT JOIN FETCH to eagerly load MessageUserMap and
     * MessageUserGroupMap collections.
     *
     * @param paginationBaseRequestModel The request model containing pagination and
     *                                   filter parameters
     * @return PaginationBaseResponseModel containing paginated message data with
     ...
     * This operation is transactional to ensure all related records are created
     * atomically.
     * 
     ...
     * This variant is used when the security context is not available (e.g., async
     * methods).
     *
    ...
    public void createMessageWithContext(MessageRequestModel messageRequestModel, Long requestingUserId,
        ...
        if (Boolean.TRUE.equals(savedMessage.getSendAsEmail())) {
            ...
            List<Long> userIds = messageRequestModel.getUserIds() != null ? messageRequestModel.getUserIds()
                    : Collections.emptyList();
            List<Long> groupIds = messageRequestModel.getUserGroupIds() != null ? messageRequestModel.getUserGroupIds()
                    : Collections.emptyList();
     ...
     * If the message is deleted, it will be restored. If not deleted, it will be
     * marked as deleted.
     * 
    ...
    */
     ...
     * Includes messages where the user is directly targeted or belongs to a
     * targeted group.
     * Only returns non-deleted messages.
     * Orders unread messages first, then read messages, both ordered by messageId
     * DESC.
     * 
     * @param paginationBaseRequestModel The pagination parameters (only start/end
     *                                   are used)
     * @return PaginationBaseResponseModel containing paginated message data with
     *         read status
    ...
    */
    ...
    public PaginationBaseResponseModel<MessageResponseModel> getMessagesByUserId(
        ...
        List<MessageResponseModel> messageResponseModels = page.getContent().stream()
                .map(message -> {
                    ...
                    model.setIsRead(messageUserReadMapRepository.findByMessageIdAndUserId(message.getMessageId(),
                            userId) != null);
                    return model;
                })
                .toList();
     ...
     * Creates a record in MessageUserReadMap to track when the user read the
     * message.
     * 
