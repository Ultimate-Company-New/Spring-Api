...
@Repository
public interface ProductPickupLocationMappingRepository
    extends JpaRepository<ProductPickupLocationMapping, Long> {
  ...
  List<ProductPickupLocationMapping> findByProductIdAndClientId(
   ...
   * @return List of ProductPickupLocationMappings
   */
  List<ProductPickupLocationMapping> findByPickupLocationId(
   ...
   */
  @Query(
      "SELECT pplm.pickupLocationId, COUNT(pplm) FROM ProductPickupLocationMapping pplm "
   ...
   * Fetches pickup location with address for distance calculation. Only returns locations that have
   * a valid address (excludes bad data where address is missing).
   *
   * @param productId The product ID
  ...
  List<ProductPickupLocationMapping> findByProductIdWithPickupLocationAndAddress(
