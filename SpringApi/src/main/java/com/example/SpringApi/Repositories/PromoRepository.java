...
public interface PromoRepository extends JpaRepository<Promo, Long> {
  ...
  Optional<Promo> findByPromoCodeAndClientId(
      @Param("promoCode") String promoCode, @Param("clientId") Long clientId);

  @Query("SELECT p FROM Promo p WHERE p.promoId = :promoId AND p.clientId = :clientId")
  Optional<Promo> findByPromoIdAndClientId(
   ...
   * new start (or existing has no end) Case 2: Existing promo has no expiry - overlaps if new
   * promo's end is after existing start (or new has no end) Case 3: Both have expiry dates -
   * standard overlap: new.start <= existing.end AND new.end >= existing.start
   *
