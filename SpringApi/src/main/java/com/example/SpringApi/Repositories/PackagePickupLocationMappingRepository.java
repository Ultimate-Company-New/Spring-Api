...
public interface PackagePickupLocationMappingRepository
  ...
  @Query(
      "select m from PackagePickupLocationMapping m join m.packageEntity p "
          + "where m.pickupLocationId = :pickupLocationId and p.clientId = :"
          + "clientId order by p.packageId desc")
   ...
   */
  @Query(
      "SELECT COUNT(m) FROM PackagePickupLocationMapping m WHERE "
          + "m.pickupLocationId = :pickupLocationId")
