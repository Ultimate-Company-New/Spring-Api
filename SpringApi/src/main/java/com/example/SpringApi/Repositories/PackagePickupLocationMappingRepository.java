...
public interface PackagePickupLocationMappingRepository
  ...
  @Query(
      "select m from PackagePickupLocationMapping m join m.packageEntity p where m.pickupLocationId = :pickupLocationId and p.clientId = :clientId order by p.packageId desc")
  List<PackagePickupLocationMapping> findByPickupLocationIdAndClientId(
   ...
   */
  @Query(
      "SELECT COUNT(m) FROM PackagePickupLocationMapping m WHERE m.pickupLocationId = :pickupLocationId")
  Integer countByPickupLocationId(@Param("pickupLocationId") Long pickupLocationId);

  /**
   * Get counts of PackagePickupLocationMappings grouped by pickup location ID for multiple
   ...
   */
  @Query(
      "SELECT m.pickupLocationId, COUNT(m) FROM PackagePickupLocationMapping m "
   ...
   */
  @Query(
      "SELECT m FROM PackagePickupLocationMapping m "
  ...
  List<PackagePickupLocationMapping> findByPickupLocationIdsWithPackages(
      @Param("pickupLocationIds") List<Long> pickupLocationIds);
