...
/**
 * Repository for TestRunResult entity.
 * Provides methods for querying individual test results.
 */
@Repository
public interface TestRunResultRepository extends JpaRepository<TestRunResult, Long> {
     ...
     */
    List<TestRunResult> findByClientIdAndServiceNameOrderByExecutedAtDesc(Long clientId, String serviceName);

    /**
     * Find results by test method name
     */
    List<TestRunResult> findByClientIdAndTestMethodNameOrderByExecutedAtDesc(Long clientId, String testMethodName);
     ...
     */
    @Query("SELECT trr FROM TestRunResult trr WHERE trr.clientId = :clientId " +
     ...
     */
    @Query("SELECT trr.status, COUNT(trr) FROM TestRunResult trr WHERE trr.testRunId = :testRunId " +
           "GROUP BY trr.status")
