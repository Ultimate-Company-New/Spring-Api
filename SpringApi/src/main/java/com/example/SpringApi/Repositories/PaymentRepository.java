 ...
 * Provides methods for payment tracking and Razorpay integration.
 */
@Repository
public interface PaymentRepository extends JpaRepository<Payment, Long> {
     ...
     */
    @Query("SELECT p FROM Payment p WHERE p.entityType = :entityType AND p.entityId = :entityId " +
     ...
     */
    Optional<Payment> findFirstByEntityTypeAndEntityIdOrderByCreatedAtDesc(String entityType, Long entityId);
     ...
     */
    List<Payment> findByClientIdAndPaymentStatusOrderByCreatedAtDesc(Long clientId, String paymentStatus);
     ...
     */
    @Query("SELECT COALESCE(SUM(p.amountPaidPaise - COALESCE(p.amountRefundedPaise, 0)), 0) " +
