...
public interface MessageUserReadMapRepository extends JpaRepository<MessageUserReadMap, Long> {
    ...
    @Query("SELECT murm FROM MessageUserReadMap murm WHERE murm.messageId = :messageId AND murm.userId = :userId")
    MessageUserReadMap findByMessageIdAndUserId(@Param("messageId") Long messageId, @Param("userId") Long userId);
}
